/* ==========================================
   PicDuel
   categories.js
========================================== */

window.categories = {

chemicalElements: [

{
    answers: ["υδρογόνο", "hydrogen"],
    image: "images/elements/hydrogen.png",
    difficulty: "easy"
},

{
    answers: ["ήλιο", "helium"],
    image: "images/elements/helium.png",
    difficulty: "easy"
},

{
    answers: ["λίθιο", "lithium"],
    image: "images/elements/lithium.png",
    difficulty: "easy"
},

{
    answers: ["βηρύλλιο", "beryllium"],
    image: "images/elements/beryllium.png",
    difficulty: "easy"
},

{
    answers: ["βόριο", "boron"],
    image: "images/elements/boron.png",
    difficulty: "easy"
},

{
    answers: ["άνθρακας", "carbon"],
    image: "images/elements/carbon.png",
    difficulty: "easy"
},

{
    answers: ["άζωτο", "nitrogen"],
    image: "images/elements/nitrogen.png",
    difficulty: "easy"
},

{
    answers: ["οξυγόνο", "oxygen"],
    image: "images/elements/oxygen.png",
    difficulty: "easy"
},

{
    answers: ["φθόριο", "fluorine"],
    image: "images/elements/fluorine.png",
    difficulty: "easy"
},

{
    answers: ["νέον", "neon"],
    image: "images/elements/neon.png",
    difficulty: "easy"
},

{
    answers: ["νάτριο", "sodium"],
    image: "images/elements/sodium.png",
    difficulty: "easy"
},

{
    answers: ["μαγνήσιο", "magnesium"],
    image: "images/elements/magnesium.png",
    difficulty: "easy"
},

{
    answers: ["αλουμίνιο", "aluminium", "aluminum"],
    image: "images/elements/aluminium.png",
    difficulty: "easy"
},

{
    answers: ["πυρίτιο", "silicon"],
    image: "images/elements/silicon.png",
    difficulty: "easy"
},

{
    answers: ["φώσφορος", "phosphorus"],
    image: "images/elements/phosphorus.png",
    difficulty: "easy"
},

{
    answers: ["θείο", "sulfur", "sulphur"],
    image: "images/elements/sulfur.png",
    difficulty: "easy"
},

{
    answers: ["χλώριο", "chlorine"],
    image: "images/elements/chlorine.png",
    difficulty: "easy"
},

{
    answers: ["αργό", "argon"],
    image: "images/elements/argon.png",
    difficulty: "easy"
},

{
    answers: ["κάλιο", "potassium"],
    image: "images/elements/potassium.png",
    difficulty: "easy"
},

{
    answers: ["ασβέστιο", "calcium"],
    image: "images/elements/calcium.png",
    difficulty: "easy"
},

{
    answers: ["σκάνδιο", "scandium"],
    image: "images/elements/scandium.png",
    difficulty: "medium"
},

{
    answers: ["τιτάνιο", "titanium"],
    image: "images/elements/titanium.png",
    difficulty: "easy"
},

{
    answers: ["βανάδιο", "vanadium"],
    image: "images/elements/vanadium.png",
    difficulty: "medium"
},

{
    answers: ["χρώμιο", "chromium"],
    image: "images/elements/chromium.png",
    difficulty: "easy"
},

{
    answers: ["μαγγάνιο", "manganese"],
    image: "images/elements/manganese.png",
    difficulty: "easy"
},

{
    answers: ["σίδηρος", "iron"],
    image: "images/elements/iron.png",
    difficulty: "easy"
},

{
    answers: ["κοβάλτιο", "cobalt"],
    image: "images/elements/cobalt.png",
    difficulty: "easy"
},

{
    answers: ["νικέλιο", "nickel"],
    image: "images/elements/nickel.png",
    difficulty: "easy"
},

{
    answers: ["χαλκός", "copper"],
    image: "images/elements/copper.png",
    difficulty: "easy"
},

{
    answers: ["ψευδάργυρος", "zinc"],
    image: "images/elements/zinc.png",
    difficulty: "easy"
},

{
    answers: ["γάλλιο", "gallium"],
    image: "images/elements/gallium.png",
    difficulty: "medium"
},

{
    answers: ["γερμάνιο", "germanium"],
    image: "images/elements/germanium.png",
    difficulty: "medium"
},

{
    answers: ["αρσενικό", "arsenic"],
    image: "images/elements/arsenic.png",
    difficulty: "medium"
},

{
    answers: ["σελήνιο", "selenium"],
    image: "images/elements/selenium.png",
    difficulty: "medium"
},

{
    answers: ["βρώμιο", "bromine"],
    image: "images/elements/bromine.png",
    difficulty: "easy"
},

{
    answers: ["κρυπτό", "krypton"],
    image: "images/elements/krypton.png",
    difficulty: "medium"
},

{
    answers: ["ρουβίδιο", "rubidium"],
    image: "images/elements/rubidium.png",
    difficulty: "medium"
},

{
    answers: ["στρόντιο", "strontium"],
    image: "images/elements/strontium.png",
    difficulty: "medium"
},

{
    answers: ["ύττριο", "yttrium"],
    image: "images/elements/yttrium.png",
    difficulty: "medium"
},

{
    answers: ["ζιρκόνιο", "zirconium"],
    image: "images/elements/zirconium.png",
    difficulty: "medium"
},

{
    answers: ["νιόβιο", "niobium"],
    image: "images/elements/niobium.png",
    difficulty: "medium"
},

{
    answers: ["μολυβδαίνιο", "molybdenum"],
    image: "images/elements/molybdenum.png",
    difficulty: "medium"
},

{
    answers: ["τεχνήτιο", "technetium"],
    image: "images/elements/technetium.png",
    difficulty: "medium"
},

{
    answers: ["ρουθήνιο", "ruthenium"],
    image: "images/elements/ruthenium.png",
    difficulty: "medium"
},

{
    answers: ["ρόδιο", "rhodium"],
    image: "images/elements/rhodium.png",
    difficulty: "medium"
},

{
    answers: ["παλλάδιο", "palladium"],
    image: "images/elements/palladium.png",
    difficulty: "medium"
},

{
    answers: ["άργυρος", "silver"],
    image: "images/elements/silver.png",
    difficulty: "easy"
},

{
    answers: ["κάδμιο", "cadmium"],
    image: "images/elements/cadmium.png",
    difficulty: "medium"
},

{
    answers: ["ίνδιο", "indium"],
    image: "images/elements/indium.png",
    difficulty: "medium"
},

{
    answers: ["κασσίτερος", "tin"],
    image: "images/elements/tin.png",
    difficulty: "easy"
},

{
    answers: ["αντιμόνιο", "antimony"],
    image: "images/elements/antimony.png",
    difficulty: "medium"
},

{
    answers: ["τελλούριο", "tellurium"],
    image: "images/elements/tellurium.png",
    difficulty: "medium"
},

{
    answers: ["ιώδιο", "iodine"],
    image: "images/elements/iodine.png",
    difficulty: "easy"
},

{
    answers: ["ξένο", "xenon"],
    image: "images/elements/xenon.png",
    difficulty: "medium"
},

{
    answers: ["καίσιο", "caesium", "cesium"],
    image: "images/elements/caesium.png",
    difficulty: "medium"
},

{
    answers: ["βάριο", "barium"],
    image: "images/elements/barium.png",
    difficulty: "medium"
},

{
    answers: ["λανθάνιο", "lanthanum"],
    image: "images/elements/lanthanum.png",
    difficulty: "medium"
},

{
    answers: ["δημήτριο", "cerium"],
    image: "images/elements/cerium.png",
    difficulty: "medium"
},

{
    answers: ["πρασεοδύμιο", "praseodymium"],
    image: "images/elements/praseodymium.png",
    difficulty: "hard"
},

{
    answers: ["νεοδύμιο", "neodymium"],
    image: "images/elements/neodymium.png",
    difficulty: "hard"
},

{
    answers: ["προμήθιο", "promethium"],
    image: "images/elements/promethium.png",
    difficulty: "hard"
},

{
    answers: ["σαμάριο", "samarium"],
    image: "images/elements/samarium.png",
    difficulty: "medium"
},

{
    answers: ["ευρώπιο", "europium"],
    image: "images/elements/europium.png",
    difficulty: "medium"
},

{
    answers: ["γαδολίνιο", "gadolinium"],
    image: "images/elements/gadolinium.png",
    difficulty: "medium"
},

{
    answers: ["τέρβιο", "terbium"],
    image: "images/elements/terbium.png",
    difficulty: "medium"
},

{
    answers: ["δυσπρόσιο", "dysprosium"],
    image: "images/elements/dysprosium.png",
    difficulty: "medium"
},

{
    answers: ["όλμιο", "holmium"],
    image: "images/elements/holmium.png",
    difficulty: "medium"
},

{
    answers: ["έρβιο", "erbium"],
    image: "images/elements/erbium.png",
    difficulty: "medium"
},

{
    answers: ["θούλιο", "thulium"],
    image: "images/elements/thulium.png",
    difficulty: "medium"
},

{
    answers: ["υττέρβιο", "ytterbium"],
    image: "images/elements/ytterbium.png",
    difficulty: "medium"
},

{
    answers: ["λουτήτιο", "lutetium"],
    image: "images/elements/lutetium.png",
    difficulty: "medium"
},

{
    answers: ["άφνιο", "hafnium"],
    image: "images/elements/hafnium.png",
    difficulty: "medium"
},

{
    answers: ["ταντάλιο", "tantalum"],
    image: "images/elements/tantalum.png",
    difficulty: "medium"
},

{
    answers: ["βολφράμιο", "tungsten", "wolfram"],
    image: "images/elements/tungsten.png",
    difficulty: "medium"
},

{
    answers: ["ρήνιο", "rhenium"],
    image: "images/elements/rhenium.png",
    difficulty: "medium"
},

{
    answers: ["όσμιο", "osmium"],
    image: "images/elements/osmium.png",
    difficulty: "medium"
},

{
    answers: ["ιρίδιο", "iridium"],
    image: "images/elements/iridium.png",
    difficulty: "hard"
},

{
    answers: ["πλατίνα", "platinum"],
    image: "images/elements/platinum.png",
    difficulty: "easy"
},

{
    answers: ["χρυσός", "gold"],
    image: "images/elements/gold.png",
    difficulty: "easy"
},

{
    answers: ["υδράργυρος", "mercury"],
    image: "images/elements/mercury.png",
    difficulty: "easy"
},

{
    answers: ["θάλλιο", "thallium"],
    image: "images/elements/thallium.png",
    difficulty: "hard"
},

{
    answers: ["μόλυβδος", "lead"],
    image: "images/elements/lead.png",
    difficulty: "easy"
},

{
    answers: ["βισμούθιο", "bismuth"],
    image: "images/elements/bismuth.png",
    difficulty: "hard"
},

{
    answers: ["πολώνιο", "polonium"],
    image: "images/elements/polonium.png",
    difficulty: "hard"
},

{
    answers: ["άστατο", "astatine"],
    image: "images/elements/astatine.png",
    difficulty: "hard"
},

{
    answers: ["ραδόνιο", "radon"],
    image: "images/elements/radon.png",
    difficulty: "hard"
},

{
    answers: ["φράγκιο", "francium"],
    image: "images/elements/francium.png",
    difficulty: "hard"
},

{
    answers: ["ράδιο", "radium"],
    image: "images/elements/radium.png",
    difficulty: "easy"
},

{
    answers: ["ακτίνιο", "actinium"],
    image: "images/elements/actinium.png",
    difficulty: "hard"
},

{
    answers: ["θόριο", "thorium"],
    image: "images/elements/thorium.png",
    difficulty: "hard"
},

{
    answers: ["πρωτακτίνιο", "protactinium"],
    image: "images/elements/protactinium.png",
    difficulty: "hard"
},

{
    answers: ["ουράνιο", "uranium"],
    image: "images/elements/uranium.png",
    difficulty: "easy"
},

{
    answers: ["ποσειδώνιο", "neptunium"],
    image: "images/elements/neptunium.png",
    difficulty: "hard"
},

{
    answers: ["πλουτώνιο", "plutonium"],
    image: "images/elements/plutonium.png",
    difficulty: "easy"
},

{
    answers: ["αμερίκιο", "americium"],
    image: "images/elements/americium.png",
    difficulty: "hard"
},

{
    answers: ["κιούριο", "curium"],
    image: "images/elements/curium.png",
    difficulty: "hard"
},

{
    answers: ["βερκέλιο", "berkelium"],
    image: "images/elements/berkelium.png",
    difficulty: "hard"
},

{
    answers: ["καλιφόρνιο", "californium"],
    image: "images/elements/californium.png",
    difficulty: "hard"
},

{
    answers: ["αϊνσταΐνιο", "einsteinium"],
    image: "images/elements/einsteinium.png",
    difficulty: "hard"
},

{
    answers: ["φέρμιο", "fermium"],
    image: "images/elements/fermium.png",
    difficulty: "hard"
},
 
{
    answers: ["μεντελέβιο", "mendelevium"],
    image: "images/elements/mendelevium.png",
    difficulty: "hard"
},

{
    answers: ["νοβέλιο", "nobelium"],
    image: "images/elements/nobelium.png",
    difficulty: "hard"
},

{
    answers: ["λωρένσιο", "lawrencium"],
    image: "images/elements/lawrencium.png",
    difficulty: "hard"
},

{
    answers: ["ράδερφορντιο", "rutherfordium"],
    image: "images/elements/rutherfordium.png",
    difficulty: "hard"
},

{
    answers: ["ντούμπνιο", "dubnium"],
    image: "images/elements/dubnium.png",
    difficulty: "hard"
},

{
    answers: ["σιμπόργκιο", "seaborgium"],
    image: "images/elements/seaborgium.png",
    difficulty: "hard"
},

{
    answers: ["μπόριο", "bohrium"],
    image: "images/elements/bohrium.png",
    difficulty: "hard"
},

{
    answers: ["χάσιο", "hassium"],
    image: "images/elements/hassium.png",
    difficulty: "hard"
},

{
    answers: ["μάιτνεριο", "meitnerium"],
    image: "images/elements/meitnerium.png",
    difficulty: "hard"
},

{
    answers: ["νταρμστάντιο", "darmstadtium"],
    image: "images/elements/darmstadtium.png",
    difficulty: "hard"
},

{
    answers: ["ρέντγκενιο", "roentgenium"],
    image: "images/elements/roentgenium.png",
    difficulty: "hard"
},

{
    answers: ["κοπερνίκιο", "copernicium"],
    image: "images/elements/copernicium.png",
    difficulty: "hard"
},

{
    answers: ["νιχόνιο", "nihonium"],
    image: "images/elements/nihonium.png",
    difficulty: "hard"
},

{
    answers: ["φλερόβιο", "flerovium"],
    image: "images/elements/flerovium.png",
    difficulty: "hard"
},

{
    answers: ["μοσκόβιο", "moscovium"],
    image: "images/elements/moscovium.png",
    difficulty: "hard"
},

{
    answers: ["λιβερμόριο", "livermorium"],
    image: "images/elements/livermorium.png",
    difficulty: "hard"
},

{
    answers: ["τενέσσιο", "tennessine"],
    image: "images/elements/tennessine.png",
    difficulty: "hard"
},

{
    answers: ["ογκανεσόν", "oganesson"],
    image: "images/elements/oganesson.png",
    difficulty: "hard"
}

],



pokemon: [

{
    answers: ["pikachu"],
    image: "images/pokemon/pikachu.png",
    difficulty: "easy"
},

{
    answers: ["raichu"],
    image: "images/pokemon/raichu.png",
    difficulty: "easy"
},

{
    answers: ["charmander"],
    image: "images/pokemon/charmander.png",
    difficulty: "easy"
},

{
    answers: ["charmeleon"],
    image: "images/pokemon/charmeleon.png",
    difficulty: "easy"
},

{
    answers: ["charizard"],
    image: "images/pokemon/charizard.png",
    difficulty: "easy"
},

{
    answers: ["bulbasaur"],
    image: "images/pokemon/bulbasaur.png",
    difficulty: "easy"
},

{
    answers: ["ivysaur"],
    image: "images/pokemon/ivysaur.png",
    difficulty: "easy"
},

{
    answers: ["venusaur"],
    image: "images/pokemon/venusaur.png",
    difficulty: "easy"
},

{
    answers: ["squirtle"],
    image: "images/pokemon/squirtle.png",
    difficulty: "easy"
},

{
    answers: ["wartortle"],
    image: "images/pokemon/wartortle.png",
    difficulty: "easy"
},

{
    answers: ["blastoise"],
    image: "images/pokemon/blastoise.png",
    difficulty: "easy"
},

{
    answers: ["eevee"],
    image: "images/pokemon/eevee.png",
    difficulty: "easy"
},

{
    answers: ["vaporeon"],
    image: "images/pokemon/vaporeon.png",
    difficulty: "easy"
},

{
    answers: ["jolteon"],
    image: "images/pokemon/jolteon.png",
    difficulty: "easy"
},

{
    answers: ["flareon"],
    image: "images/pokemon/flareon.png",
    difficulty: "easy"
},

{
    answers: ["snorlax"],
    image: "images/pokemon/snorlax.png",
    difficulty: "easy"
},

{
    answers: ["gengar"],
    image: "images/pokemon/gengar.png",
    difficulty: "easy"
},

{
    answers: ["gastly"],
    image: "images/pokemon/gastly.png",
    difficulty: "easy"
},

{
    answers: ["haunter"],
    image: "images/pokemon/haunter.png",
    difficulty: "easy"
},

{
    answers: ["jigglypuff"],
    image: "images/pokemon/jigglypuff.png",
    difficulty: "easy"
},

{
    answers: ["meowth"],
    image: "images/pokemon/meowth.png",
    difficulty: "easy"
},

{
    answers: ["psyduck"],
    image: "images/pokemon/psyduck.png",
    difficulty: "easy"
},

{
    answers: ["magikarp"],
    image: "images/pokemon/magikarp.png",
    difficulty: "easy"
},

{
    answers: ["gyarados"],
    image: "images/pokemon/gyarados.png",
    difficulty: "easy"
},

{
    answers: ["dragonite"],
    image: "images/pokemon/dragonite.png",
    difficulty: "easy"
},

{
    answers: ["lapras"],
    image: "images/pokemon/lapras.png",
    difficulty: "easy"
},

{
    answers: ["onix"],
    image: "images/pokemon/onix.png",
    difficulty: "easy"
},

{
    answers: ["geodude"],
    image: "images/pokemon/geodude.png",
    difficulty: "easy"
},

{
    answers: ["machop"],
    image: "images/pokemon/machop.png",
    difficulty: "easy"
},

{
    answers: ["machoke"],
    image: "images/pokemon/machoke.png",
    difficulty: "easy"
},

{
    answers: ["machamp"],
    image: "images/pokemon/machamp.png",
    difficulty: "easy"
},

{
    answers: ["abra"],
    image: "images/pokemon/abra.png",
    difficulty: "easy"
},

{
    answers: ["kadabra"],
    image: "images/pokemon/kadabra.png",
    difficulty: "easy"
},

{
    answers: ["alakazam"],
    image: "images/pokemon/alakazam.png",
    difficulty: "easy"
},

{
    answers: ["cubone"],
    image: "images/pokemon/cubone.png",
    difficulty: "easy"
},

{
    answers: ["ditto"],
    image: "images/pokemon/ditto.png",
    difficulty: "easy"
},

{
    answers: ["lucario"],
    image: "images/pokemon/lucario.png",
    difficulty: "easy"
},

{
    answers: ["riolu"],
    image: "images/pokemon/riolu.png",
    difficulty: "easy"
},

{
    answers: ["mew"],
    image: "images/pokemon/mew.png",
    difficulty: "easy"
},

{
    answers: ["mewtwo"],
    image: "images/pokemon/mewtwo.png",
    difficulty: "easy"
},

{
    answers: ["greninja"],
    image: "images/pokemon/greninja.png",
    difficulty: "medium"
},

{
    answers: ["froakie"],
    image: "images/pokemon/froakie.png",
    difficulty: "medium"
},

{
    answers: ["frogadier"],
    image: "images/pokemon/frogadier.png",
    difficulty: "medium"
},

{
    answers: ["blaziken"],
    image: "images/pokemon/blaziken.png",
    difficulty: "medium"
},

{
    answers: ["torchic"],
    image: "images/pokemon/torchic.png",
    difficulty: "medium"
},

{
    answers: ["combusken"],
    image: "images/pokemon/combusken.png",
    difficulty: "medium"
},

{
    answers: ["sceptile"],
    image: "images/pokemon/sceptile.png",
    difficulty: "medium"
},

{
    answers: ["treecko"],
    image: "images/pokemon/treecko.png",
    difficulty: "medium"
},

{
    answers: ["grovyle"],
    image: "images/pokemon/grovyle.png",
    difficulty: "medium"
},

{
    answers: ["swampert"],
    image: "images/pokemon/swampert.png",
    difficulty: "medium"
},

{
    answers: ["mudkip"],
    image: "images/pokemon/mudkip.png",
    difficulty: "medium"
},

{
    answers: ["marshtomp"],
    image: "images/pokemon/marshtomp.png",
    difficulty: "medium"
},

{
    answers: ["gardevoir"],
    image: "images/pokemon/gardevoir.png",
    difficulty: "medium"
},

{
    answers: ["gallade"],
    image: "images/pokemon/gallade.png",
    difficulty: "medium"
},

{
    answers: ["ralts"],
    image: "images/pokemon/ralts.png",
    difficulty: "medium"
},

{
    answers: ["kirlia"],
    image: "images/pokemon/kirlia.png",
    difficulty: "medium"
},

{
    answers: ["tyranitar"],
    image: "images/pokemon/tyranitar.png",
    difficulty: "medium"
},

{
    answers: ["larvitar"],
    image: "images/pokemon/larvitar.png",
    difficulty: "medium"
},

{
    answers: ["salamence"],
    image: "images/pokemon/salamence.png",
    difficulty: "medium"
},

{
    answers: ["metagross"],
    image: "images/pokemon/metagross.png",
    difficulty: "medium"
},

{
    answers: ["rayquaza"],
    image: "images/pokemon/rayquaza.png",
    difficulty: "medium"
},

{
    answers: ["kyogre"],
    image: "images/pokemon/kyogre.png",
    difficulty: "medium"
},

{
    answers: ["groudon"],
    image: "images/pokemon/groudon.png",
    difficulty: "medium"
},

{
    answers: ["lugia"],
    image: "images/pokemon/lugia.png",
    difficulty: "medium"
},

{
    answers: ["ho-oh", "ho oh", "hooh"],
    image: "images/pokemon/ho-oh.png",
    difficulty: "medium"
},

{
    answers: ["darkrai"],
    image: "images/pokemon/darkrai.png",
    difficulty: "medium"
},

{
    answers: ["dialga"],
    image: "images/pokemon/dialga.png",
    difficulty: "medium"
},

{
    answers: ["palkia"],
    image: "images/pokemon/palkia.png",
    difficulty: "medium"
},

{
    answers: ["giratina"],
    image: "images/pokemon/giratina.png",
    difficulty: "medium"
},

{
    answers: ["zekrom"],
    image: "images/pokemon/zekrom.png",
    difficulty: "medium"
},

{
    answers: ["reshiram"],
    image: "images/pokemon/reshiram.png",
    difficulty: "medium"
},

{
    answers: ["kyurem"],
    image: "images/pokemon/kyurem.png",
    difficulty: "medium"
},

{
    answers: ["xerneas"],
    image: "images/pokemon/xerneas.png",
    difficulty: "medium"
},

{
    answers: ["yveltal"],
    image: "images/pokemon/yveltal.png",
    difficulty: "medium"
},

{
    answers: ["zoroark"],
    image: "images/pokemon/zoroark.png",
    difficulty: "medium"
},

{
    answers: ["arceus"],
    image: "images/pokemon/arceus.png",
    difficulty: "hard"
},

{
    answers: ["deoxys"],
    image: "images/pokemon/deoxys.png",
    difficulty: "hard"
},

{
    answers: ["genesect"],
    image: "images/pokemon/genesect.png",
    difficulty: "hard"
},

{
    answers: ["regigigas", "re gigigas", "re giggigas", "rejijigas"],
    image: "images/pokemon/regigigas.png",
    difficulty: "hard"
},

{
    answers: ["eternatus"],
    image: "images/pokemon/eternatus.png",
    difficulty: "hard"
},

{
    answers: ["necrozma"],
    image: "images/pokemon/necrozma.png",
    difficulty: "hard"
},

{
    answers: ["calyrex"],
    image: "images/pokemon/calyrex.png",
    difficulty: "hard"
},

{
    answers: ["keldeo"],
    image: "images/pokemon/keldeo.png",
    difficulty: "hard"
},

{
    answers: ["cobalion"],
    image: "images/pokemon/cobalion.png",
    difficulty: "hard"
},

{
    answers: ["terrakion"],
    image: "images/pokemon/terrakion.png",
    difficulty: "hard"
},

{
    answers: ["virizion"],
    image: "images/pokemon/virizion.png",
    difficulty: "hard"
},

{
    answers: ["entei"],
    image: "images/pokemon/entei.png",
    difficulty: "hard"
},

{
    answers: ["raikou"],
    image: "images/pokemon/raikou.png",
    difficulty: "hard"
},

{
    answers: ["suicune"],
    image: "images/pokemon/suicune.png",
    difficulty: "hard"
},

{
    answers: ["latias"],
    image: "images/pokemon/latias.png",
    difficulty: "hard"
},

{
    answers: ["latios"],
    image: "images/pokemon/latios.png",
    difficulty: "hard"
},

{
    answers: ["hoopa"],
    image: "images/pokemon/hoopa.png",
    difficulty: "hard"
},

{
    answers: ["zygarde"],
    image: "images/pokemon/zygarde.png",
    difficulty: "hard"
},

{
    answers: ["solgaleo"],
    image: "images/pokemon/solgaleo.png",
    difficulty: "hard"
},

{
    answers: ["lunala"],
    image: "images/pokemon/lunala.png",
    difficulty: "hard"
},

{
    answers: ["zacian"],
    image: "images/pokemon/zacian.png",
    difficulty: "hard"
},

{
    answers: ["zamazenta"],
    image: "images/pokemon/zamazenta.png",
    difficulty: "hard"
},

{
    answers: ["koraidon"],
    image: "images/pokemon/koraidon.png",
    difficulty: "hard"
},

{
    answers: ["miraidon"],
    image: "images/pokemon/miraidon.png",
    difficulty: "hard"
},

{
    answers: ["nihilego"],
    image: "images/pokemon/nihilego.png",
    difficulty: "hard"
},

{
    answers: ["buzzwole"],
    image: "images/pokemon/buzzwole.png",
    difficulty: "hard"
},

{
    answers: ["pheromosa"],
    image: "images/pokemon/pheromosa.png",
    difficulty: "hard"
},

{
    answers: ["xurkitree"],
    image: "images/pokemon/xurkitree.png",
    difficulty: "hard"
},

{
    answers: ["celesteela"],
    image: "images/pokemon/celesteela.png",
    difficulty: "hard"
},

{
    answers: ["kartana"],
    image: "images/pokemon/kartana.png",
    difficulty: "hard"
},

{
    answers: ["guzzlord"],
    image: "images/pokemon/guzzlord.png",
    difficulty: "hard"
},

{
    answers: ["poipole"],
    image: "images/pokemon/poipole.png",
    difficulty: "hard"
},

{
    answers: ["naganadel"],
    image: "images/pokemon/naganadel.png",
    difficulty: "hard"
},

{
    answers: ["stakataka"],
    image: "images/pokemon/stakataka.png",
    difficulty: "hard"
},

{
    answers: ["blacephalon"],
    image: "images/pokemon/blacephalon.png",
    difficulty: "hard"
},

{
    answers: ["tapu koko"],
    image: "images/pokemon/tapu-koko.png",
    difficulty: "hard"
}

],

mythology: [

{
    answers: ["δίας", "ζευς", "zeus", "dias", "zevs"],
    image: "images/mythology/zeus.jpg",
    difficulty: "easy"
},

{
    answers: ["ποσειδώνας", "poseidon", "poseidonas"],
    image: "images/mythology/poseidon.jpg",
    difficulty: "easy"
},

{
    answers: ["άδης", "hades"],
    image: "images/mythology/hades.jpg",
    difficulty: "easy"
},

{
    answers: ["αθηνά", "athena", "athina"],
    image: "images/mythology/athena.jpg",
    difficulty: "easy"
},

{
    answers: ["άρης", "ares"],
    image: "images/mythology/ares.jpg",
    difficulty: "easy"
},

{
    answers: ["απόλλωνας", "apollo"],
    image: "images/mythology/apollo.jpg",
    difficulty: "easy"
},

{
    answers: ["άρτεμις", "artemis"],
    image: "images/mythology/artemis.jpg",
    difficulty: "easy"
},

{
    answers: ["ήρα", "hera"],
    image: "images/mythology/hera.jpg",
    difficulty: "easy"
},

{
    answers: ["ερμής", "hermes"],
    image: "images/mythology/hermes.jpg",
    difficulty: "easy"
},

{
    answers: ["αφροδίτη", "aphrodite"],
    image: "images/mythology/aphrodite.jpg",
    difficulty: "easy"
},

{
    answers: ["ήφαιστος", "hephaestus"],
    image: "images/mythology/hephaestus.jpg",
    difficulty: "easy"
},

{
    answers: ["δήμητρα", "demeter"],
    image: "images/mythology/demeter.jpg",
    difficulty: "easy"
},

{
    answers: ["περσεφόνη", "persephone"],
    image: "images/mythology/persephone.jpg",
    difficulty: "easy"
},

{
    answers: ["έρως", "eros", "cupid"],
    image: "images/mythology/eros.jpg",
    difficulty: "easy"
},

{
    answers: ["μέδουσα", "medusa"],
    image: "images/mythology/medusa.jpg",
    difficulty: "easy"
},

{
    answers: ["πήγασος", "pegasus"],
    image: "images/mythology/pegasus.jpg",
    difficulty: "easy"
},

{
    answers: ["κέρβερος", "cerberus"],
    image: "images/mythology/cerberus.jpg",
    difficulty: "easy"
},

{
    answers: ["μινώταυρος", "minotaur"],
    image: "images/mythology/minotaur.jpg",
    difficulty: "easy"
},

{
    answers: ["κύκλωπας", "cyclops"],
    image: "images/mythology/cyclops.jpg",
    difficulty: "easy"
},

{
    answers: ["κένταυρος", "centaur"],
    image: "images/mythology/centaur.jpg",
    difficulty: "easy"
},

{
    answers: ["σφίγγα", "sphinx"],
    image: "images/mythology/sphinx.jpg",
    difficulty: "easy"
},

{
    answers: ["ύδρα", "ύδρα της λέρνας", "lernaean hydra", "hydra"],
    image: "images/mythology/hydra.jpg",
    difficulty: "easy"
},

{
    answers: ["χίμαιρα", "chimera"],
    image: "images/mythology/chimera.jpg",
    difficulty: "easy"
},

{
    answers: ["άτλας", "atlas"],
    image: "images/mythology/atlas.jpg",
    difficulty: "easy"
},

{
    answers: ["κρόνος", "cronus", "kronos"],
    image: "images/mythology/cronus.jpg",
    difficulty: "easy"
},

{
    answers: ["ρέα", "rhea"],
    image: "images/mythology/rhea.jpg",
    difficulty: "easy"
},

{
    answers: ["ουρανός", "uranus"],
    image: "images/mythology/uranus.jpg",
    difficulty: "easy"
},

{
    answers: ["γαία", "gaia", "gaea"],
    image: "images/mythology/gaia.jpg",
    difficulty: "easy"
},

{
    answers: ["θωρ", "thor"],
    image: "images/mythology/thor.jpg",
    difficulty: "easy"
},

{
    answers: ["λόκι", "loki"],
    image: "images/mythology/loki.jpg",
    difficulty: "easy"
},


{
    answers: ["όντιν", "odin"],
    image: "images/mythology/odin.jpg",
    difficulty: "medium"
},

{
    answers: ["φρέγια", "freya", "freyja"],
    image: "images/mythology/freya.jpg",
    difficulty: "medium"
},

{
    answers: ["τυρ", "tyr", "tir"],
    image: "images/mythology/tyr.jpg",
    difficulty: "medium"
},

{
    answers: ["χέιμνταλ", "heimdall"],
    image: "images/mythology/heimdall.jpg",
    difficulty: "medium"
},

{
    answers: ["φενρίρ", "fenrir"],
    image: "images/mythology/fenrir.jpg",
    difficulty: "medium"
},

{
    answers: ["γιορμουνγκάντρ", "jormungandr", "jörmungandr"],
    image: "images/mythology/jormungandr.jpg",
    difficulty: "medium"
},

{
    answers: ["σλεϊπνιρ", "sleipnir"],
    image: "images/mythology/sleipnir.jpg",
    difficulty: "medium"
},

{
    answers: ["ανούβις", "anubis"],
    image: "images/mythology/anubis.jpg",
    difficulty: "medium"
},

{
    answers: ["ρα", "ra"],
    image: "images/mythology/ra.jpg",
    difficulty: "medium"
},

{
    answers: ["ώρος", "horus"],
    image: "images/mythology/horus.jpg",
    difficulty: "medium"
},

{
    answers: ["όσιρις", "osiris"],
    image: "images/mythology/osiris.jpg",
    difficulty: "medium"
},

{
    answers: ["ίσιδα", "isis"],
    image: "images/mythology/isis.jpg",
    difficulty: "medium"
},

{
    answers: ["σεθ", "set", "seth"],
    image: "images/mythology/set.jpg",
    difficulty: "medium"
},

{
    answers: ["θωθ", "thoth"],
    image: "images/mythology/thoth.jpg",
    difficulty: "medium"
},

{
    answers: ["βάστιτ", "bastet"],
    image: "images/mythology/bastet.jpg",
    difficulty: "medium"
},

{
    answers: ["σομπέκ", "sobek"],
    image: "images/mythology/sobek.jpg",
    difficulty: "medium"
},

{
    answers: ["άμμων", "amon", "amun"],
    image: "images/mythology/amun.jpg",
    difficulty: "medium"
},

{
    answers: ["νίκη", "nike"],
    image: "images/mythology/nike.jpg",
    difficulty: "medium"
},

{
    answers: ["νέμεσις", "nemesis"],
    image: "images/mythology/nemesis.jpg",
    difficulty: "medium"
},

{
    answers: ["χάροντας", "charon"],
    image: "images/mythology/charon.jpg",
    difficulty: "medium"
},

{
    answers: ["μοίρες", "the fates", "fates", "moirai"],
    image: "images/mythology/fates.jpg",
    difficulty: "medium"
},

{
    answers: ["νύξ", "nyx"],
    image: "images/mythology/nyx.jpg",
    difficulty: "medium"
},

{
    answers: ["έριδα", "eris"],
    image: "images/mythology/eris.jpg",
    difficulty: "medium"
},

{
    answers: ["πάνας", "pan"],
    image: "images/mythology/pan.jpg",
    difficulty: "medium"
},

{
    answers: ["μορφέας", "morpheus"],
    image: "images/mythology/morpheus.jpg",
    difficulty: "medium"
},

{
    answers: ["τύχη", "tyche"],
    image: "images/mythology/tyche.jpg",
    difficulty: "medium"
},

{
    answers: ["ηρακλής", "heracles", "hercules"],
    image: "images/mythology/heracles.jpg",
    difficulty: "medium"
},

{
    answers: ["αχιλλέας", "achilles"],
    image: "images/mythology/achilles.jpg",
    difficulty: "medium"
},

{
    answers: ["οδυσσέας", "odysseus", "ulysses"],
    image: "images/mythology/odysseus.jpg",
    difficulty: "medium"
},

{
    answers: ["περσέας", "perseus"],
    image: "images/mythology/perseus.jpg",
    difficulty: "medium"
},


{
    answers: ["κίρκη", "circe"],
    image: "images/mythology/circe.jpg",
    difficulty: "medium"
},

{
    answers: ["αιόλος", "aeolus"],
    image: "images/mythology/aeolus.jpg",
    difficulty: "medium"
},

{
    answers: ["προμηθέας", "prometheus"],
    image: "images/mythology/prometheus.jpg",
    difficulty: "medium"
},

{
    answers: ["επιμηθέας", "epimetheus"],
    image: "images/mythology/epimetheus.jpg",
    difficulty: "hard"
},

{
    answers: ["διονύσος", "dionysus"],
    image: "images/mythology/dionysus.jpg",
    difficulty: "easy"
},

{
    answers: ["εστία", "hestia"],
    image: "images/mythology/hestia.jpg",
    difficulty: "easy"
},

{
    answers: ["σελήνη", "selene"],
    image: "images/mythology/selene.jpg",
    difficulty: "medium"
},

{
    answers: ["ηώς", "eos"],
    image: "images/mythology/eos.jpg",
    difficulty: "medium"
},

{
    answers: ["χάος", "chaos"],
    image: "images/mythology/chaos.jpg",
    difficulty: "hard"
},

{
    answers: ["έκτωρας", "hector"],
    image: "images/mythology/hector.jpg",
    difficulty: "medium"
},

{
    answers: ["πάτροκλος", "patroclus"],
    image: "images/mythology/patroclus.jpg",
    difficulty: "medium"
},

{
    answers: ["ιάσονας", "jason"],
    image: "images/mythology/jason.jpg",
    difficulty: "medium"
},

{
    answers: ["ορφέας", "orpheus"],
    image: "images/mythology/orpheus.jpg",
    difficulty: "medium"
},

{
    answers: ["δάδαλος", "daedalus"],
    image: "images/mythology/daedalus.jpg",
    difficulty: "medium"
},

{
    answers: ["ίκαρος", "icarus"],
    image: "images/mythology/icarus.jpg",
    difficulty: "easy"
},

{
    answers: ["θησέας", "theseus"],
    image: "images/mythology/theseus.jpg",
    difficulty: "easy"
},

{
    answers: ["ανδρομέδα", "andromeda"],
    image: "images/mythology/andromeda.jpg",
    difficulty: "medium"
},

{
    answers: ["κάστωρ", "castor"],
    image: "images/mythology/castor.jpg",
    difficulty: "hard"
},

{
    answers: ["πολυδεύκης", "polydeuces", "pollux"],
    image: "images/mythology/polydeuces.jpg",
    difficulty: "hard"
},

{
    answers: ["πανδώρα", "pandora"],
    image: "images/mythology/pandora.jpg",
    difficulty: "easy"
},


{
    answers: ["γοργόνες", "gorgons"],
    image: "images/mythology/gorgons.jpg",
    difficulty: "medium"
},


{
    answers: ["σειρήνες", "sirens"],
    image: "images/mythology/sirens.jpg",
    difficulty: "medium"
},

{
    answers: ["άρπυιες", "harpies"],
    image: "images/mythology/harpies.jpg",
    difficulty: "medium"
},

{
    answers: ["σάτυρος", "satyr"],
    image: "images/mythology/satyr.jpg",
    difficulty: "medium"
},

{
    answers: ["νύμφη", "nymph"],
    image: "images/mythology/nymph.jpg",
    difficulty: "medium"
},

{
    answers: ["τυφώνας", "typhon"],
    image: "images/mythology/typhon.jpg",
    difficulty: "hard"
},

{
    answers: ["εχίδνα", "echidna"],
    image: "images/mythology/echidna.jpg",
    difficulty: "hard"
},

{
    answers: ["σκύλλα", "scylla"],
    image: "images/mythology/scylla.jpg",
    difficulty: "medium"
},

{
    answers: ["χάρυβδη", "charybdis"],
    image: "images/mythology/charybdis.jpg",
    difficulty: "medium"
},

{
    answers: ["όρθρος", "orthrus", "orthos"],
    image: "images/mythology/orthrus.jpg",
    difficulty: "hard"
},

{
    answers: ["λάδων", "ladon"],
    image: "images/mythology/ladon.jpg",
    difficulty: "hard"
},

{
    answers: ["λερναίος καρκίνος", "carcinus"],
    image: "images/mythology/carcinus.jpg",
    difficulty: "hard"
},

{
    answers: ["τελχίνες", "telchines"],
    image: "images/mythology/telchines.jpg",
    difficulty: "hard"
},

{
    answers: ["ερινύες", "erinyes", "furies"],
    image: "images/mythology/erinyes.jpg",
    difficulty: "hard"
},

{
    answers: ["γίγαντες", "gigantes", "giants","γίγαντας","giant",],
    image: "images/mythology/gigantes.jpg",
    difficulty: "medium"
},

{
    answers: ["τιτάνες", "titans"],
    image: "images/mythology/titans.jpg",
    difficulty: "medium"
},

{
    answers: ["δράκος", "dragon"],
    image: "images/mythology/dragon.jpg",
    difficulty: "medium"
},

{
    answers: ["φοίνικας", "phoenix"],
    image: "images/mythology/phoenix.jpg",
    difficulty: "medium"
},

{
    answers: ["κράκεν", "kraken"],
    image: "images/mythology/kraken.jpg",
    difficulty: "medium"
},

{
    answers: ["γρύπας", "griffin", "gryphon"],
    image: "images/mythology/griffin.jpg",
    difficulty: "medium"
}

],


    capitals: [

{
    answers: ["αθήνα", "athens"],
    image: "images/countries/greece.png",
    difficulty: "easy"
},

{
    answers: ["παρίσι", "paris"],
    image: "images/countries/france.png",
    difficulty: "easy"
},

{
    answers: ["βερολίνο", "berlin"],
    image: "images/countries/germany.png",
    difficulty: "easy"
},

{
    answers: ["ρώμη", "rome", "roma"],
    image: "images/countries/italy.png",
    difficulty: "easy"
},

{
    answers: ["μαδρίτη", "madrid"],
    image: "images/countries/spain.png",
    difficulty: "easy"
},

{
    answers: ["λισαβόνα", "lisbon", "lisboa"],
    image: "images/countries/portugal.png",
    difficulty: "easy"
},

{
    answers: ["λονδίνο", "london"],
    image: "images/countries/united-kingdom.png",
    difficulty: "easy"
},

{
    answers: ["δουβλίνο", "dublin"],
    image: "images/countries/ireland.png",
    difficulty: "easy"
},

{
    answers: ["βρυξέλλες", "brussels"],
    image: "images/countries/belgium.png",
    difficulty: "easy"
},

{
    answers: ["άμστερνταμ", "amsterdam"],
    image: "images/countries/netherlands.png",
    difficulty: "easy"
},

{
    answers: ["βέρνη", "bern"],
    image: "images/countries/switzerland.png",
    difficulty: "easy"
},

{
    answers: ["βιέννη", "vienna"],
    image: "images/countries/austria.png",
    difficulty: "easy"
},

{
    answers: ["στοκχόλμη", "stockholm"],
    image: "images/countries/sweden.png",
    difficulty: "easy"
},

{
    answers: ["όσλο", "oslo"],
    image: "images/countries/norway.png",
    difficulty: "easy"
},

{
    answers: ["κοπεγχάγη", "copenhagen"],
    image: "images/countries/denmark.png",
    difficulty: "easy"
},

{
    answers: ["ελσίνκι", "helsinki"],
    image: "images/countries/finland.png",
    difficulty: "easy"
},

{
    answers: ["βαρσοβία", "warsaw"],
    image: "images/countries/poland.png",
    difficulty: "easy"
},

{
    answers: ["πράγα", "prague", "praha"],
    image: "images/countries/czechia.png",
    difficulty: "easy"
},

{
    answers: ["βουδαπέστη", "budapest"],
    image: "images/countries/hungary.png",
    difficulty: "easy"
},

{
    answers: ["βουκουρέστι", "bucharest"],
    image: "images/countries/romania.png",
    difficulty: "easy"
},

{
    answers: ["σοφία", "sofia"],
    image: "images/countries/bulgaria.png",
    difficulty: "easy"
},

{
    answers: ["άγκυρα", "ankara"],
    image: "images/countries/turkiye.png",
    difficulty: "easy"
},

{
    answers: ["μόσχα", "moscow"],
    image: "images/countries/russia.png",
    difficulty: "easy"
},

{
    answers: ["κίεβο", "κιεβο", "kyiv", "kiev"],
    image: "images/countries/ukraine.png",
    difficulty: "easy"
},

{
    answers: ["οτάβα", "ottawa"],
    image: "images/countries/canada.png",
    difficulty: "easy"
},

{
    answers: ["ουάσινγκτον", "washington", "washington dc"],
    image: "images/countries/united-states.png",
    difficulty: "easy"
},

{
    answers: ["πόλη του μεξικού", "μεξικό σίτι", "mexico city"],
    image: "images/countries/mexico.png",
    difficulty: "easy"
},

{
    answers: ["μπραζίλια", "brasilia"],
    image: "images/countries/brazil.png",
    difficulty: "easy"
},

{
    answers: ["μπουένος άιρες", "buenos aires"],
    image: "images/countries/argentina.png",
    difficulty: "easy"
},

{
    answers: ["σαντιάγο", "santiago"],
    image: "images/countries/chile.png",
    difficulty: "easy"
},

{
    answers: ["λίμα", "lima"],
    image: "images/countries/peru.png",
    difficulty: "easy"
},

{
    answers: ["μπογκοτά", "bogota", "bogotá"],
    image: "images/countries/colombia.png",
    difficulty: "easy"
},

{
    answers: ["τόκιο", "tokyo"],
    image: "images/countries/japan.png",
    difficulty: "easy"
},

{
    answers: ["πεκίνο", "beijing"],
    image: "images/countries/china.png",
    difficulty: "easy"
},

{
    answers: ["σεούλ", "seoul"],
    image: "images/countries/south-korea.png",
    difficulty: "easy"
},

{
    answers: ["πιονγκγιάνγκ", "πιονγκ γιανγκ", "pyongyang"],
    image: "images/countries/north-korea.png",
    difficulty: "medium"
},

{
    answers: ["νέο δελχί", "new delhi"],
    image: "images/countries/india.png",
    difficulty: "easy"
},

{
    answers: ["ισλαμαμπάντ", "islamabad"],
    image: "images/countries/pakistan.png",
    difficulty: "medium"
},

{
    answers: ["μπανγκόκ", "bangkok"],
    image: "images/countries/thailand.png",
    difficulty: "medium"
},

{
    answers: ["ανόι", "hanoi"],
    image: "images/countries/vietnam.png",
    difficulty: "medium"
},

{
    answers: ["τζακάρτα", "jakarta"],
    image: "images/countries/indonesia.png",
    difficulty: "medium"
},

{
    answers: ["μανίλα", "manila"],
    image: "images/countries/philippines.png",
    difficulty: "medium"
},

{
    answers: ["καμπέρα", "canberra"],
    image: "images/countries/australia.png",
    difficulty: "easy"
},

{
    answers: ["ουέλινγκτον", "wellington"],
    image: "images/countries/new-zealand.png",
    difficulty: "medium"
},

{
    answers: ["κάιρο", "cairo"],
    image: "images/countries/egypt.png",
    difficulty: "easy"
},

{
    answers: ["ραμπάτ", "rabat"],
    image: "images/countries/morocco.png",
    difficulty: "medium"
},

{
    answers: ["πρετόρια", "pretoria"],
    image: "images/countries/south-africa.png",
    difficulty: "hard"
},

{
    answers: ["ριάντ", "riyadh"],
    image: "images/countries/saudi-arabia.png",
    difficulty: "medium"
},

{
    answers: ["αμπού ντάμπι", "abu dhabi"],
    image: "images/countries/united-arab-emirates.png",
    difficulty: "medium"
},

{
    answers: ["ιερουσαλήμ", "jerusalem"],
    image: "images/countries/israel.png",
    difficulty: "medium"
},

{
    answers: ["σιγκαπούρη", "singapore"],
    image: "images/countries/singapore.png",
    difficulty: "medium"
},

{
    answers: ["κουάλα λουμπούρ", "kuala lumpur"],
    image: "images/countries/malaysia.png",
    difficulty: "medium"
},

{
    answers: ["ντόχα", "doha"],
    image: "images/countries/qatar.png",
    difficulty: "medium"
},

{
    answers: ["ζάγκρεμπ", "zagreb"],
    image: "images/countries/croatia.png",
    difficulty: "easy"
},

{
    answers: ["βελιγράδι", "belgrade"],
    image: "images/countries/serbia.png",
    difficulty: "medium"
},

{
    answers: ["λιουμπλιάνα", "ljubljana"],
    image: "images/countries/slovenia.png",
    difficulty: "medium"
},

{
    answers: ["μπρατισλάβα", "bratislava"],
    image: "images/countries/slovakia.png",
    difficulty: "medium"
},

{
    answers: ["σαράγεβο", "sarajevo"],
    image: "images/countries/bosnia-and-herzegovina.png",
    difficulty: "medium"
},

{
    answers: ["ποντγκόριτσα", "podgorica"],
    image: "images/countries/montenegro.png",
    difficulty: "medium"
},

{
    answers: ["σκόπια", "skopje"],
    image: "images/countries/north-macedonia.png",
    difficulty: "medium"
},

{
    answers: ["τίρανα", "tirana"],
    image: "images/countries/albania.png",
    difficulty: "easy"
},

{
    answers: ["λευκωσία", "nicosia"],
    image: "images/countries/cyprus.png",
    difficulty: "easy"
},

{
    answers: ["βίλνιους", "vilnius"],
    image: "images/countries/lithuania.png",
    difficulty: "medium"
},

{
    answers: ["ρίγα", "riga"],
    image: "images/countries/latvia.png",
    difficulty: "medium"
},

{
    answers: ["τάλιν", "tallinn"],
    image: "images/countries/estonia.png",
    difficulty: "medium"
},

{
    answers: ["μινσκ", "minsk"],
    image: "images/countries/belarus.png",
    difficulty: "medium"
},

{
    answers: ["κισινάου", "chisinau", "chișinău"],
    image: "images/countries/moldova.png",
    difficulty: "medium"
},

{
    answers: ["τιφλίδα", "tbilisi"],
    image: "images/countries/georgia.png",
    difficulty: "medium"
},

{
    answers: ["γιερεβάν", "yerevan"],
    image: "images/countries/armenia.png",
    difficulty: "medium"
},

{
    answers: ["μπακού", "baku"],
    image: "images/countries/azerbaijan.png",
    difficulty: "medium"
},

{
    answers: ["αστάνα", "astana"],
    image: "images/countries/kazakhstan.png",
    difficulty: "medium"
},

{
    answers: ["τάσкент", "tashkent"],
    image: "images/countries/uzbekistan.png",
    difficulty: "hard"
},

{
    answers: ["ασγκαμπάτ", "ashgabat"],
    image: "images/countries/turkmenistan.png",
    difficulty: "hard"
},

{
    answers: ["μπισκέκ", "bishkek"],
    image: "images/countries/kyrgyzstan.png",
    difficulty: "hard"
},

{
    answers: ["ντουσάνμπε", "dushanbe"],
    image: "images/countries/tajikistan.png",
    difficulty: "hard"
},

{
    answers: ["κατμαντού", "kathmandu"],
    image: "images/countries/nepal.png",
    difficulty: "medium"
},

{
    answers: ["ντάκα", "dhaka"],
    image: "images/countries/bangladesh.png",
    difficulty: "medium"
},

{
    answers: ["σρι τζαγιαγουαρντενεπούρα κότε", "sri jayawardenepura kotte"],
    image: "images/countries/sri-lanka.png",
    difficulty: "hard"
},

{
    answers: ["ουλάν μπατόρ", "ulaanbaatar", "ulan bator"],
    image: "images/countries/mongolia.png",
    difficulty: "medium"
},

{
    answers: ["ταϊπέι", "taipei"],
    image: "images/countries/taiwan.png",
    difficulty: "medium"
},

{
    answers: ["πνομ πενχ", "phnom penh"],
    image: "images/countries/cambodia.png",
    difficulty: "medium"
},

{
    answers: ["βιεντιάν", "vientiane"],
    image: "images/countries/laos.png",
    difficulty: "hard"
},

{
    answers: ["νεπιντό", "naypyidaw"],
    image: "images/countries/myanmar.png",
    difficulty: "hard"
},

{
    answers: ["μπαντάρ σερί μπεγκαβάν", "bandar seri begawan"],
    image: "images/countries/brunei.png",
    difficulty: "hard"
},

{
    answers: ["πορτ μόρεσμπι", "port moresby"],
    image: "images/countries/papua-new-guinea.png",
    difficulty: "hard"
},


{
    answers: ["ρέικιαβικ", "reykjavik"],
    image: "images/countries/iceland.png",
    difficulty: "medium"
},

{
    answers: ["λουξεμβούργο", "luxembourg"],
    image: "images/countries/luxembourg.png",
    difficulty: "medium"
},

{
    answers: ["βαντούζ", "vaduz"],
    image: "images/countries/liechtenstein.png",
    difficulty: "hard"
},

{
    answers: ["ανδόρα λα βέγια", "andorra la vella"],
    image: "images/countries/andorra.png",
    difficulty: "hard"
},

{
    answers: ["μονακό", "monaco"],
    image: "images/countries/monaco.png",
    difficulty: "medium"
},

{
    answers: ["άγιος μαρίνος", "san marino"],
    image: "images/countries/san-marino.png",
    difficulty: "hard"
},

{
    answers: ["πόλη του βατικανού", "vatican city"],
    image: "images/countries/vatican-city.png",
    difficulty: "medium"
},

{
    answers: ["βαλέτα", "valletta"],
    image: "images/countries/malta.png",
    difficulty: "medium"
},

{
    answers: ["τυνήσια", "tunis"],
    image: "images/countries/tunisia.png",
    difficulty: "medium"
},

{
    answers: ["αλγέρι", "algiers"],
    image: "images/countries/algeria.png",
    difficulty: "medium"
},

{
    answers: ["τρίπολη", "tripoli"],
    image: "images/countries/libya.png",
    difficulty: "medium"
},

{
    answers: ["αμπούτζα", "abuja"],
    image: "images/countries/nigeria.png",
    difficulty: "medium"
},

{
    answers: ["ναϊρόμπι", "nairobi"],
    image: "images/countries/kenya.png",
    difficulty: "medium"
},

{
    answers: ["αντίς αμπέμπα", "addis ababa"],
    image: "images/countries/ethiopia.png",
    difficulty: "medium"
},

{
    answers: ["ντοντόμα", "dodoma"],
    image: "images/countries/tanzania.png",
    difficulty: "hard"
},

{
    answers: ["χαράρε", "harare"],
    image: "images/countries/zimbabwe.png",
    difficulty: "hard"
},

{
    answers: ["λουσάκα", "lusaka"],
    image: "images/countries/zambia.png",
    difficulty: "hard"
},

{
    answers: ["βίντχουκ", "windhoek"],
    image: "images/countries/namibia.png",
    difficulty: "hard"
},

{
    answers: ["γκαμπορόνε", "gaborone"],
    image: "images/countries/botswana.png",
    difficulty: "hard"
},

{
    answers: ["καράκας", "caracas"],
    image: "images/countries/venezuela.png",
    difficulty: "medium"
},

{
    answers: ["κίτο", "quito"],
    image: "images/countries/ecuador.png",
    difficulty: "medium"
},

{
    answers: ["μοντεβιδέο", "montevideo"],
    image: "images/countries/uruguay.png",
    difficulty: "medium"
},

{
    answers: ["ασουνσιόν", "asuncion", "asunción"],
    image: "images/countries/paraguay.png",
    difficulty: "medium"
},

{
    answers: ["σούκρε", "sucre"],
    image: "images/countries/bolivia.png",
    difficulty: "medium"
},

{
    answers: ["αβάνα", "havana"],
    image: "images/countries/cuba.png",
    difficulty: "medium"
},

{
    answers: ["κίνγκστον", "kingston"],
    image: "images/countries/jamaica.png",
    difficulty: "medium"
},

{
    answers: ["σάντο ντομίνγκο", "santo domingo"],
    image: "images/countries/dominican-republic.png",
    difficulty: "medium"
},

{
    answers: ["σαν χοσέ", "san jose", "san josé"],
    image: "images/countries/costa-rica.png",
    difficulty: "medium"
},

{
    answers: ["πόλη του παναμά", "παναμά σίτι", "panama city"],
    image: "images/countries/panama.png",
    difficulty: "medium"
},

{
    answers: ["τίμφου", "thimphu"],
    image: "images/countries/bhutan.png",
    difficulty: "hard"
},

{
    answers: ["ανταναναρίβο", "antananarivo"],
    image: "images/countries/madagascar.png",
    difficulty: "hard"
},

{
    answers: ["πορτ λούις", "port louis"],
    image: "images/countries/mauritius.png",
    difficulty: "hard"
},

{
    answers: ["βικτώρια", "victoria"],
    image: "images/countries/seychelles.png",
    difficulty: "hard"
},

{
    answers: ["μαλέ", "male", "malé"],
    image: "images/countries/maldives.png",
    difficulty: "hard"
},

{
    answers: ["ντίλι", "dili"],
    image: "images/countries/timor-leste.png",
    difficulty: "hard"
},

{
    answers: ["νγκερουλμούντ", "ngerulmud"],
    image: "images/countries/palau.png",
    difficulty: "hard"
},

{
    answers: ["σούβα", "suva"],
    image: "images/countries/fiji.png",
    difficulty: "hard"
},

{
    answers: ["νουκουαλόφα", "nukuʻalofa", "nukualofa"],
    image: "images/countries/tonga.png",
    difficulty: "hard"
},

{
    answers: ["απία", "apia"],
    image: "images/countries/samoa.png",
    difficulty: "hard"
},

{
    answers: ["πορτ βίλα", "port vila"],
    image: "images/countries/vanuatu.png",
    difficulty: "hard"
},

{
    answers: ["γιάρεν", "yaren"],
    image: "images/countries/nauru.png",
    difficulty: "hard"
},

{
    answers: ["φουναφούτι", "funafuti"],
    image: "images/countries/tuvalu.png",
    difficulty: "hard"
},

{
    answers: ["ταραουά", "tarawa"],
    image: "images/countries/kiribati.png",
    difficulty: "hard"
},

{
    answers: ["ματζούρο", "majuro"],
    image: "images/countries/marshall-islands.png",
    difficulty: "hard"
}

],

      
 animals: [

        {
            answers: ["λιοντάρι"],
            image: "images/animals/lion.jpg",
            difficulty: "easy"
        },

        {
            answers: ["τίγρης"],
            image: "images/animals/tiger.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ελέφαντας"],
            image: "images/animals/elephant.jpg",
            difficulty: "easy"
        },

        {
            answers: ["καμηλοπάρδαλη"],
            image: "images/animals/giraffe.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ζέβρα"],
            image: "images/animals/zebra.jpg",
            difficulty: "easy"
        },

        {
            answers: ["πάντα"],
            image: "images/animals/panda.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κοάλα"],
            image: "images/animals/koala.jpg",
            difficulty: "easy"
        },

        {
            answers: ["λύκος"],
            image: "images/animals/wolf.jpg",
            difficulty: "easy"
        },

        {
            answers: ["αλεπού"],
            image: "images/animals/fox.jpg",
            difficulty: "easy"
        },

        {
            answers: ["καγκουρό"],
            image: "images/animals/kangaroo.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ιπποπόταμος"],
            image: "images/animals/hippopotamus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ρινόκερος"],
            image: "images/animals/rhinoceros.jpg",
            difficulty: "medium"
        },

        {
            answers: ["τσίτα"],
            image: "images/animals/cheetah.jpg",
            difficulty: "medium"
        },

        {
            answers: ["λεοπάρδαλη"],
            image: "images/animals/leopard.jpg",
            difficulty: "medium"
        },

        {
            answers: ["γορίλας"],
            image: "images/animals/gorilla.jpg",
            difficulty: "medium"
        },

        {
            answers: ["χιμπατζής"],
            image: "images/animals/chimpanzee.jpg",
            difficulty: "medium"
        },

        {
            answers: ["βούβαλος", "βουβάλι"],
            image: "images/animals/buffalo.jpg",
            difficulty: "medium"
        },

        {
            answers: ["καμήλα"],
            image: "images/animals/camel.jpg",
            difficulty: "medium"
        },

        {
            answers: ["πολική αρκούδα"],
            image: "images/animals/polar-bear.jpg",
            difficulty: "medium"
        },

        {
            answers: ["βίσονας"],
            image: "images/animals/bison.jpg",
            difficulty: "hard"
        }        ,

        {
            answers: ["τάρανδος"],
            image: "images/animals/reindeer.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ελάφι"],
            image: "images/animals/deer.jpg",
            difficulty: "easy"
        },

        {
            answers: ["άλκη"],
            image: "images/animals/moose.jpg",
            difficulty: "hard"
        },

        {
            answers: ["αγριογούρουνο"],
            image: "images/animals/wild-boar.jpg",
            difficulty: "medium"
        },

        {
            answers: ["γουρούνι"],
            image: "images/animals/pig.jpg",
            difficulty: "easy"
        },

        {
            answers: ["αγελάδα"],
            image: "images/animals/cow.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ταύρος"],
            image: "images/animals/bull.jpg",
            difficulty: "easy"
        },

        {
            answers: ["άλογο"],
            image: "images/animals/horse.jpg",
            difficulty: "easy"
        },

        {
            answers: ["γάιδαρος"],
            image: "images/animals/donkey.jpg",
            difficulty: "easy"
        },

        {
            answers: ["μουλάρι"],
            image: "images/animals/mule.jpg",
            difficulty: "medium"
        },

        {
            answers: ["πρόβατο"],
            image: "images/animals/sheep.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κατσίκα"],
            image: "images/animals/goat.jpg",
            difficulty: "easy"
        },

        {
            answers: ["λάμα"],
            image: "images/animals/llama.jpg",
            difficulty: "medium"
        },

        {
            answers: ["αλπάκα"],
            image: "images/animals/alpaca.jpg",
            difficulty: "medium"
        },

        {
            answers: ["βίδρα"],
            image: "images/animals/otter.jpg",
            difficulty: "medium"
        },

        {
            answers: ["κάστορας"],
            image: "images/animals/beaver.jpg",
            difficulty: "medium"
        },

        {
            answers: ["σκίουρος"],
            image: "images/animals/squirrel.jpg",
            difficulty: "easy"
        },

        {
            answers: ["σκαντζόχοιρος"],
            image: "images/animals/hedgehog.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ρακούν"],
            image: "images/animals/raccoon.jpg",
            difficulty: "medium"
        },

        {
            answers: ["βραδύπους"],
            image: "images/animals/sloth.jpg",
            difficulty: "hard"
        }        ,

        {
            answers: ["μαϊμού"],
            image: "images/animals/monkey.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ουρακοτάγκος"],
            image: "images/animals/orangutan.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μανδρίλος"],
            image: "images/animals/mandrill.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μπαμπουίνος"],
            image: "images/animals/baboon.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ύαινα"],
            image: "images/animals/hyena.jpg",
            difficulty: "medium"
        },

        {
            answers: ["κογιότ"],
            image: "images/animals/coyote.jpg",
            difficulty: "hard"
        },

        {
            answers: ["τσακάλι"],
            image: "images/animals/jackal.jpg",
            difficulty: "hard"
        },

        {
            answers: ["αρμαντίλο"],
            image: "images/animals/armadillo.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μυρμηγκοφάγος"],
            image: "images/animals/anteater.jpg",
            difficulty: "hard"
        },

        {
            answers: ["παγκολίνος"],
            image: "images/animals/pangolin.jpg",
            difficulty: "hard"
        },

        {
            answers: ["γουάλαμπι"],
            image: "images/animals/wallaby.jpg",
            difficulty: "hard"
        },

        {
            answers: ["γουόμπατ"],
            image: "images/animals/wombat.jpg",
            difficulty: "hard"
        },

        {
            answers: ["τάπιρος"],
            image: "images/animals/tapir.jpg",
            difficulty: "hard"
        },

        {
            answers: ["οκάπι"],
            image: "images/animals/okapi.jpg",
            difficulty: "hard"
        },

        {
            answers: ["λεμούριος"],
            image: "images/animals/lemur.jpg",
            difficulty: "medium"
        },

        {
            answers: ["σουρικάτα"],
            image: "images/animals/meerkat.jpg",
            difficulty: "medium"
        },

        {
            answers: ["νυχτερίδα"],
            image: "images/animals/bat.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κουνέλι"],
            image: "images/animals/rabbit.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ποντικός"],
            image: "images/animals/mouse.jpg",
            difficulty: "easy"
        },

        {
            answers: ["αρουραίος"],
            image: "images/animals/rat.jpg",
            difficulty: "medium"
        }        ,

        {
            answers: ["σκύλος"],
            image: "images/animals/dog.jpg",
            difficulty: "easy"
        },

        {
            answers: ["γάτα"],
            image: "images/animals/cat.jpg",
            difficulty: "easy"
        },

        {
            answers: ["αρκούδα"],
            image: "images/animals/bear.jpg",
            difficulty: "easy"
        },

        {
            answers: ["πάνθηρας"],
            image: "images/animals/panther.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ιαγουάρος"],
            image: "images/animals/jaguar.jpg",
            difficulty: "medium"
        },

        {
            answers: ["πούμα"],
            image: "images/animals/puma.jpg",
            difficulty: "medium"
        },

        {
            answers: ["λύγκας"],
            image: "images/animals/lynx.jpg",
            difficulty: "hard"
        },

        {
            answers: ["κόκκινο πάντα"],
            image: "images/animals/red-panda.jpg",
            difficulty: "medium"
        },

        {
            answers: ["φώκια"],
            image: "images/animals/seal.jpg",
            difficulty: "easy"
        },

        {
            answers: ["θαλάσσιος ελέφαντας"],
            image: "images/animals/elephant-seal.jpg",
            difficulty: "hard"
        },

        {
            answers: ["θαλάσσιο λιοντάρι"],
            image: "images/animals/sea-lion.jpg",
            difficulty: "medium"
        },

        {
            answers: ["θαλάσσιος ίππος"],
            image: "images/animals/walrus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ασβός"],
            image: "images/animals/badger.jpg",
            difficulty: "medium"
        },

        {
            answers: ["κουνάβι"],
            image: "images/animals/ferret.jpg",
            difficulty: "medium"
        },

        {
            answers: ["βίκουνα"],
            image: "images/animals/vicuna.jpg",
            difficulty: "hard"
        },

        {
            answers: ["γιάκ"],
            image: "images/animals/yak.jpg",
            difficulty: "hard"
        },

        {
            answers: ["γαζέλα"],
            image: "images/animals/gazelle.jpg",
            difficulty: "medium"
        },

        {
            answers: ["αντιλόπη"],
            image: "images/animals/antelope.jpg",
            difficulty: "medium"
        },

        {
            answers: ["λόρις"],
            image: "images/animals/slow-loris.jpg",
            difficulty: "hard"
        },

        {
            answers: ["καρακάλ"],
            image: "images/animals/caracal.jpg",
            difficulty: "hard"
        }

    ],

    birds: [
        {
            answers: ["αετός"],
            image: "images/birds/eagle.jpg",
            difficulty: "easy"
        },

        {
            answers: ["γεράκι"],
            image: "images/birds/hawk.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κουκουβάγια"],
            image: "images/birds/owl.jpg",
            difficulty: "easy"
        },

        {
            answers: ["παπαγάλος"],
            image: "images/birds/parrot.jpg",
            difficulty: "easy"
        },

        {
            answers: ["φλαμίνγκο"],
            image: "images/birds/flamingo.jpg",
            difficulty: "easy"
        },

        {
            answers: ["παγώνι"],
            image: "images/birds/peacock.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κύκνος"],
            image: "images/birds/swan.jpg",
            difficulty: "easy"
        },

        {
            answers: ["πελεκάνος"],
            image: "images/birds/pelican.jpg",
            difficulty: "easy"
        },

        {
            answers: ["πιγκουίνος"],
            image: "images/birds/penguin.jpg",
            difficulty: "easy"
        },

        {
            answers: ["στρουθοκάμηλος"],
            image: "images/birds/ostrich.jpg",
            difficulty: "easy"
        },

        {
            answers: ["γλάρος"],
            image: "images/birds/seagull.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κόρακας"],
            image: "images/birds/raven.jpg",
            difficulty: "medium"
        },

        {
            answers: ["κοράκι"],
            image: "images/birds/crow.jpg",
            difficulty: "medium"
        },

        {
            answers: ["γύπας"],
            image: "images/birds/vulture.jpg",
            difficulty: "medium"
        },

        {
            answers: ["καρδερίνα"],
            image: "images/birds/goldfinch.jpg",
            difficulty: "medium"
        },

        {
            answers: ["καναρίνι"],
            image: "images/birds/canary.jpg",
            difficulty: "easy"
        },

        {
            answers: ["χελιδόνι"],
            image: "images/birds/swallow.jpg",
            difficulty: "medium"
        },

        {
            answers: ["περιστέρι"],
            image: "images/birds/pigeon.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κόκορας"],
            image: "images/birds/rooster.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κότα"],
            image: "images/birds/chicken.jpg",
            difficulty: "easy"
        },
        {
            answers: ["πάπια"],
            image: "images/birds/duck.jpg",
            difficulty: "easy"
        },

        {
            answers: ["χήνα"],
            image: "images/birds/goose.jpg",
            difficulty: "easy"
        },

        {
            answers: ["γαλοπούλα"],
            image: "images/birds/turkey.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ορτύκι"],
            image: "images/birds/quail.jpg",
            difficulty: "medium"
        },

        {
            answers: ["φασιανός"],
            image: "images/birds/pheasant.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ερωδιός"],
            image: "images/birds/heron.jpg",
            difficulty: "medium"
        },

        {
            answers: ["λευκοτσικνιάς"],
            image: "images/birds/egret.jpg",
            difficulty: "hard"
        },

        {
            answers: ["πελαργός"],
            image: "images/birds/stork.jpg",
            difficulty: "easy"
        },

        {
            answers: ["αλκυόνη"],
            image: "images/birds/kingfisher.jpg",
            difficulty: "hard"
        },

        {
            answers: ["δρυοκολάπτης"],
            image: "images/birds/woodpecker.jpg",
            difficulty: "medium"
        },

        {
            answers: ["σπουργίτι"],
            image: "images/birds/sparrow.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κοκκινολαίμης"],
            image: "images/birds/robin.jpg",
            difficulty: "medium"
        },

        {
            answers: ["κίσσα"],
            image: "images/birds/magpie.jpg",
            difficulty: "medium"
        },

        {
            answers: ["καρακάξα"],
            image: "images/birds/jay.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ψαρόνι"],
            image: "images/birds/starling.jpg",
            difficulty: "medium"
        },

        {
            answers: ["κορυδαλλός"],
            image: "images/birds/lark.jpg",
            difficulty: "hard"
        },

        {
            answers: ["γκιώνης"],
            image: "images/birds/scops-owl.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μαυροπετρίτης"],
            image: "images/birds/eleonoras-falcon.jpg",
            difficulty: "hard"
        },

        {
            answers: ["καλαμοκανάς"],
            image: "images/birds/black-winged-stilt.jpg",
            difficulty: "hard"
        },

        {
            answers: ["χαλκόκοτα"],
            image: "images/birds/glossy-ibis.jpg",
            difficulty: "hard"
        },
        {
            answers: ["κακατούα"],
            image: "images/birds/cockatoo.jpg",
            difficulty: "easy"
        },

        {
            answers: ["μακάο"],
            image: "images/birds/macaw.jpg",
            difficulty: "medium"
        },

        {
            answers: ["τουκάν"],
            image: "images/birds/toucan.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κολιμπρί"],
            image: "images/birds/hummingbird.jpg",
            difficulty: "medium"
        },

        {
            answers: ["εμού"],
            image: "images/birds/emu.jpg",
            difficulty: "medium"
        },

        {
            answers: ["κασοβάριος"],
            image: "images/birds/cassowary.jpg",
            difficulty: "hard"
        },

        {
            answers: ["γερανός"],
            image: "images/birds/crane.jpg",
            difficulty: "medium"
        },

        {
            answers: ["πέρδικα"],
            image: "images/birds/partridge.jpg",
            difficulty: "medium"
        },

        {
            answers: ["μπεκάτσα"],
            image: "images/birds/woodcock.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μπεκατσίνι"],
            image: "images/birds/snipe.jpg",
            difficulty: "hard"
        },

        {
            answers: ["λαγγόνα"],
            image: "images/birds/cormorant.jpg",
            difficulty: "medium"
        },

        {
            answers: ["φρεγάτα"],
            image: "images/birds/frigatebird.jpg",
            difficulty: "hard"
        },

        {
            answers: ["άλμπατρος"],
            image: "images/birds/albatross.jpg",
            difficulty: "medium"
        },

        {
            answers: ["θαλασσοκόρακας"],
            image: "images/birds/shag.jpg",
            difficulty: "hard"
        },

        {
            answers: ["καρδινάλιος"],
            image: "images/birds/cardinal.jpg",
            difficulty: "medium"
        },

        {
            answers: ["μπλε τζέι"],
            image: "images/birds/blue-jay.jpg",
            difficulty: "medium"
        },

        {
            answers: ["καφετιά κουκουβάγια"],
            image: "images/birds/tawny-owl.jpg",
            difficulty: "hard"
        },

        {
            answers: ["χιονόκουκου"],
            image: "images/birds/snowy-owl.jpg",
            difficulty: "medium"
        },

        {
            answers: ["αρπυία"],
            image: "images/birds/harpy-eagle.jpg",
            difficulty: "hard"
        },

        {
            answers: ["θαλασσαετός"],
            image: "images/birds/sea-eagle.jpg",
            difficulty: "hard"
        },
        {
            answers: ["καναβούρι"],
            image: "images/birds/linnet.jpg",
            difficulty: "hard"
        },

        {
            answers: ["σπίνος"],
            image: "images/birds/chaffinch.jpg",
            difficulty: "medium"
        },

        {
            answers: ["φλώρος"],
            image: "images/birds/greenfinch.jpg",
            difficulty: "medium"
        },

        {
            answers: ["σταυρομύτης"],
            image: "images/birds/crossbill.jpg",
            difficulty: "hard"
        },

        {
            answers: ["κιρκινέζι"],
            image: "images/birds/kestrel.jpg",
            difficulty: "medium"
        },

        {
            answers: ["πετρίτης"],
            image: "images/birds/peregrine-falcon.jpg",
            difficulty: "medium"
        },

        {
            answers: ["σαΐνι"],
            image: "images/birds/goshawk.jpg",
            difficulty: "hard"
        },

        {
            answers: ["φιδαετός"],
            image: "images/birds/short-toed-eagle.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μελισσοφάγος"],
            image: "images/birds/bee-eater.jpg",
            difficulty: "medium"
        },

        {
            answers: ["τσαλαπετεινός"],
            image: "images/birds/hoopoe.jpg",
            difficulty: "medium"
        },

        {
            answers: ["νερόκοτα"],
            image: "images/birds/moorhen.jpg",
            difficulty: "hard"
        },

        {
            answers: ["φαλαρίδα"],
            image: "images/birds/coot.jpg",
            difficulty: "hard"
        },

        {
            answers: ["γλαρόνι"],
            image: "images/birds/tern.jpg",
            difficulty: "medium"
        },

        {
            answers: ["χαλκόκοτας"],
            image: "images/birds/glossy-ibis.jpg",
            difficulty: "hard"
        },

        {
            answers: ["στικταετός"],
            image: "images/birds/spotted-eagle.jpg",
            difficulty: "hard"
        },

        {
            answers: ["βαλτόπαπια"],
            image: "images/birds/ferruginous-duck.jpg",
            difficulty: "hard"
        },

        {
            answers: ["αργυροπελεκάνος"],
            image: "images/birds/dalmatian-pelican.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ροδοπελεκάνος"],
            image: "images/birds/great-white-pelican.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μαυροπελαργός"],
            image: "images/birds/black-stork.jpg",
            difficulty: "hard"
        },

        {
            answers: ["λευκοπελαργός"],
            image: "images/birds/white-stork.jpg",
            difficulty: "medium"
        }

    ],

    "snakes-reptiles-amphibians": [
        {
            answers: ["βασιλική κόμπρα"],
            image: "images/snakes-reptiles-amphibians/king-cobra.jpg",
            difficulty: "easy"
        },

        {
            answers: ["μαύρη μάμπα"],
            image: "images/snakes-reptiles-amphibians/black-mamba.jpg",
            difficulty: "easy"
        },

        {
            answers: ["πύθωνας"],
            image: "images/snakes-reptiles-amphibians/python.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ανακόντα"],
            image: "images/snakes-reptiles-amphibians/anaconda.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κροταλίας"],
            image: "images/snakes-reptiles-amphibians/rattlesnake.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κόμπρα"],
            image: "images/snakes-reptiles-amphibians/cobra.jpg",
            difficulty: "easy"
        },

        {
            answers: ["οχιά"],
            image: "images/snakes-reptiles-amphibians/viper.jpg",
            difficulty: "easy"
        },

        {
            answers: ["βόας"],
            image: "images/snakes-reptiles-amphibians/boa.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κοραλλιογενές φίδι"],
            image: "images/snakes-reptiles-amphibians/coral-snake.jpg",
            difficulty: "medium"
        },

        {
            answers: ["θαλάσσιο φίδι"],
            image: "images/snakes-reptiles-amphibians/sea-snake.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ταϊπάν"],
            image: "images/snakes-reptiles-amphibians/taipan.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μπουμσλανγκ"],
            image: "images/snakes-reptiles-amphibians/boomslang.jpg",
            difficulty: "hard"
        },

        {
            answers: ["δενδρόφιδο"],
            image: "images/snakes-reptiles-amphibians/tree-snake.jpg",
            difficulty: "medium"
        },

        {
            answers: ["πράσινη μάμπα"],
            image: "images/snakes-reptiles-amphibians/green-mamba.jpg",
            difficulty: "hard"
        },

        {
            answers: ["γάτα φίδι"],
            image: "images/snakes-reptiles-amphibians/cat-snake.jpg",
            difficulty: "hard"
        },

        {
            answers: ["φίδι γκαμπούν"],
            image: "images/snakes-reptiles-amphibians/gaboon-viper.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ίντιγκο φίδι"],
            image: "images/snakes-reptiles-amphibians/indigo-snake.jpg",
            difficulty: "hard"
        },

        {
            answers: ["φίδι καλαμποκιού"],
            image: "images/snakes-reptiles-amphibians/corn-snake.jpg",
            difficulty: "medium"
        },

        {
            answers: ["βασιλόφιδο"],
            image: "images/snakes-reptiles-amphibians/kingsnake.jpg",
            difficulty: "medium"
        },

        {
            answers: ["γάλα φίδι"],
            image: "images/snakes-reptiles-amphibians/milk-snake.jpg",
            difficulty: "medium"
        },
        {
            answers: ["κροκόδειλος"],
            image: "images/snakes-reptiles-amphibians/crocodile.jpg",
            difficulty: "easy"
        },

        {
            answers: ["αλιγάτορας"],
            image: "images/snakes-reptiles-amphibians/alligator.jpg",
            difficulty: "easy"
        },

        {
            answers: ["καϊμάν"],
            image: "images/snakes-reptiles-amphibians/caiman.jpg",
            difficulty: "medium"
        },

        {
            answers: ["γάβιαλος"],
            image: "images/snakes-reptiles-amphibians/gharial.jpg",
            difficulty: "hard"
        },

        {
            answers: ["δράκος του κομόντο"],
            image: "images/snakes-reptiles-amphibians/komodo-dragon.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ιγκουάνα"],
            image: "images/snakes-reptiles-amphibians/iguana.jpg",
            difficulty: "easy"
        },

        {
            answers: ["χαμαιλέοντας"],
            image: "images/snakes-reptiles-amphibians/chameleon.jpg",
            difficulty: "easy"
        },

        {
            answers: ["γκέκο"],
            image: "images/snakes-reptiles-amphibians/gecko.jpg",
            difficulty: "medium"
        },

        {
            answers: ["σαλαμάνδρα"],
            image: "images/snakes-reptiles-amphibians/salamander.jpg",
            difficulty: "medium"
        },

        {
            answers: ["τρίτωνας"],
            image: "images/snakes-reptiles-amphibians/newt.jpg",
            difficulty: "hard"
        },

        {
            answers: ["βάτραχος"],
            image: "images/snakes-reptiles-amphibians/frog.jpg",
            difficulty: "easy"
        },

        {
            answers: ["φρύνος"],
            image: "images/snakes-reptiles-amphibians/toad.jpg",
            difficulty: "easy"
        },

        {
            answers: ["δενδροβάτραχος"],
            image: "images/snakes-reptiles-amphibians/tree-frog.jpg",
            difficulty: "medium"
        },

        {
            answers: ["χελώνα"],
            image: "images/snakes-reptiles-amphibians/tortoise.jpg",
            difficulty: "easy"
        },

        {
            answers: ["θαλάσσια χελώνα"],
            image: "images/snakes-reptiles-amphibians/sea-turtle.jpg",
            difficulty: "easy"
        },

        {
            answers: ["δερματοχελώνα"],
            image: "images/snakes-reptiles-amphibians/leatherback-turtle.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μαλακή χελώνα"],
            image: "images/snakes-reptiles-amphibians/softshell-turtle.jpg",
            difficulty: "hard"
        },

        {
            answers: ["σαύρα"],
            image: "images/snakes-reptiles-amphibians/lizard.jpg",
            difficulty: "easy"
        },

        {
            answers: ["σκίγκος"],
            image: "images/snakes-reptiles-amphibians/skink.jpg",
            difficulty: "hard"
        },

        {
            answers: ["αγκάμα"],
            image: "images/snakes-reptiles-amphibians/agama.jpg",
            difficulty: "hard"
        },
        {
            answers: ["βασιλίσκος"],
            image: "images/snakes-reptiles-amphibians/basilisk-lizard.jpg",
            difficulty: "hard"
        },

        {
            answers: ["γενειοφόρος δράκος"],
            image: "images/snakes-reptiles-amphibians/bearded-dragon.jpg",
            difficulty: "medium"
        },

        {
            answers: ["μονίτορ σαύρα"],
            image: "images/snakes-reptiles-amphibians/monitor-lizard.jpg",
            difficulty: "medium"
        },

        {
            answers: ["τέγκου"],
            image: "images/snakes-reptiles-amphibians/tegu.jpg",
            difficulty: "hard"
        },

        {
            answers: ["γκίλα τέρας"],
            image: "images/snakes-reptiles-amphibians/gila-monster.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μεξικανική σαύρα με χάντρες"],
            image: "images/snakes-reptiles-amphibians/mexican-beaded-lizard.jpg",
            difficulty: "hard"
        },

        {
            answers: ["δενδροσαύρα"],
            image: "images/snakes-reptiles-amphibians/tree-lizard.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ανόλη"],
            image: "images/snakes-reptiles-amphibians/anole.jpg",
            difficulty: "hard"
        },

        {
            answers: ["τυφλόφιδο"],
            image: "images/snakes-reptiles-amphibians/blind-snake.jpg",
            difficulty: "hard"
        },

        {
            answers: ["χάλκινο φίδι"],
            image: "images/snakes-reptiles-amphibians/copperhead.jpg",
            difficulty: "medium"
        },

        {
            answers: ["μοκασίνι του νερού"],
            image: "images/snakes-reptiles-amphibians/cottonmouth.jpg",
            difficulty: "hard"
        },

        {
            answers: ["οχιά με κέρατα"],
            image: "images/snakes-reptiles-amphibians/horned-viper.jpg",
            difficulty: "hard"
        },

        {
            answers: ["οχιά γκαμπούν"],
            image: "images/snakes-reptiles-amphibians/gaboon-viper.jpg",
            difficulty: "hard"
        },

        {
            answers: ["θαλάσσια ιγκουάνα"],
            image: "images/snakes-reptiles-amphibians/marine-iguana.jpg",
            difficulty: "hard"
        },

        {
            answers: ["γκέκο λεοπάρδαλη"],
            image: "images/snakes-reptiles-amphibians/leopard-gecko.jpg",
            difficulty: "medium"
        },

        {
            answers: ["μπλε γλώσσα σκίγκος"],
            image: "images/snakes-reptiles-amphibians/blue-tongue-skink.jpg",
            difficulty: "hard"
        },

        {
            answers: ["αξολότλ"],
            image: "images/snakes-reptiles-amphibians/axolotl.jpg",
            difficulty: "medium"
        },

        {
            answers: ["δηλητηριώδης βάτραχος"],
            image: "images/snakes-reptiles-amphibians/poison-dart-frog.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ταυροβάτραχος"],
            image: "images/snakes-reptiles-amphibians/bullfrog.jpg",
            difficulty: "medium"
        },

        {
            answers: ["καικίλιος"],
            image: "images/snakes-reptiles-amphibians/caecilian.jpg",
            difficulty: "hard"
        },
        {
            answers: ["πράσινο δενδρόφιδο"],
            image: "images/snakes-reptiles-amphibians/green-tree-snake.jpg",
            difficulty: "medium"
        },

        {
            answers: ["φίδι αμπελιού"],
            image: "images/snakes-reptiles-amphibians/vine-snake.jpg",
            difficulty: "hard"
        },

        {
            answers: ["δίκρωμο φίδι"],
            image: "images/snakes-reptiles-amphibians/kukri-snake.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μαύρος πύθωνας"],
            image: "images/snakes-reptiles-amphibians/black-python.jpg",
            difficulty: "hard"
        },

        {
            answers: ["πράσινος πύθωνας"],
            image: "images/snakes-reptiles-amphibians/green-tree-python.jpg",
            difficulty: "medium"
        },

        {
            answers: ["βραχώδης πύθωνας"],
            image: "images/snakes-reptiles-amphibians/rock-python.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ερυθρός βόας"],
            image: "images/snakes-reptiles-amphibians/red-tail-boa.jpg",
            difficulty: "medium"
        },

        {
            answers: ["χελώνα αλιγάτορας"],
            image: "images/snakes-reptiles-amphibians/alligator-snapping-turtle.jpg",
            difficulty: "hard"
        },

        {
            answers: ["χαρτοχελώνα"],
            image: "images/snakes-reptiles-amphibians/map-turtle.jpg",
            difficulty: "hard"
        },

        {
            answers: ["πράσινη θαλάσσια χελώνα"],
            image: "images/snakes-reptiles-amphibians/green-sea-turtle.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ερυθροπόδαρη χελώνα"],
            image: "images/snakes-reptiles-amphibians/red-footed-tortoise.jpg",
            difficulty: "hard"
        },

        {
            answers: ["λεοπάρ χελώνα"],
            image: "images/snakes-reptiles-amphibians/leopard-tortoise.jpg",
            difficulty: "hard"
        },

        {
            answers: ["σαύρα με λοφίο"],
            image: "images/snakes-reptiles-amphibians/frilled-lizard.jpg",
            difficulty: "medium"
        },

        {
            answers: ["γυάλινη σαύρα"],
            image: "images/snakes-reptiles-amphibians/glass-lizard.jpg",
            difficulty: "hard"
        },

        {
            answers: ["κερασφόρος βάτραχος"],
            image: "images/snakes-reptiles-amphibians/horned-frog.jpg",
            difficulty: "medium"
        },

        {
            answers: ["βάτραχος ταύρος αφρικής"],
            image: "images/snakes-reptiles-amphibians/african-bullfrog.jpg",
            difficulty: "hard"
        },

        {
            answers: ["πυριτόσαυρα"],
            image: "images/snakes-reptiles-amphibians/fire-salamander.jpg",
            difficulty: "medium"
        },

        {
            answers: ["τίγρης σαλαμάνδρα"],
            image: "images/snakes-reptiles-amphibians/tiger-salamander.jpg",
            difficulty: "hard"
        },

        {
            answers: ["κινέζικη γιγάντια σαλαμάνδρα"],
            image: "images/snakes-reptiles-amphibians/chinese-giant-salamander.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ιαπωνική γιγάντια σαλαμάνδρα"],
            image: "images/snakes-reptiles-amphibians/japanese-giant-salamander.jpg",
            difficulty: "hard"
        }

    ],

    "sea-animals": [
        {
            answers: ["μεγάλος λευκός καρχαρίας"],
            image: "images/sea-animals/great-white-shark.jpg",
            difficulty: "easy"
        },

        {
            answers: ["καρχαρίας τίγρης"],
            image: "images/sea-animals/tiger-shark.jpg",
            difficulty: "medium"
        },

        {
            answers: ["σφυροκέφαλος καρχαρίας"],
            image: "images/sea-animals/hammerhead-shark.jpg",
            difficulty: "easy"
        },

        {
            answers: ["φάλαινα"],
            image: "images/sea-animals/whale.jpg",
            difficulty: "easy"
        },

        {
            answers: ["γαλάζια φάλαινα"],
            image: "images/sea-animals/blue-whale.jpg",
            difficulty: "easy"
        },

        {
            answers: ["όρκα"],
            image: "images/sea-animals/orca.jpg",
            difficulty: "easy"
        },

        {
            answers: ["δελφίνι"],
            image: "images/sea-animals/dolphin.jpg",
            difficulty: "easy"
        },

        {
            answers: ["φυσητήρας"],
            image: "images/sea-animals/sperm-whale.jpg",
            difficulty: "medium"
        },

        {
            answers: ["μπελούγκα"],
            image: "images/sea-animals/beluga.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ναρβάλ"],
            image: "images/sea-animals/narwhal.jpg",
            difficulty: "medium"
        },

        {
            answers: ["φάλαινα όρκα ψευδής"],
            image: "images/sea-animals/false-killer-whale.jpg",
            difficulty: "hard"
        },

        {
            answers: ["φώκια"],
            image: "images/sea-animals/seal.jpg",
            difficulty: "easy"
        },

        {
            answers: ["θαλάσσιο λιοντάρι"],
            image: "images/sea-animals/sea-lion.jpg",
            difficulty: "easy"
        },

        {
            answers: ["θαλάσσιος ίππος"],
            image: "images/sea-animals/walrus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["μανάτι"],
            image: "images/sea-animals/manatee.jpg",
            difficulty: "medium"
        },

        {
            answers: ["δουγκόνγκος"],
            image: "images/sea-animals/dugong.jpg",
            difficulty: "hard"
        },

        {
            answers: ["χταπόδι"],
            image: "images/sea-animals/octopus.jpg",
            difficulty: "easy"
        },

        {
            answers: ["γιγάντιο χταπόδι του ειρηνικού"],
            image: "images/sea-animals/giant-pacific-octopus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["καλαμάρι"],
            image: "images/sea-animals/squid.jpg",
            difficulty: "easy"
        },

        {
            answers: ["γιγάντιο καλαμάρι"],
            image: "images/sea-animals/giant-squid.jpg",
            difficulty: "medium"
        },

        {
            answers: ["σουπιά"],
            image: "images/sea-animals/cuttlefish.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ναυτίλος"],
            image: "images/sea-animals/nautilus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["αστακός"],
            image: "images/sea-animals/lobster.jpg",
            difficulty: "easy"
        },

        {
            answers: ["καβούρι"],
            image: "images/sea-animals/crab.jpg",
            difficulty: "easy"
        },

        {
            answers: ["βασιλικό καβούρι"],
            image: "images/sea-animals/king-crab.jpg",
            difficulty: "medium"
        },

        {
            answers: ["γαρίδα"],
            image: "images/sea-animals/shrimp.jpg",
            difficulty: "easy"
        },

        {
            answers: ["γαρίδα μάντις"],
            image: "images/sea-animals/mantis-shrimp.jpg",
            difficulty: "hard"
        },

        {
            answers: ["καραβίδα"],
            image: "images/sea-animals/crayfish.jpg",
            difficulty: "medium"
        },

        {
            answers: ["αχινός"],
            image: "images/sea-animals/sea-urchin.jpg",
            difficulty: "medium"
        },

        {
            answers: ["αστερίας"],
            image: "images/sea-animals/starfish.jpg",
            difficulty: "easy"
        },

        {
            answers: ["αγγούρι της θάλασσας"],
            image: "images/sea-animals/sea-cucumber.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μέδουσα"],
            image: "images/sea-animals/jellyfish.jpg",
            difficulty: "easy"
        },

        {
            answers: ["πορτογαλική καραβέλα"],
            image: "images/sea-animals/portuguese-man-of-war.jpg",
            difficulty: "hard"
        },

        {
            answers: ["κοράλλι"],
            image: "images/sea-animals/coral.jpg",
            difficulty: "medium"
        },

        {
            answers: ["σφουγγάρι"],
            image: "images/sea-animals/sea-sponge.jpg",
            difficulty: "medium"
        },

        {
            answers: ["χιτώνας"],
            image: "images/sea-animals/chiton.jpg",
            difficulty: "hard"
        },

        {
            answers: ["αχιβάδα"],
            image: "images/sea-animals/clam.jpg",
            difficulty: "easy"
        },

        {
            answers: ["στρείδι"],
            image: "images/sea-animals/oyster.jpg",
            difficulty: "easy"
        },

        {
            answers: ["μύδι"],
            image: "images/sea-animals/mussel.jpg",
            difficulty: "easy"
        },

        {
            answers: ["χτένι"],
            image: "images/sea-animals/scallop.jpg",
            difficulty: "medium"
        },
        {
            answers: ["σαλάχι"],
            image: "images/sea-animals/stingray.jpg",
            difficulty: "easy"
        },

        {
            answers: ["σαλάχι μάντα"],
            image: "images/sea-animals/manta-ray.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ηλεκτρικό σαλάχι"],
            image: "images/sea-animals/electric-ray.jpg",
            difficulty: "medium"
        },

        {
            answers: ["πριονόψαρο"],
            image: "images/sea-animals/sawfish.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ξιφίας"],
            image: "images/sea-animals/swordfish.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ιστιοφόρο ψάρι"],
            image: "images/sea-animals/sailfish.jpg",
            difficulty: "medium"
        },

        {
            answers: ["μάρλιν"],
            image: "images/sea-animals/marlin.jpg",
            difficulty: "medium"
        },

        {
            answers: ["τόνος"],
            image: "images/sea-animals/tuna.jpg",
            difficulty: "easy"
        },

        {
            answers: ["σολομός"],
            image: "images/sea-animals/salmon.jpg",
            difficulty: "easy"
        },

        {
            answers: ["μπαρακούντα"],
            image: "images/sea-animals/barracuda.jpg",
            difficulty: "medium"
        },

        {
            answers: ["χέλι"],
            image: "images/sea-animals/eel.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ηλεκτρικό χέλι"],
            image: "images/sea-animals/electric-eel.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ιππόκαμπος"],
            image: "images/sea-animals/seahorse.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ψάρι κλόουν"],
            image: "images/sea-animals/clownfish.jpg",
            difficulty: "easy"
        },

        {
            answers: ["μπλε χειρουργός"],
            image: "images/sea-animals/blue-tang.jpg",
            difficulty: "medium"
        },

        {
            answers: ["λεοντόψαρο"],
            image: "images/sea-animals/lionfish.jpg",
            difficulty: "medium"
        },

        {
            answers: ["πέτρινο ψάρι"],
            image: "images/sea-animals/stonefish.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ψάρι μπαλόνι"],
            image: "images/sea-animals/pufferfish.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ψάρι σκαντζόχοιρος"],
            image: "images/sea-animals/porcupinefish.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μορφήνα"],
            image: "images/sea-animals/moray-eel.jpg",
            difficulty: "medium"
        },

        {
            answers: ["γοβιός"],
            image: "images/sea-animals/goby.jpg",
            difficulty: "hard"
        },

        {
            answers: ["δίσκος"],
            image: "images/sea-animals/discus-fish.jpg",
            difficulty: "medium"
        },

        {
            answers: ["αγγελόψαρο"],
            image: "images/sea-animals/angelfish.jpg",
            difficulty: "easy"
        },

        {
            answers: ["πεταλούδα ψάρι"],
            image: "images/sea-animals/butterflyfish.jpg",
            difficulty: "medium"
        },

        {
            answers: ["παπαγάλος ψάρι"],
            image: "images/sea-animals/parrotfish.jpg",
            difficulty: "medium"
        },

        {
            answers: ["δράκος της θάλασσας"],
            image: "images/sea-animals/leafy-seadragon.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ιπτάμενο ψάρι"],
            image: "images/sea-animals/flying-fish.jpg",
            difficulty: "medium"
        },

        {
            answers: ["καρχαρίας φάλαινα"],
            image: "images/sea-animals/whale-shark.jpg",
            difficulty: "easy"
        },

        {
            answers: ["καρχαρίας προσκυνητής"],
            image: "images/sea-animals/basking-shark.jpg",
            difficulty: "medium"
        },

        {
            answers: ["καρχαρίας ταύρος"],
            image: "images/sea-animals/bull-shark.jpg",
            difficulty: "medium"
        },

        {
            answers: ["καρχαρίας μάκο"],
            image: "images/sea-animals/mako-shark.jpg",
            difficulty: "medium"
        },

        {
            answers: ["καρχαρίας αλεπού"],
            image: "images/sea-animals/thresher-shark.jpg",
            difficulty: "hard"
        },

        {
            answers: ["καρχαρίας νοσοκόμος"],
            image: "images/sea-animals/nurse-shark.jpg",
            difficulty: "medium"
        },

        {
            answers: ["καρχαρίας λεοπάρδαλη"],
            image: "images/sea-animals/leopard-shark.jpg",
            difficulty: "hard"
        },

        {
            answers: ["καρχαρίας ζέβρα"],
            image: "images/sea-animals/zebra-shark.jpg",
            difficulty: "hard"
        },

        {
            answers: ["καρχαρίας γκόμπλιν"],
            image: "images/sea-animals/goblin-shark.jpg",
            difficulty: "hard"
        },

        {
            answers: ["καρχαρίας με φραμπαλά"],
            image: "images/sea-animals/frilled-shark.jpg",
            difficulty: "hard"
        },

        {
            answers: ["καρχαρίας αγγελούδι"],
            image: "images/sea-animals/angel-shark.jpg",
            difficulty: "hard"
        },

        {
            answers: ["καρχαρίας λεμόνι"],
            image: "images/sea-animals/lemon-shark.jpg",
            difficulty: "medium"
        },

        {
            answers: ["καρχαρίας μαυροπτέρυγος"],
            image: "images/sea-animals/blacktip-shark.jpg",
            difficulty: "medium"
        }

    ],

    dinosaurs: [
        {
            answers: ["τυραννόσαυρος ρεξ"],
            image: "images/dinosaurs/tyrannosaurus-rex.jpg",
            difficulty: "easy"
        },

        {
            answers: ["βελοσιράπτορας"],
            image: "images/dinosaurs/velociraptor.jpg",
            difficulty: "easy"
        },

        {
            answers: ["τρικεράτωπας"],
            image: "images/dinosaurs/triceratops.jpg",
            difficulty: "easy"
        },

        {
            answers: ["στεγόσαυρος"],
            image: "images/dinosaurs/stegosaurus.jpg",
            difficulty: "easy"
        },

        {
            answers: ["βραχιόσαυρος"],
            image: "images/dinosaurs/brachiosaurus.jpg",
            difficulty: "easy"
        },

        {
            answers: ["διπλόδοκος"],
            image: "images/dinosaurs/diplodocus.jpg",
            difficulty: "easy"
        },

        {
            answers: ["σπινόσαυρος"],
            image: "images/dinosaurs/spinosaurus.jpg",
            difficulty: "easy"
        },

        {
            answers: ["αγκυλόσαυρος"],
            image: "images/dinosaurs/ankylosaurus.jpg",
            difficulty: "easy"
        },

        {
            answers: ["παχυκεφαλόσαυρος"],
            image: "images/dinosaurs/pachycephalosaurus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["παρασαυρόλοφος"],
            image: "images/dinosaurs/parasaurolophus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["αλλόσαυρος"],
            image: "images/dinosaurs/allosaurus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["καρνόταυρος"],
            image: "images/dinosaurs/carnotaurus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["γιγαντόσαυρος"],
            image: "images/dinosaurs/giganotosaurus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["σερατόσαυρος"],
            image: "images/dinosaurs/ceratosaurus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ιγκουανόδοντας"],
            image: "images/dinosaurs/iguanodon.jpg",
            difficulty: "medium"
        },

        {
            answers: ["κορυθόσαυρος"],
            image: "images/dinosaurs/corythosaurus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["μαγιασαύρα"],
            image: "images/dinosaurs/maiasaura.jpg",
            difficulty: "medium"
        },

        {
            answers: ["κομψόγναθος"],
            image: "images/dinosaurs/compsognathus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["δεϊνόνυχος"],
            image: "images/dinosaurs/deinonychus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ουταράπτορας"],
            image: "images/dinosaurs/utahraptor.jpg",
            difficulty: "hard"
        },

        {
            answers: ["απατόσαυρος"],
            image: "images/dinosaurs/apatosaurus.jpg",
            difficulty: "easy"
        },

        {
            answers: ["καμαράσαυρος"],
            image: "images/dinosaurs/camarasaurus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["αλβερτόσαυρος"],
            image: "images/dinosaurs/albertosaurus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ακροκανθόσαυρος"],
            image: "images/dinosaurs/acrocanthosaurus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μονολόφοσαυρος"],
            image: "images/dinosaurs/monolophosaurus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["οβιράπτορας"],
            image: "images/dinosaurs/oviraptor.jpg",
            difficulty: "medium"
        },

        {
            answers: ["γκαλιμίμος"],
            image: "images/dinosaurs/gallimimus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["θηριζινόσαυρος"],
            image: "images/dinosaurs/therizinosaurus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["σαυχένια"],
            image: "images/dinosaurs/sauschenia.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ευοπλόκεφαλος"],
            image: "images/dinosaurs/euoplocephalus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["προτοκεράτωπας"],
            image: "images/dinosaurs/protoceratops.jpg",
            difficulty: "medium"
        },

        {
            answers: ["στυρακόσαυρος"],
            image: "images/dinosaurs/styracosaurus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["πεντακεράτωπας"],
            image: "images/dinosaurs/pentaceratops.jpg",
            difficulty: "hard"
        },

        {
            answers: ["κεντρόσαυρος"],
            image: "images/dinosaurs/centrosaurus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["σαουρόπελτα"],
            image: "images/dinosaurs/sauropelta.jpg",
            difficulty: "hard"
        },

        {
            answers: ["εδμοντοσαυρος"],
            image: "images/dinosaurs/edmontosaurus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ληαελλυνάσαυρα"],
            image: "images/dinosaurs/leaellynasaura.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μουτταμπούρασαυρος"],
            image: "images/dinosaurs/muttaburrasaurus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["πλατεόσαυρος"],
            image: "images/dinosaurs/plateosaurus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["κοελοφύσις"],
            image: "images/dinosaurs/coelophysis.jpg",
            difficulty: "hard"
        },
        {
            answers: ["δασπλητόσαυρος"],
            image: "images/dinosaurs/daspletosaurus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ταρβόσαυρος"],
            image: "images/dinosaurs/tarbosaurus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μαπουσάυρος"],
            image: "images/dinosaurs/mapusaurus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["καρχαροδοντόσαυρος"],
            image: "images/dinosaurs/carcharodontosaurus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ρουγκόπς"],
            image: "images/dinosaurs/rugops.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μαζουνγκάσαυρος"],
            image: "images/dinosaurs/majungasaurus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["βαρυόνυχος"],
            image: "images/dinosaurs/baryonyx.jpg",
            difficulty: "medium"
        },

        {
            answers: ["σουχομίμος"],
            image: "images/dinosaurs/suchomimus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ιριτάτορας"],
            image: "images/dinosaurs/irritator.jpg",
            difficulty: "hard"
        },

        {
            answers: ["αμαργάσαυρος"],
            image: "images/dinosaurs/amargasaurus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["σαλτασαυρος"],
            image: "images/dinosaurs/saltasaurus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["αργεντινόσαυρος"],
            image: "images/dinosaurs/argentinosaurus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["παταγοτιτάνας"],
            image: "images/dinosaurs/patagotitan.jpg",
            difficulty: "hard"
        },

        {
            answers: ["νοτόσαυρος"],
            image: "images/dinosaurs/nothosaurus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["κρυολοφόσαυρος"],
            image: "images/dinosaurs/cryolophosaurus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["διλοφόσαυρος"],
            image: "images/dinosaurs/dilophosaurus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["εοραπτόρας"],
            image: "images/dinosaurs/eoraptor.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ερρέραςαυρος"],
            image: "images/dinosaurs/herrerasaurus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μουσσαύρος"],
            image: "images/dinosaurs/mussaurus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["μικροραπτόρας"],
            image: "images/dinosaurs/microraptor.jpg",
            difficulty: "hard"
        },

        {
            answers: ["αρχαιοπτέρυξ"],
            image: "images/dinosaurs/archaeopteryx.jpg",
            difficulty: "medium"
        },

        {
            answers: ["γιγαντοράπτορας"],
            image: "images/dinosaurs/gigantoraptor.jpg",
            difficulty: "hard"
        },

        {
            answers: ["σινόσαυροπτερυξ"],
            image: "images/dinosaurs/sinosauropteryx.jpg",
            difficulty: "hard"
        },

        {
            answers: ["τροόδοντας"],
            image: "images/dinosaurs/troodon.jpg",
            difficulty: "medium"
        },

        {
            answers: ["τροοπεόγναθος"],
            image: "images/dinosaurs/tropeognathus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["πτεροδάκτυλος"],
            image: "images/dinosaurs/pterodactylus.jpg",
            difficulty: "easy"
        },

        {
            answers: ["πτερανόδοντας"],
            image: "images/dinosaurs/pteranodon.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κετζαλκοάτλους"],
            image: "images/dinosaurs/quetzalcoatlus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ραμφορύγχος"],
            image: "images/dinosaurs/rhamphorhynchus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ταπεγιάρα"],
            image: "images/dinosaurs/tupandactylus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["διμορφόδοντας"],
            image: "images/dinosaurs/dimorphodon.jpg",
            difficulty: "hard"
        },

        {
            answers: ["κρόνόσαυρος"],
            image: "images/dinosaurs/kronosaurus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["μοσάσαυρος"],
            image: "images/dinosaurs/mosasaurus.jpg",
            difficulty: "easy"
        },

        {
            answers: ["πλεισιόσαυρος"],
            image: "images/dinosaurs/plesiosaurus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ελασμόσαυρος"],
            image: "images/dinosaurs/elasmosaurus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ιχθυόσαυρος"],
            image: "images/dinosaurs/ichthyosaurus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["λιοπλευρόδοντας"],
            image: "images/dinosaurs/liopleurodon.jpg",
            difficulty: "hard"
        },

        {
            answers: ["πλακόδους"],
            image: "images/dinosaurs/placodus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["τανυστρόφεας"],
            image: "images/dinosaurs/tanystropheus.jpg",
            difficulty: "hard"
        },

        {
            answers: ["στουπεντέμις"],
            image: "images/dinosaurs/stupendemys.jpg",
            difficulty: "hard"
        }

    ],

    "football-players": [
        {
            answers: ["λιονέλ μέσι", "μέσι"],
            image: "images/football-players/lionel-messi.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κριστιάνο ρονάλντο", "ρονάλντο"],
            image: "images/football-players/cristiano-ronaldo.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κιλιάν εμπαπέ", "εμπαπέ"],
            image: "images/football-players/kylian-mbappe.jpg",
            difficulty: "easy"
        },

        {
            answers: ["έρλινγκ χάαλαντ", "χάαλαντ"],
            image: "images/football-players/erling-haaland.jpg",
            difficulty: "easy"
        },

        {
            answers: ["λαμίν γιαμάλ", "γιαμάλ"],
            image: "images/football-players/lamine-yamal.jpg",
            difficulty: "easy"
        },

        {
            answers: ["τζουντ μπέλιγχαμ", "μπέλιγχαμ"],
            image: "images/football-players/jude-bellingham.jpg",
            difficulty: "easy"
        },

        {
            answers: ["βινίσιους τζούνιορ", "βινίσιους"],
            image: "images/football-players/vinicius-junior.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ροδρί"],
            image: "images/football-players/rodri.jpg",
            difficulty: "medium"
        },

        {
            answers: ["μοχάμεντ σαλάχ", "σαλάχ"],
            image: "images/football-players/mohamed-salah.jpg",
            difficulty: "easy"
        },

        {
            answers: ["χάρι κέιν", "κέιν"],
            image: "images/football-players/harry-kane.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ρομπέρτ λεβαντόφσκι", "λεβαντόφσκι"],
            image: "images/football-players/robert-lewandowski.jpg",
            difficulty: "easy"
        },

        {
            answers: ["νεϊμάρ", "νεϊμαρ"],
            image: "images/football-players/neymar.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κεβιν ντε μπρόινε", "ντε μπρόινε"],
            image: "images/football-players/kevin-de-bruyne.jpg",
            difficulty: "easy"
        },

        {
            answers: ["λουίς σουάρες", "σουάρες"],
            image: "images/football-players/luis-suarez.jpg",
            difficulty: "easy"
        },

        {
            answers: ["αντουάν γκριεζμάν", "γκριεζμάν"],
            image: "images/football-players/antoine-griezmann.jpg",
            difficulty: "medium"
        },

        {
            answers: ["μπουκάγιο σάκα", "σάκα"],
            image: "images/football-players/bukayo-saka.jpg",
            difficulty: "medium"
        },

        {
            answers: ["φιλ φόντεν", "φόντεν"],
            image: "images/football-players/phil-foden.jpg",
            difficulty: "medium"
        },

        {
            answers: ["τζαμάλ μουσιάλα", "μουσιάλα"],
            image: "images/football-players/jamal-musiala.jpg",
            difficulty: "medium"
        },

        {
            answers: ["κολ παλμερ", "πάλμερ"],
            image: "images/football-players/cole-palmer.jpg",
            difficulty: "medium"
        },

        {
            answers: ["βίκτορ γκιόκερες", "γκιόκερες"],
            image: "images/football-players/viktor-gyokeres.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ραφίνια"],
            image: "images/football-players/raphinha.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ουσμάν ντεμπελέ", "ντεμπελέ"],
            image: "images/football-players/ousmane-dembele.jpg",
            difficulty: "medium"
        },

        {
            answers: ["χβίτσα κβαρατσχέλια", "κβαρατσχέλια"],
            image: "images/football-players/khvicha-kvaratskhelia.jpg",
            difficulty: "hard"
        },

        {
            answers: ["τζιανλουίτζι ντοναρούμα", "ντοναρούμα"],
            image: "images/football-players/gianluigi-donnarumma.jpg",
            difficulty: "medium"
        },

        {
            answers: ["τιμπό κουρτουά", "κουρτουά"],
            image: "images/football-players/thibaut-courtois.jpg",
            difficulty: "medium"
        },

        {
            answers: ["μαρκ Αντρέ τερ στέγκεν", "τερ στέγκεν"],
            image: "images/football-players/marc-andre-ter-stegen.jpg",
            difficulty: "medium"
        },

        {
            answers: ["βίρτζιλ φαν ντάικ", "φαν ντάικ"],
            image: "images/football-players/virgil-van-dijk.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ρουμπέν ντίας"],
            image: "images/football-players/ruben-dias.jpg",
            difficulty: "medium"
        },

        {
            answers: ["αλεξάντερ Άρνολντ", "Άρνολντ"],
            image: "images/football-players/trent-alexander-arnold.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ασράφ χακίμι", "χακίμι"],
            image: "images/football-players/achraf-hakimi.jpg",
            difficulty: "medium"
        },

        {
            answers: ["φεδερίκο βαλβέρδε", "βαλβέρδε"],
            image: "images/football-players/federico-valverde.jpg",
            difficulty: "medium"
        },

        {
            answers: ["πεντρί"],
            image: "images/football-players/pedri.jpg",
            difficulty: "medium"
        },

        {
            answers: ["γκάβι"],
            image: "images/football-players/gavi.jpg",
            difficulty: "medium"
        },

        {
            answers: ["μπρούνο φερνάντες"],
            image: "images/football-players/bruno-fernandes.jpg",
            difficulty: "medium"
        },

        {
            answers: ["μπερνάρντο σίλβα"],
            image: "images/football-players/bernardo-silva.jpg",
            difficulty: "medium"
        },

        {
            answers: ["καζεμίρο"],
            image: "images/football-players/casemiro.jpg",
            difficulty: "medium"
        },

        {
            answers: ["τόνι κρόος", "κρόος"],
            image: "images/football-players/toni-kroos.jpg",
            difficulty: "medium"
        },

        {
            answers: ["λούκα μόντριτς", "μόντριτς"],
            image: "images/football-players/luka-modric.jpg",
            difficulty: "easy"
        },

        {
            answers: ["σέρχιο ράμος", "ράμος"],
            image: "images/football-players/sergio-ramos.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ζλάταν ιμπραΐμοβιτς", "ιμπραΐμοβιτς"],
            image: "images/football-players/zlatan-ibrahimovic.jpg",
            difficulty: "easy"
        },
        {
            answers: ["αντρές ινιέστα", "ινιέστα"],
            image: "images/football-players/andres-iniesta.jpg",
            difficulty: "easy"
        },

        {
            answers: ["τσάβι", "τσάβι ερνάντεθ"],
            image: "images/football-players/xavi.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ροναλντίνιο"],
            image: "images/football-players/ronaldinho.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κακά"],
            image: "images/football-players/kaka.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ζινεντίν ζιντάν", "ζιντάν"],
            image: "images/football-players/zinedine-zidane.jpg",
            difficulty: "easy"
        },

        {
            answers: ["πελέ"],
            image: "images/football-players/pele.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ντιέγκο μαραντόνα", "μαραντόνα"],
            image: "images/football-players/diego-maradona.jpg",
            difficulty: "easy"
        },

        {
            answers: ["γιόχαν κρόιφ", "κρόιφ"],
            image: "images/football-players/johan-cruyff.jpg",
            difficulty: "medium"
        },

        {
            answers: ["φραντς μπεκενμπάουερ", "μπεκενμπάουερ"],
            image: "images/football-players/franz-beckenbauer.jpg",
            difficulty: "medium"
        },

        {
            answers: ["πάολο μαλντίνι", "μαλντίνι"],
            image: "images/football-players/paolo-maldini.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ρομπέρτο μπάτζιο", "μπάτζιο"],
            image: "images/football-players/roberto-baggio.jpg",
            difficulty: "hard"
        },

        {
            answers: ["τζιανλουίτζι μπουφόν", "μπουφόν"],
            image: "images/football-players/gianluigi-buffon.jpg",
            difficulty: "easy"
        },

        {
            answers: ["μανουέλ νοϊερ", "νοϊερ"],
            image: "images/football-players/manuel-neuer.jpg",
            difficulty: "easy"
        },

        {
            answers: ["τσαβι σίμονς"],
            image: "images/football-players/xavi-simons.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ντέκλαν ράις", "ράις"],
            image: "images/football-players/declan-rice.jpg",
            difficulty: "medium"
        },

        {
            answers: ["μαρτιν όντεγκααρντ", "όντεγκααρντ"],
            image: "images/football-players/martin-odegaard.jpg",
            difficulty: "medium"
        },

        {
            answers: ["βίκτορ οσιμέν", "οσιμέν"],
            image: "images/football-players/victor-osimhen.jpg",
            difficulty: "medium"
        },

        {
            answers: ["λάουταρο μαρτίνες", "λάουταρο"],
            image: "images/football-players/lautaro-martinez.jpg",
            difficulty: "medium"
        },

        {
            answers: ["τζιανλουίτζι ντοναρούμα"],
            image: "images/football-players/gianluigi-donnarumma.jpg",
            difficulty: "medium"
        },

        {
            answers: ["σον χέουνγκ μιν", "σον"],
            image: "images/football-players/son-heung-min.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ριγιάντ μαχρέζ", "μαχρέζ"],
            image: "images/football-players/riyad-mahrez.jpg",
            difficulty: "medium"
        },

        {
            answers: ["σάντι καθόρλα"],
            image: "images/football-players/santi-cazorla.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ανχελ ντι μαρία", "ντι μαρία"],
            image: "images/football-players/angel-di-maria.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ντάβιντ βίγια", "βίγια"],
            image: "images/football-players/david-villa.jpg",
            difficulty: "medium"
        },

        {
            answers: ["φερνάντο τόρες", "τόρες"],
            image: "images/football-players/fernando-torres.jpg",
            difficulty: "medium"
        },

        {
            answers: ["γουέιν ρούνεϊ", "ρούνεϊ"],
            image: "images/football-players/wayne-rooney.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Ντέιβιντ Μπέκαμ", "μπέκαμ"],
            image: "images/football-players/david-beckham.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Ρίο Φέρντιναντ", "φέρντιναντ"],
            image: "images/football-players/rio-ferdinand.jpg",
            difficulty: "hard"
        },

        {
            answers: ["Κάρλες Πουγιόλ", "πουγιόλ"],
            image: "images/football-players/carles-puyol.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Ρομπέρτο Κάρλος"],
            image: "images/football-players/roberto-carlos.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Καφού"],
            image: "images/football-players/cafu.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Ρονάλντο Ναζάριο", "το φαινόμενο", "ρονάλντο ναζάριο"],
            image: "images/football-players/ronaldo-nazario.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Ριβάλντο"],
            image: "images/football-players/rivaldo.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Ρομάριο"],
            image: "images/football-players/romario.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Τιερί Ανρί", "ανρί"],
            image: "images/football-players/thierry-henry.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Ντένις Μπέργκαμπ", "μπέργκαμπ"],
            image: "images/football-players/dennis-bergkamp.jpg",
            difficulty: "hard"
        },

        {
            answers: ["Τζορτζ Μπεστ", "μπεστ"],
            image: "images/football-players/george-best.jpg",
            difficulty: "hard"
        },

        {
            answers: ["Λουίς Φίγκο", "φίγκο"],
            image: "images/football-players/luis-figo.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Μίκαελ Λάουντρουπ", "λάουντρουπ"],
            image: "images/football-players/michael-laudrup.jpg",
            difficulty: "hard"
        },

        {
            answers: ["Λόταρ Ματέους", "ματέους"],
            image: "images/football-players/lothar-matthaus.jpg",
            difficulty: "hard"
        }

    ],

    "basketball-clubs": [
        {
            answers: ["λόσ άντζελες λέικερς", "λέικερς"],
            image: "images/basketball-clubs/los-angeles-lakers.png",
            difficulty: "easy"
        },

        {
            answers: ["γκόλντεν στέιτ γουόριορς", "γουόριορς"],
            image: "images/basketball-clubs/golden-state-warriors.png",
            difficulty: "easy"
        },

        {
            answers: ["μπόστον σέλτικς", "σέλτικς"],
            image: "images/basketball-clubs/boston-celtics.png",
            difficulty: "easy"
        },

        {
            answers: ["σιικάγο μπουλς", "μπουλς"],
            image: "images/basketball-clubs/chicago-bulls.png",
            difficulty: "easy"
        },

        {
            answers: ["μιλγουόκι μπακς", "μπακς"],
            image: "images/basketball-clubs/milwaukee-bucks.png",
            difficulty: "easy"
        },

        {
            answers: ["κλίβελαντ καβαλίερς", "καβαλίερς"],
            image: "images/basketball-clubs/cleveland-cavaliers.png",
            difficulty: "medium"
        },

        {
            answers: ["ντένβερ νάγκετς", "νάγκετς"],
            image: "images/basketball-clubs/denver-nuggets.png",
            difficulty: "easy"
        },

        {
            answers: ["οκλαχόμα σίτι θάντερ", "θάντερ"],
            image: "images/basketball-clubs/oklahoma-city-thunder.png",
            difficulty: "medium"
        },

        {
            answers: ["φοίνιξ σανς", "σανς"],
            image: "images/basketball-clubs/phoenix-suns.png",
            difficulty: "medium"
        },

        {
            answers: ["ντάλας μάβερικς", "μάβερικς"],
            image: "images/basketball-clubs/dallas-mavericks.png",
            difficulty: "easy"
        },

        {
            answers: ["μεμφις γκρίζλις", "γκρίζλις"],
            image: "images/basketball-clubs/memphis-grizzlies.png",
            difficulty: "medium"
        },

        {
            answers: ["μιάμι χιτ", "χιτ"],
            image: "images/basketball-clubs/miami-heat.png",
            difficulty: "easy"
        },

        {
            answers: ["φιλαδέλφεια 76ερς", "76ερς", "σίξερς"],
            image: "images/basketball-clubs/philadelphia-76ers.png",
            difficulty: "medium"
        },

        {
            answers: ["νιου γιορκ νικς", "νικς"],
            image: "images/basketball-clubs/new-york-knicks.png",
            difficulty: "easy"
        },

        {
            answers: ["σαν αντόνιο σπερς", "σπερς"],
            image: "images/basketball-clubs/san-antonio-spurs.png",
            difficulty: "easy"
        },

        {
            answers: ["σακραμέντο κινγκς", "κινγκς"],
            image: "images/basketball-clubs/sacramento-kings.png",
            difficulty: "medium"
        },

        {
            answers: ["πορτλαντ τρέιλ μπλέιζερς", "μπλέιζερς"],
            image: "images/basketball-clubs/portland-trail-blazers.png",
            difficulty: "medium"
        },

        {
            answers: ["ιντιάνα πέισερς", "πέισερς"],
            image: "images/basketball-clubs/indiana-pacers.png",
            difficulty: "medium"
        },

        {
            answers: ["ορλάντο μάτζικ", "μάτζικ"],
            image: "images/basketball-clubs/orlando-magic.png",
            difficulty: "medium"
        },

        {
            answers: ["τορόντο ράπτορς", "ράπτορς"],
            image: "images/basketball-clubs/toronto-raptors.png",
            difficulty: "medium"
        },

        {
            answers: ["ολυμπιακός"],
            image: "images/basketball-clubs/olympiacos.png",
            difficulty: "easy"
        },

        {
            answers: ["παναθηναϊκός"],
            image: "images/basketball-clubs/panathinaikos.png",
            difficulty: "easy"
        },

        {
            answers: ["ρεάλ μαδρίτης"],
            image: "images/basketball-clubs/real-madrid.png",
            difficulty: "easy"
        },

        {
            answers: ["μπαρτσελόνα"],
            image: "images/basketball-clubs/barcelona.png",
            difficulty: "easy"
        },

        {
            answers: ["φενέρμπαχτσε"],
            image: "images/basketball-clubs/fenerbahce.png",
            difficulty: "easy"
        },

        {
            answers: ["ανατολού εφές", "εφές"],
            image: "images/basketball-clubs/anadolu-efes.png",
            difficulty: "medium"
        },

        {
            answers: ["μακάμπι τελ αβίβ", "μακάμπι"],
            image: "images/basketball-clubs/maccabi-tel-aviv.png",
            difficulty: "medium"
        },

        {
            answers: ["μονακό"],
            image: "images/basketball-clubs/monaco.png",
            difficulty: "medium"
        },

        {
            answers: ["παρτίζαν"],
            image: "images/basketball-clubs/partizan.png",
            difficulty: "medium"
        },

        {
            answers: ["ερυθρός αστέρας"],
            image: "images/basketball-clubs/crvena-zvezda.png",
            difficulty: "medium"
        },

        {
            answers: ["ζάλγκιρις"],
            image: "images/basketball-clubs/zalgiris-kaunas.png",
            difficulty: "medium"
        },

        {
            answers: ["βίρτους μπολόνια"],
            image: "images/basketball-clubs/virtus-bologna.png",
            difficulty: "medium"
        },

        {
            answers: ["μπασκόνια"],
            image: "images/basketball-clubs/baskonia.png",
            difficulty: "medium"
        },

        {
            answers: ["βιλερμπάν"],
            image: "images/basketball-clubs/asvel.png",
            difficulty: "hard"
        },

        {
            answers: ["άλμπα βερολίνου"],
            image: "images/basketball-clubs/alba-berlin.png",
            difficulty: "hard"
        },

        {
            answers: ["πάρις μπάσκετ"],
            image: "images/basketball-clubs/paris-basketball.png",
            difficulty: "medium"
        },

        {
            answers: ["βαλένθια"],
            image: "images/basketball-clubs/valencia-basket.png",
            difficulty: "medium"
        },

        {
            answers: ["μπάγερν μονάχου"],
            image: "images/basketball-clubs/bayern-munich.png",
            difficulty: "medium"
        },

        {
            answers: ["αρμάνι μιλάνο"],
            image: "images/basketball-clubs/olimpia-milano.png",
            difficulty: "medium"
        },

        {
            answers: ["ντουμπάι μπάσκετ"],
            image: "images/basketball-clubs/dubai-basketball.png",
            difficulty: "hard"
        },

        {
            answers: ["τσσκα μόσχας", "τσσκα", "cska moscow", "cska"],
            image: "images/basketball-clubs/cska-moscow.png",
            difficulty: "hard"
        },

        {
            answers: ["χιμκί", "khimki"],
            image: "images/basketball-clubs/khimki.png",
            difficulty: "hard"
        },

        {
            answers: ["ούνικς καζάν", "ούνικς", "unics kazan", "unics"],
            image: "images/basketball-clubs/unics-kazan.png",
            difficulty: "hard"
        },

        {
            answers: ["ζενίτ αγίας πετρούπολης", "ζενίτ", "zenit st petersburg", "zenit"],
            image: "images/basketball-clubs/zenit-st-petersburg.png",
            difficulty: "hard"
        },

        {
            answers: ["λοκομοτίβ κουμπάν", "lokomotiv kuban"],
            image: "images/basketball-clubs/lokomotiv-kuban.png",
            difficulty: "hard"
        },

        {
            answers: ["μοντεπάσκι σιένα", "σιένα", "montepaschi siena"],
            image: "images/basketball-clubs/montepaschi-siena.png",
            difficulty: "hard"
        },

        {
            answers: ["μπενετόν τρεβίζο", "μπενετόν", "benetton treviso"],
            image: "images/basketball-clubs/benetton-treviso.png",
            difficulty: "hard"
        },

        {
            answers: ["φορτιτούντο μπολόνια", "fortitudo bologna"],
            image: "images/basketball-clubs/fortitudo-bologna.png",
            difficulty: "hard"
        },

        {
            answers: ["τσιμπόνα ζάγκρεμπ", "τσιμπόνα", "cibona zagreb", "cibona"],
            image: "images/basketball-clubs/cibona-zagreb.png",
            difficulty: "hard"
        },

        {
            answers: ["ρίτας βίλνιους", "ρίτας", "lietuvos rytas", "rytas vilnius"],
            image: "images/basketball-clubs/rytas-vilnius.png",
            difficulty: "hard"
        },

        {
            answers: ["ντιναμό σάσαρι", "dinamo sassari", "sassari"],
            image: "images/basketball-clubs/dinamo-sassari.png",
            difficulty: "hard"
        },

        {
            answers: ["μπουντούτσνοστ", "buducnost"],
            image: "images/basketball-clubs/buducnost.png",
            difficulty: "hard"
        },

        {
            answers: ["γαλατασαράι"],
            image: "images/basketball-clubs/galatasaray.png",
            difficulty: "hard"
        },

        {
            answers: ["νταρουσάφακα", "darussafaka"],
            image: "images/basketball-clubs/darussafaka.png",
            difficulty: "hard"
        },

        {
            answers: ["ουνικάχα μαλάγα", "ουνικάχα", "unicaja malaga", "unicaja"],
            image: "images/basketball-clubs/unicaja-malaga.png",
            difficulty: "hard"
        },

        {
            answers: ["χοβεντούτ μπανταλόνα", "χοβεντούτ", "joventut badalona", "joventut"],
            image: "images/basketball-clubs/joventut-badalona.png",
            difficulty: "hard"
        },

        {
            answers: ["μπρόουζ μπάμπεργκ", "μπάμπεργκ", "brose bamberg"],
            image: "images/basketball-clubs/brose-bamberg.png",
            difficulty: "hard"
        },

        {
            answers: ["λιμόζ", "limoges csp", "limoges"],
            image: "images/basketball-clubs/limoges-csp.png",
            difficulty: "hard"
        },

        {
            answers: ["κκ σπλιτ", "kk split", "σπλιτ"],
            image: "images/basketball-clubs/kk-split.png",
            difficulty: "hard"
        },

        {
            answers: ["ντιναμό τιφλίδας", "dinamo tbilisi"],
            image: "images/basketball-clubs/dinamo-tbilisi.png",
            difficulty: "hard"
        }

        ],

    actors: [

        {
            answers: ["λεονάρντο ντι κάπριο", "ντι κάπριο"],
            image: "images/actors/leonardo-dicaprio.jpg",
            difficulty: "easy"
        },

        {
            answers: ["τομ κρουζ"],
            image: "images/actors/tom-cruise.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ντουέιν τζόνσον", "the rock", "ροκ"],
            image: "images/actors/dwayne-johnson.jpg",
            difficulty: "easy"
        },

        {
            answers: ["μπραντ πιτ"],
            image: "images/actors/brad-pitt.jpg",
            difficulty: "easy"
        },

        {
            answers: ["τζόνι ντεπ"],
            image: "images/actors/johnny-depp.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ρόμπερτ ντάουνι τζούνιορ"],
            image: "images/actors/robert-downey-jr.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κιάνου ριβς"],
            image: "images/actors/keanu-reeves.jpg",
            difficulty: "easy"
        },

        {
            answers: ["μόργκαν φρίμαν"],
            image: "images/actors/morgan-freeman.jpg",
            difficulty: "easy"
        },

        {
            answers: ["γουίλ σμιθ"],
            image: "images/actors/will-smith.jpg",
            difficulty: "easy"
        },

        {
            answers: ["τζιμ κάρεϊ"],
            image: "images/actors/jim-carrey.jpg",
            difficulty: "easy"
        },

        {
            answers: ["χιου τζάκμαν"],
            image: "images/actors/hugh-jackman.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κρίστιαν μπέιλ"],
            image: "images/actors/christian-bale.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ματ ντέιμον"],
            image: "images/actors/matt-damon.jpg",
            difficulty: "medium"
        },

        {
            answers: ["μπεν άφλεκ"],
            image: "images/actors/ben-affleck.jpg",
            difficulty: "medium"
        },

        {
            answers: ["κρις εβανς"],
            image: "images/actors/chris-evans.jpg",
            difficulty: "easy"
        },

        {
            answers: ["κρις χέμσγουορθ"],
            image: "images/actors/chris-hemsworth.jpg",
            difficulty: "easy"
        },

        {
            answers: ["μαρκ ράφαλο"],
            image: "images/actors/mark-ruffalo.jpg",
            difficulty: "medium"
        },

        {
            answers: ["σάμιουελ λ. τζάκσον"],
            image: "images/actors/samuel-l-jackson.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Τομ Χανκς"],
            image: "images/actors/tom-hanks.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Αλ Πατσίνο"],
            image: "images/actors/al-pacino.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Ρόμπερτ Ντε Νίρο"],
            image: "images/actors/robert-de-niro.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Άντονι Χόπκινς"],
            image: "images/actors/anthony-hopkins.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Χοακίν Φίνιξ"],
            image: "images/actors/joaquin-phoenix.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Κίλιαν Μέρφι"],
            image: "images/actors/cillian-murphy.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Ράιαν Γκόσλινγκ"],
            image: "images/actors/ryan-gosling.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Ράιαν Ρέινολντς"],
            image: "images/actors/ryan-reynolds.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Όρλαντο Μπλουμ"],
            image: "images/actors/orlando-bloom.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Ιαν ΜακΚέλεν"],
            image: "images/actors/ian-mckellen.jpg",
            difficulty: "hard"
        },

        {
            answers: ["Βίγκο Μόρτενσεν"],
            image: "images/actors/viggo-mortensen.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Ελάιτζα Γουντ"],
            image: "images/actors/elijah-wood.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Άντριου Γκάρφιλντ"],
            image: "images/actors/andrew-garfield.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Τόμπι Μαγκουάιρ"],
            image: "images/actors/tobey-maguire.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Τομ Χόλαντ"],
            image: "images/actors/tom-holland.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Χένρι Κάβιλ"],
            image: "images/actors/henry-cavill.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Τζέισον Στέιθαμ"],
            image: "images/actors/jason-statham.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Άρνολντ Σβαρτσενέγκερ"],
            image: "images/actors/arnold-schwarzenegger.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Σιλβέστερ Σταλόνε"],
            image: "images/actors/sylvester-stallone.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Τζάκι Τσαν"],
            image: "images/actors/jackie-chan.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Τζετ Λι"],
            image: "images/actors/jet-li.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Νίκολας Κέιτζ"],
            image: "images/actors/nicolas-cage.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ρασελ κρόου"],
            image: "images/actors/russell-crowe.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Έντουαρντ Νόρτον"],
            image: "images/actors/edward-norton.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Ρέιφ Φάινς"],
            image: "images/actors/ralph-fiennes.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Γκάρι Όλντμαν"],
            image: "images/actors/gary-oldman.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Ντάνιελ Ράντκλιφ"],
            image: "images/actors/daniel-radcliffe.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Ρούπερτ Γκριντ"],
            image: "images/actors/rupert-grint.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Έμα Γουότσον"],
            image: "images/actors/emma-watson.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Έμα Στόουν"],
            image: "images/actors/emma-stone.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Σκάρλετ Γιόχανσον"],
            image: "images/actors/scarlett-johansson.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Τζένιφερ Λόρενς"],
            image: "images/actors/jennifer-lawrence.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Μάργκοτ Ρόμπι"],
            image: "images/actors/margot-robbie.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Γκαλ Γκαντότ"],
            image: "images/actors/gal-gadot.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Νάταλι Πόρτμαν"],
            image: "images/actors/natalie-portman.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Αν Χάθαγουεϊ"],
            image: "images/actors/anne-hathaway.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Σαρλίζ Θερόν"],
            image: "images/actors/charlize-theron.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Νικόλ Κίντμαν"],
            image: "images/actors/nicole-kidman.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Κέιτ Γουίνσλετ"],
            image: "images/actors/kate-winslet.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Μέριλ Στριπ"],
            image: "images/actors/meryl-streep.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Τζούλια Ρόμπερτς"],
            image: "images/actors/julia-roberts.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Σάντρα Μπούλοκ"],
            image: "images/actors/sandra-bullock.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Ζεντάγια"],
            image: "images/actors/zendaya.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Φλόρενς Πιου"],
            image: "images/actors/florence-pugh.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Μίλι Μπόμπι Μπράουν"],
            image: "images/actors/millie-bobby-brown.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Άνια Τέιλορ Τζόι"],
            image: "images/actors/anya-taylor-joy.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Πέδρο Πασκάλ"],
            image: "images/actors/pedro-pascal.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Όσκαρ Άιζακ"],
            image: "images/actors/oscar-isaac.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Άνταμ Ντράιβερ"],
            image: "images/actors/adam-driver.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Μπένεντικτ Κάμπερμπατς"],
            image: "images/actors/benedict-cumberbatch.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Μάικλ Φασμπέντερ"],
            image: "images/actors/michael-fassbender.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Ίντρις Έλμπα"],
            image: "images/actors/idris-elba.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Ντάνζελ Ουάσινγκτον"],
            image: "images/actors/denzel-washington.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Τζέιμι Φοξ"],
            image: "images/actors/jamie-foxx.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Χάρισον Φορντ"],
            image: "images/actors/harrison-ford.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Γιούαν ΜακΓκρέγκορ"],
            image: "images/actors/ewan-mcgregor.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Λίαμ Νίσον"],
            image: "images/actors/liam-neeson.jpg",
            difficulty: "easy"
        },

        {
            answers: ["Τιμοτέ Σαλαμέ"],
            image: "images/actors/timothee-chalamet.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Όστιν Μπάτλερ"],
            image: "images/actors/austin-butler.jpg",
            difficulty: "medium"
        },

        {
            answers: ["Γκλεν Πάουελ"],
            image: "images/actors/glen-powell.jpg",
            difficulty: "hard"
        },

        {
            answers: ["Πολ Μέσκαλ"],
            image: "images/actors/paul-mescal.jpg",
            difficulty: "hard"
        },

        {
            answers: ["Κόλιν Φάρελ"],
            image: "images/actors/colin-farrell.jpg",
            difficulty: "medium"
        }

    ],

    "famous-cars": [

        {
            answers: ["mercedes amg gt black series", "amg gt black series"],
            image: "images/famous-cars/mercedes-amg-gt-black-series.jpg",
            difficulty: "easy"
        },

        {
            answers: ["bugatti chiron"],
            image: "images/famous-cars/bugatti-chiron.jpg",
            difficulty: "easy"
        },

        {
            answers: ["bugatti bolide"],
            image: "images/famous-cars/bugatti-bolide.jpg",
            difficulty: "easy"
        },

        {
            answers: ["koenigsegg jesko", "jesko"],
            image: "images/famous-cars/koenigsegg-jesko.jpg",
            difficulty: "easy"
        },

        {
            answers: ["koenigsegg jesko absolut", "jesko absolut"],
            image: "images/famous-cars/koenigsegg-jesko-absolut.jpg",
            difficulty: "medium"
        },

        {
            answers: ["koenigsegg regera"],
            image: "images/famous-cars/koenigsegg-regera.jpg",
            difficulty: "medium"
        },

        {
            answers: ["koenigsegg agera rs"],
            image: "images/famous-cars/koenigsegg-agera-rs.jpg",
            difficulty: "medium"
        },

        {
            answers: ["hennessey venom f5"],
            image: "images/famous-cars/hennessey-venom-f5.jpg",
            difficulty: "medium"
        },

        {
            answers: ["rimac nevera"],
            image: "images/famous-cars/rimac-nevera.jpg",
            difficulty: "medium"
        },

        {
            answers: ["lamborghini aventador"],
            image: "images/famous-cars/lamborghini-aventador.jpg",
            difficulty: "easy"
        },

        {
            answers: ["lamborghini revuelto"],
            image: "images/famous-cars/lamborghini-revuelto.jpg",
            difficulty: "easy"
        },

        {
            answers: ["lamborghini veneno"],
            image: "images/famous-cars/lamborghini-veneno.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ferrari laferrari"],
            image: "images/famous-cars/ferrari-laferrari.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ferrari f40"],
            image: "images/famous-cars/ferrari-f40.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ferrari enzo"],
            image: "images/famous-cars/ferrari-enzo.jpg",
            difficulty: "easy"
        },

        {
            answers: ["mclaren p1"],
            image: "images/famous-cars/mclaren-p1.jpg",
            difficulty: "easy"
        },

        {
            answers: ["mclaren senna"],
            image: "images/famous-cars/mclaren-senna.jpg",
            difficulty: "medium"
        },

        {
            answers: ["mclaren speedtail"],
            image: "images/famous-cars/mclaren-speedtail.jpg",
            difficulty: "medium"
        },

        {
            answers: ["aston martin valkyrie"],
            image: "images/famous-cars/aston-martin-valkyrie.jpg",
            difficulty: "medium"
        },

        {
            answers: ["aston martin valkyrie amr pro"],
            image: "images/famous-cars/aston-martin-valkyrie-amr-pro.jpg",
            difficulty: "hard"
        },

        {
            answers: ["pagani huayra bc"],
            image: "images/famous-cars/pagani-huayra-bc.jpg",
            difficulty: "medium"
        },

        {
            answers: ["pagani zonda r"],
            image: "images/famous-cars/pagani-zonda-r.jpg",
            difficulty: "medium"
        },

        {
            answers: ["apollo intensa emozione", "apollo ie"],
            image: "images/famous-cars/apollo-intensa-emozione.jpg",
            difficulty: "hard"
        },

        {
            answers: ["porsche 911 gt3 rs"],
            image: "images/famous-cars/porsche-911-gt3-rs.jpg",
            difficulty: "easy"
        },

        {
            answers: ["porsche taycan turbo gt"],
            image: "images/famous-cars/porsche-taycan-turbo-gt.jpg",
            difficulty: "medium"
        },

        {
            answers: ["porsche 918 spyder"],
            image: "images/famous-cars/porsche-918-spyder.jpg",
            difficulty: "easy"
        },

        {
            answers: ["bugatti veyron"],
            image: "images/famous-cars/bugatti-veyron.jpg",
            difficulty: "easy"
        },

        {
            answers: ["bugatti mistral"],
            image: "images/famous-cars/bugatti-mistral.jpg",
            difficulty: "medium"
        },

        {
            answers: ["chevrolet corvette zr1x"],
            image: "images/famous-cars/chevrolet-corvette-zr1x.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ford gt"],
            image: "images/famous-cars/ford-gt.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ford mustang shelby gt500"],
            image: "images/famous-cars/ford-mustang-shelby-gt500.jpg",
            difficulty: "medium"
        },

        {
            answers: ["nissan gt-r", "nissan gtr", "gt-r", "gtr"],
            image: "images/famous-cars/nissan-gtr-r35.jpg",
            difficulty: "easy"
        },

        {
            answers: ["toyota supra mk4", "toyota supra", "supra mk4"],
            image: "images/famous-cars/toyota-supra-mk4.jpg",
            difficulty: "easy"
        },

        {
            answers: ["mazda rx-7", "mazda rx7"],
            image: "images/famous-cars/mazda-rx7.jpg",
            difficulty: "medium"
        },

        {
            answers: ["dodge challenger srt demon 170"],
            image: "images/famous-cars/dodge-challenger-srt-demon-170.jpg",
            difficulty: "medium"
        },

        {
            answers: ["bmw m4 competition"],
            image: "images/famous-cars/bmw-m4-competition.jpg",
            difficulty: "easy"
        },

        {
            answers: ["bmw m5 cs"],
            image: "images/famous-cars/bmw-m5-cs.jpg",
            difficulty: "easy"
        },

        {
            answers: ["mercedes amg one"],
            image: "images/famous-cars/mercedes-amg-one.jpg",
            difficulty: "easy"
        },

        {
            answers: ["mercedes benz 300 slr uhlenhaut coupe", "300 slr uhlenhaut coupe"],
            image: "images/famous-cars/mercedes-300-slr-uhlenhaut-coupe.jpg",
            difficulty: "hard"
        },

        {
            answers: ["mercedes vision avtr", "vision avtr"],
            image: "images/famous-cars/mercedes-vision-avtr.jpg",
            difficulty: "medium"
        },

        {
            answers: ["mercedes slr mclaren"],
            image: "images/famous-cars/mercedes-slr-mclaren.jpg",
            difficulty: "medium"
        },

        {
            answers: ["mercedes amg gt r"],
            image: "images/famous-cars/mercedes-amg-gt-r.jpg",
            difficulty: "medium"
        },

        {
            answers: ["mercedes g63 amg", "g63 amg", "g wagon", "g-class"],
            image: "images/famous-cars/mercedes-g63-amg.jpg",
            difficulty: "easy"
        },

        {
            answers: ["mercedes cls 63 amg"],
            image: "images/famous-cars/mercedes-cls63-amg.jpg",
            difficulty: "hard"
        },

        {
            answers: ["bmw m3 gtr"],
            image: "images/famous-cars/bmw-m3-gtr.jpg",
            difficulty: "easy"
        },

        {
            answers: ["bmw i8"],
            image: "images/famous-cars/bmw-i8.jpg",
            difficulty: "easy"
        },

        {
            answers: ["bmw m8 competition"],
            image: "images/famous-cars/bmw-m8-competition.jpg",
            difficulty: "medium"
        },

        {
            answers: ["audi r8"],
            image: "images/famous-cars/audi-r8.jpg",
            difficulty: "easy"
        },

        {
            answers: ["audi rs6 avant"],
            image: "images/famous-cars/audi-rs6-avant.jpg",
            difficulty: "medium"
        },

        {
            answers: ["audi rs7"],
            image: "images/famous-cars/audi-rs7.jpg",
            difficulty: "medium"
        },

        {
            answers: ["lexus lfa"],
            image: "images/famous-cars/lexus-lfa.jpg",
            difficulty: "medium"
        },

        {
            answers: ["honda nsx"],
            image: "images/famous-cars/honda-nsx.jpg",
            difficulty: "medium"
        },

        {
            answers: ["mitsubishi lancer evolution x", "lancer evo x", "evo x"],
            image: "images/famous-cars/mitsubishi-lancer-evo-x.jpg",
            difficulty: "medium"
        },

        {
            answers: ["subaru impreza wrx sti", "wrx sti"],
            image: "images/famous-cars/subaru-impreza-wrx-sti.jpg",
            difficulty: "medium"
        },

        {
            answers: ["nissan silvia s15"],
            image: "images/famous-cars/nissan-silvia-s15.jpg",
            difficulty: "hard"
        },

        {
            answers: ["toyota ae86"],
            image: "images/famous-cars/toyota-ae86.jpg",
            difficulty: "hard"
        },

        {
            answers: ["dodge viper acr"],
            image: "images/famous-cars/dodge-viper-acr.jpg",
            difficulty: "medium"
        },

        {
            answers: ["chevrolet camaro zl1"],
            image: "images/famous-cars/chevrolet-camaro-zl1.jpg",
            difficulty: "medium"
        },

        {
            answers: ["chevrolet corvette c8"],
            image: "images/famous-cars/chevrolet-corvette-c8.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ford mustang dark horse"],
            image: "images/famous-cars/ford-mustang-dark-horse.jpg",
            difficulty: "medium"
        },

        {
            answers: ["aston martin db5"],
            image: "images/famous-cars/aston-martin-db5.jpg",
            difficulty: "medium"
        },

        {
            answers: ["aston martin dbs superleggera"],
            image: "images/famous-cars/aston-martin-dbs-superleggera.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ferrari sf90 stradale"],
            image: "images/famous-cars/ferrari-sf90-stradale.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ferrari 812 superfast"],
            image: "images/famous-cars/ferrari-812-superfast.jpg",
            difficulty: "medium"
        },

        {
            answers: ["lamborghini huracan"],
            image: "images/famous-cars/lamborghini-huracan.jpg",
            difficulty: "easy"
        },

        {
            answers: ["lamborghini sian"],
            image: "images/famous-cars/lamborghini-sian.jpg",
            difficulty: "medium"
        },

        {
            answers: ["pagani utopia"],
            image: "images/famous-cars/pagani-utopia.jpg",
            difficulty: "hard"
        },

        {
            answers: ["koenigsegg cc850"],
            image: "images/famous-cars/koenigsegg-cc850.jpg",
            difficulty: "hard"
        },

        {
            answers: ["koenigsegg gemera"],
            image: "images/famous-cars/koenigsegg-gemera.jpg",
            difficulty: "medium"
        },

        {
            answers: ["bugatti tourbillon"],
            image: "images/famous-cars/bugatti-tourbillon.jpg",
            difficulty: "easy"
        },

        {
            answers: ["mclaren 720s"],
            image: "images/famous-cars/mclaren-720s.jpg",
            difficulty: "easy"
        },

        {
            answers: ["mclaren 765lt"],
            image: "images/famous-cars/mclaren-765lt.jpg",
            difficulty: "medium"
        },

        {
            answers: ["rolls royce phantom"],
            image: "images/famous-cars/rolls-royce-phantom.jpg",
            difficulty: "easy"
        },

        {
            answers: ["rolls royce ghost"],
            image: "images/famous-cars/rolls-royce-ghost.jpg",
            difficulty: "easy"
        },

        {
            answers: ["bentley continental gt"],
            image: "images/famous-cars/bentley-continental-gt.jpg",
            difficulty: "medium"
        },

        {
            answers: ["porsche carrera gt"],
            image: "images/famous-cars/porsche-carrera-gt.jpg",
            difficulty: "medium"
        },

        {
            answers: ["porsche 959"],
            image: "images/famous-cars/porsche-959.jpg",
            difficulty: "hard"
        },

        {
            answers: ["jaguar xj220"],
            image: "images/famous-cars/jaguar-xj220.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ssc tuatara"],
            image: "images/famous-cars/ssc-tuatara.jpg",
            difficulty: "hard"
        },

        {
            answers: ["de tomaso p72"],
            image: "images/famous-cars/de-tomaso-p72.jpg",
            difficulty: "hard"
        }

    ],

"f1-drivers": [

        {
            answers: ["lewis hamilton", "hamilton"],
            image: "images/f1-drivers/lewis-hamilton.jpg",
            difficulty: "easy"
        },

        {
            answers: ["max verstappen", "verstappen"],
            image: "images/f1-drivers/max-verstappen.jpg",
            difficulty: "easy"
        },

        {
            answers: ["charles leclerc", "leclerc"],
            image: "images/f1-drivers/charles-leclerc.jpg",
            difficulty: "easy"
        },

        {
            answers: ["lando norris", "norris"],
            image: "images/f1-drivers/lando-norris.jpg",
            difficulty: "easy"
        },

        {
            answers: ["oscar piastri", "piastri"],
            image: "images/f1-drivers/oscar-piastri.jpg",
            difficulty: "easy"
        },

        {
            answers: ["george russell", "russell"],
            image: "images/f1-drivers/george-russell.jpg",
            difficulty: "easy"
        },

        {
            answers: ["carlos sainz", "sainz"],
            image: "images/f1-drivers/carlos-sainz.jpg",
            difficulty: "easy"
        },

        {
            answers: ["fernando alonso", "alonso"],
            image: "images/f1-drivers/fernando-alonso.jpg",
            difficulty: "easy"
        },

        {
            answers: ["sebastian vettel", "vettel"],
            image: "images/f1-drivers/sebastian-vettel.jpg",
            difficulty: "easy"
        },

        {
            answers: ["michael schumacher", "schumacher"],
            image: "images/f1-drivers/michael-schumacher.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ayrton senna", "senna"],
            image: "images/f1-drivers/ayrton-senna.jpg",
            difficulty: "easy"
        },

        {
            answers: ["alain prost", "prost"],
            image: "images/f1-drivers/alain-prost.jpg",
            difficulty: "medium"
        },

        {
            answers: ["niki lauda", "lauda"],
            image: "images/f1-drivers/niki-lauda.jpg",
            difficulty: "medium"
        },

        {
            answers: ["jenson button", "button"],
            image: "images/f1-drivers/jenson-button.jpg",
            difficulty: "medium"
        },

        {
            answers: ["kimi raikkonen", "raikkonen", "kimi"],
            image: "images/f1-drivers/kimi-raikkonen.jpg",
            difficulty: "easy"
        },

        {
            answers: ["nico rosberg", "rosberg"],
            image: "images/f1-drivers/nico-rosberg.jpg",
            difficulty: "easy"
        },

        {
            answers: ["daniel ricciardo", "ricciardo"],
            image: "images/f1-drivers/daniel-ricciardo.jpg",
            difficulty: "easy"
        },

        {
            answers: ["sergio perez", "perez", "checo perez"],
            image: "images/f1-drivers/sergio-perez.jpg",
            difficulty: "easy"
        },

        {
            answers: ["valtteri bottas", "bottas"],
            image: "images/f1-drivers/valtteri-bottas.jpg",
            difficulty: "medium"
        },

        {
            answers: ["felipe massa", "massa"],
            image: "images/f1-drivers/felipe-massa.jpg",
            difficulty: "medium"
        },

        {
            answers: ["jacques villeneuve", "villeneuve"],
            image: "images/f1-drivers/jacques-villeneuve.jpg",
            difficulty: "hard"
        },

        {
            answers: ["mika hakkinen", "hakkinen"],
            image: "images/f1-drivers/mika-hakkinen.jpg",
            difficulty: "medium"
        },

        {
            answers: ["damon hill", "hill"],
            image: "images/f1-drivers/damon-hill.jpg",
            difficulty: "hard"
        },

        {
            answers: ["nelson piquet", "piquet"],
            image: "images/f1-drivers/nelson-piquet.jpg",
            difficulty: "hard"
        },

        {
            answers: ["nigel mansell", "mansell"],
            image: "images/f1-drivers/nigel-mansell.jpg",
            difficulty: "hard"
        },

        {
            answers: ["emerson fittipaldi", "fittipaldi"],
            image: "images/f1-drivers/emerson-fittipaldi.jpg",
            difficulty: "hard"
        },

        {
            answers: ["jackie stewart", "stewart"],
            image: "images/f1-drivers/jackie-stewart.jpg",
            difficulty: "hard"
        },

        {
            answers: ["jim clark", "clark"],
            image: "images/f1-drivers/jim-clark.jpg",
            difficulty: "hard"
        },

        {
            answers: ["gilles villeneuve"],
            image: "images/f1-drivers/gilles-villeneuve.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ronnie peterson"],
            image: "images/f1-drivers/ronnie-peterson.jpg",
            difficulty: "hard"
        },

        {
            answers: ["mark webber"],
            image: "images/f1-drivers/mark-webber.jpg",
            difficulty: "medium"
        },

        {
            answers: ["rubens barrichello"],
            image: "images/f1-drivers/rubens-barrichello.jpg",
            difficulty: "medium"
        },

        {
            answers: ["david coulthard"],
            image: "images/f1-drivers/david-coulthard.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ralf schumacher"],
            image: "images/f1-drivers/ralf-schumacher.jpg",
            difficulty: "hard"
        },

        {
            answers: ["juan pablo montoya", "montoya"],
            image: "images/f1-drivers/juan-pablo-montoya.jpg",
            difficulty: "medium"
        },

        {
            answers: ["jarno trulli"],
            image: "images/f1-drivers/jarno-trulli.jpg",
            difficulty: "hard"
        },

        {
            answers: ["giancarlo fisichella"],
            image: "images/f1-drivers/giancarlo-fisichella.jpg",
            difficulty: "hard"
        },

        {
            answers: ["heikki kovalainen"],
            image: "images/f1-drivers/heikki-kovalainen.jpg",
            difficulty: "hard"
        },

        {
            answers: ["yuki tsunoda"],
            image: "images/f1-drivers/yuki-tsunoda.jpg",
            difficulty: "medium"
        },

        {
            answers: ["alex albon"],
            image: "images/f1-drivers/alex-albon.jpg",
            difficulty: "medium"
        },

        {
            answers: ["liam lawson"],
            image: "images/f1-drivers/liam-lawson.jpg",
            difficulty: "medium"
        },

        {
            answers: ["andrea kimi antonelli", "kimi antonelli", "antonelli"],
            image: "images/f1-drivers/kimi-antonelli.jpg",
            difficulty: "medium"
        },

        {
            answers: ["oliver bearman", "bearman"],
            image: "images/f1-drivers/oliver-bearman.jpg",
            difficulty: "medium"
        },

        {
            answers: ["gabriel bortoleto", "bortoleto"],
            image: "images/f1-drivers/gabriel-bortoleto.jpg",
            difficulty: "hard"
        },

        {
            answers: ["isack hadjar", "hadjar"],
            image: "images/f1-drivers/isack-hadjar.jpg",
            difficulty: "hard"
        },

        {
            answers: ["esteban ocon", "ocon"],
            image: "images/f1-drivers/esteban-ocon.jpg",
            difficulty: "medium"
        },

        {
            answers: ["pierre gasly", "gasly"],
            image: "images/f1-drivers/pierre-gasly.jpg",
            difficulty: "medium"
        },

        {
            answers: ["lance stroll", "stroll"],
            image: "images/f1-drivers/lance-stroll.jpg",
            difficulty: "medium"
        },

        {
            answers: ["nico hulkenberg", "hulkenberg"],
            image: "images/f1-drivers/nico-hulkenberg.jpg",
            difficulty: "medium"
        },

        {
            answers: ["kevin magnussen", "magnussen"],
            image: "images/f1-drivers/kevin-magnussen.jpg",
            difficulty: "medium"
        },

        {
            answers: ["romain grosjean", "grosjean"],
            image: "images/f1-drivers/romain-grosjean.jpg",
            difficulty: "hard"
        },

        {
            answers: ["pastor maldonado", "maldonado"],
            image: "images/f1-drivers/pastor-maldonado.jpg",
            difficulty: "hard"
        },

        {
            answers: ["jean alesi", "alesi"],
            image: "images/f1-drivers/jean-alesi.jpg",
            difficulty: "hard"
        },

        {
            answers: ["gerhard berger", "berger"],
            image: "images/f1-drivers/gerhard-berger.jpg",
            difficulty: "hard"
        },

        {
            answers: ["rene arnoux"],
            image: "images/f1-drivers/rene-arnoux.jpg",
            difficulty: "hard"
        },

        {
            answers: ["carlos reutemann"],
            image: "images/f1-drivers/carlos-reutemann.jpg",
            difficulty: "hard"
        },

        {
            answers: ["john surtees"],
            image: "images/f1-drivers/john-surtees.jpg",
            difficulty: "hard"
        },

        {
            answers: ["graham hill"],
            image: "images/f1-drivers/graham-hill.jpg",
            difficulty: "hard"
        },

        {
            answers: ["jochen rindt"],
            image: "images/f1-drivers/jochen-rindt.jpg",
            difficulty: "hard"
        },

        {
            answers: ["jody schekter"],
            image: "images/f1-drivers/jody-scheckter.jpg",
            difficulty: "hard"
        },

        {
            answers: ["alan jones"],
            image: "images/f1-drivers/alan-jones.jpg",
            difficulty: "hard"
        },

        {
            answers: ["keke rosberg"],
            image: "images/f1-drivers/keke-rosberg.jpg",
            difficulty: "hard"
        },

        {
            answers: ["alan prost"],
            image: "images/f1-drivers/alain-prost.jpg",
            difficulty: "hard"
        },

        {
            answers: ["jacques laffite"],
            image: "images/f1-drivers/jacques-laffite.jpg",
            difficulty: "hard"
        },

        {
            answers: ["didier pironi"],
            image: "images/f1-drivers/didier-pironi.jpg",
            difficulty: "hard"
        },

        {
            answers: ["riccardo patrese"],
            image: "images/f1-drivers/riccardo-patrese.jpg",
            difficulty: "hard"
        },

        {
            answers: ["eddie irvine"],
            image: "images/f1-drivers/eddie-irvine.jpg",
            difficulty: "hard"
        },

        {
            answers: ["heinz harald frentzen"],
            image: "images/f1-drivers/heinz-harald-frentzen.jpg",
            difficulty: "hard"
        },

        {
            answers: ["felipe drugovich"],
            image: "images/f1-drivers/felipe-drugovich.jpg",
            difficulty: "hard"
        },

        {
            answers: ["theo pourchaire"],
            image: "images/f1-drivers/theo-pourchaire.jpg",
            difficulty: "hard"
        },

        {
            answers: ["franco colapinto"],
            image: "images/f1-drivers/franco-colapinto.jpg",
            difficulty: "medium"
        },

        {
            answers: ["jack doohan"],
            image: "images/f1-drivers/jack-doohan.jpg",
            difficulty: "medium"
        },

        {
            answers: ["logan sargeant"],
            image: "images/f1-drivers/logan-sargeant.jpg",
            difficulty: "medium"
        },

        {
            answers: ["nyck de vries"],
            image: "images/f1-drivers/nyck-de-vries.jpg",
            difficulty: "medium"
        },

        {
            answers: ["antonio giovinazzi"],
            image: "images/f1-drivers/antonio-giovinazzi.jpg",
            difficulty: "hard"
        },

        {
            answers: ["robert kubica"],
            image: "images/f1-drivers/robert-kubica.jpg",
            difficulty: "medium"
        },

        {
            answers: ["kamui kobayashi"],
            image: "images/f1-drivers/kamui-kobayashi.jpg",
            difficulty: "hard"
        },

        {
            answers: ["brendon hartley"],
            image: "images/f1-drivers/brendon-hartley.jpg",
            difficulty: "hard"
        },

        {
            answers: ["stoffel vandoorne"],
            image: "images/f1-drivers/stoffel-vandoorne.jpg",
            difficulty: "hard"
        },

        {
            answers: ["felipe nasr"],
            image: "images/f1-drivers/felipe-nasr.jpg",
            difficulty: "hard"
        }

    ],

"car-logos": [

        {
            answers: ["mercedes benz", "mercedes"],
            image: "images/car-logos/mercedes.png",
            difficulty: "easy"
        },

        {
            answers: ["bmw"],
            image: "images/car-logos/bmw.png",
            difficulty: "easy"
        },

        {
            answers: ["audi"],
            image: "images/car-logos/audi.png",
            difficulty: "easy"
        },

        {
            answers: ["porsche"],
            image: "images/car-logos/porsche.png",
            difficulty: "easy"
        },

        {
            answers: ["ferrari"],
            image: "images/car-logos/ferrari.png",
            difficulty: "easy"
        },

        {
            answers: ["lamborghini"],
            image: "images/car-logos/lamborghini.png",
            difficulty: "easy"
        },

        {
            answers: ["bugatti"],
            image: "images/car-logos/bugatti.png",
            difficulty: "easy"
        },

        {
            answers: ["mclaren"],
            image: "images/car-logos/mclaren.png",
            difficulty: "easy"
        },

        {
            answers: ["aston martin"],
            image: "images/car-logos/aston-martin.png",
            difficulty: "easy"
        },

        {
            answers: ["rolls royce"],
            image: "images/car-logos/rolls-royce.png",
            difficulty: "easy"
        },

        {
            answers: ["bentley"],
            image: "images/car-logos/bentley.png",
            difficulty: "easy"
        },

        {
            answers: ["koenigsegg"],
            image: "images/car-logos/koenigsegg.png",
            difficulty: "medium"
        },

        {
            answers: ["pagani"],
            image: "images/car-logos/pagani.png",
            difficulty: "medium"
        },

        {
            answers: ["rimac"],
            image: "images/car-logos/rimac.png",
            difficulty: "medium"
        },

        {
            answers: ["apollo"],
            image: "images/car-logos/apollo.png",
            difficulty: "hard"
        },

        {
            answers: ["hennessey"],
            image: "images/car-logos/hennessey.png",
            difficulty: "medium"
        },

        {
            answers: ["ssc"],
            image: "images/car-logos/ssc.png",
            difficulty: "hard"
        },

        {
            answers: ["de tomaso"],
            image: "images/car-logos/de-tomaso.png",
            difficulty: "hard"
        },

        {
            answers: ["maserati"],
            image: "images/car-logos/maserati.png",
            difficulty: "easy"
        },

        {
            answers: ["alfa romeo"],
            image: "images/car-logos/alfa-romeo.png",
            difficulty: "easy"
        },

        {
            answers: ["lancia"],
            image: "images/car-logos/lancia.png",
            difficulty: "medium"
        },

        {
            answers: ["fiat"],
            image: "images/car-logos/fiat.png",
            difficulty: "easy"
        },

        {
            answers: ["abarth"],
            image: "images/car-logos/abarth.png",
            difficulty: "medium"
        },

        {
            answers: ["lexus"],
            image: "images/car-logos/lexus.png",
            difficulty: "easy"
        },

        {
            answers: ["toyota"],
            image: "images/car-logos/toyota.png",
            difficulty: "easy"
        },

        {
            answers: ["nissan"],
            image: "images/car-logos/nissan.png",
            difficulty: "easy"
        },

        {
            answers: ["honda"],
            image: "images/car-logos/honda.png",
            difficulty: "easy"
        },

        {
            answers: ["mazda"],
            image: "images/car-logos/mazda.png",
            difficulty: "easy"
        },

        {
            answers: ["subaru"],
            image: "images/car-logos/subaru.png",
            difficulty: "easy"
        },

        {
            answers: ["mitsubishi"],
            image: "images/car-logos/mitsubishi.png",
            difficulty: "easy"
        },

        {
            answers: ["suzuki"],
            image: "images/car-logos/suzuki.png",
            difficulty: "easy"
        },

        {
            answers: ["hyundai"],
            image: "images/car-logos/hyundai.png",
            difficulty: "easy"
        },

        {
            answers: ["kia"],
            image: "images/car-logos/kia.png",
            difficulty: "easy"
        },

        {
            answers: ["genesis"],
            image: "images/car-logos/genesis.png",
            difficulty: "medium"
        },

        {
            answers: ["volkswagen", "vw"],
            image: "images/car-logos/volkswagen.png",
            difficulty: "easy"
        },

        {
            answers: ["skoda"],
            image: "images/car-logos/skoda.png",
            difficulty: "easy"
        },

        {
            answers: ["seat"],
            image: "images/car-logos/seat.png",
            difficulty: "easy"
        },

        {
            answers: ["cupra"],
            image: "images/car-logos/cupra.png",
            difficulty: "medium"
        },

        {
            answers: ["volvo"],
            image: "images/car-logos/volvo.png",
            difficulty: "easy"
        },

        {
            answers: ["saab"],
            image: "images/car-logos/saab.png",
            difficulty: "hard"
        },

        {
            answers: ["renault"],
            image: "images/car-logos/renault.png",
            difficulty: "easy"
        },

        {
            answers: ["peugeot"],
            image: "images/car-logos/peugeot.png",
            difficulty: "easy"
        },

        {
            answers: ["citroen"],
            image: "images/car-logos/citroen.png",
            difficulty: "easy"
        },

        {
            answers: ["ds automobiles", "ds"],
            image: "images/car-logos/ds.png",
            difficulty: "medium"
        },

        {
            answers: ["opel"],
            image: "images/car-logos/opel.png",
            difficulty: "easy"
        },

        {
            answers: ["vauxhall"],
            image: "images/car-logos/vauxhall.png",
            difficulty: "medium"
        },

        {
            answers: ["ford"],
            image: "images/car-logos/ford.png",
            difficulty: "easy"
        },

        {
            answers: ["chevrolet"],
            image: "images/car-logos/chevrolet.png",
            difficulty: "easy"
        },

        {
            answers: ["cadillac"],
            image: "images/car-logos/cadillac.png",
            difficulty: "easy"
        },

        {
            answers: ["gmc"],
            image: "images/car-logos/gmc.png",
            difficulty: "medium"
        },

        {
            answers: ["buick"],
            image: "images/car-logos/buick.png",
            difficulty: "medium"
        },

        {
            answers: ["lincoln"],
            image: "images/car-logos/lincoln.png",
            difficulty: "medium"
        },

        {
            answers: ["dodge"],
            image: "images/car-logos/dodge.png",
            difficulty: "easy"
        },

        {
            answers: ["jeep"],
            image: "images/car-logos/jeep.png",
            difficulty: "easy"
        },

        {
            answers: ["chrysler"],
            image: "images/car-logos/chrysler.png",
            difficulty: "medium"
        },

        {
            answers: ["ram"],
            image: "images/car-logos/ram.png",
            difficulty: "medium"
        },

        {
            answers: ["tesla"],
            image: "images/car-logos/tesla.png",
            difficulty: "easy"
        },

        {
            answers: ["lucid"],
            image: "images/car-logos/lucid.png",
            difficulty: "hard"
        },

        {
            answers: ["rivian"],
            image: "images/car-logos/rivian.png",
            difficulty: "medium"
        },

        {
            answers: ["polestar"],
            image: "images/car-logos/polestar.png",
            difficulty: "medium"
        },

        {
            answers: ["mini"],
            image: "images/car-logos/mini.png",
            difficulty: "easy"
        },

        {
            answers: ["land rover"],
            image: "images/car-logos/land-rover.png",
            difficulty: "easy"
        },

        {
            answers: ["range rover"],
            image: "images/car-logos/range-rover.png",
            difficulty: "easy"
        },

        {
            answers: ["jaguar"],
            image: "images/car-logos/jaguar.png",
            difficulty: "easy"
        },

        {
            answers: ["infiniti"],
            image: "images/car-logos/infiniti.png",
            difficulty: "medium"
        },

        {
            answers: ["acura"],
            image: "images/car-logos/acura.png",
            difficulty: "medium"
        },

        {
            answers: ["smart"],
            image: "images/car-logos/smart.png",
            difficulty: "medium"
        },

        {
            answers: ["maybach"],
            image: "images/car-logos/maybach.png",
            difficulty: "medium"
        },

        {
            answers: ["lotus"],
            image: "images/car-logos/lotus.png",
            difficulty: "medium"
        },

        {
            answers: ["caterham"],
            image: "images/car-logos/caterham.png",
            difficulty: "hard"
        },

        {
            answers: ["tvr"],
            image: "images/car-logos/tvr.png",
            difficulty: "hard"
        },

        {
            answers: ["spyker"],
            image: "images/car-logos/spyker.png",
            difficulty: "hard"
        },

        {
            answers: ["zenvo"],
            image: "images/car-logos/zenvo.png",
            difficulty: "hard"
        },

        {
            answers: ["gumpert"],
            image: "images/car-logos/gumpert.png",
            difficulty: "hard"
        },

        {
            answers: ["artega"],
            image: "images/car-logos/artega.png",
            difficulty: "hard"
        },

        {
            answers: ["isdera"],
            image: "images/car-logos/isdera.png",
            difficulty: "hard"
        },

        {
            answers: ["pininfarina"],
            image: "images/car-logos/pininfarina.png",
            difficulty: "hard"
        },

        {
            answers: ["vinfast"],
            image: "images/car-logos/vinfast.png",
            difficulty: "hard"
        },

        {
            answers: ["xpeng"],
            image: "images/car-logos/xpeng.png",
            difficulty: "hard"
        },

        {
            answers: ["nio"],
            image: "images/car-logos/nio.png",
            difficulty: "hard"
        }

    ],

"football-clubs": [

        {
            answers: ["real madrid"],
            image: "images/football-clubs/real-madrid.png",
            difficulty: "easy"
        },

        {
            answers: ["barcelona", "fc barcelona"],
            image: "images/football-clubs/barcelona.png",
            difficulty: "easy"
        },

        {
            answers: ["manchester united", "man united"],
            image: "images/football-clubs/manchester-united.png",
            difficulty: "easy"
        },

        {
            answers: ["manchester city", "man city"],
            image: "images/football-clubs/manchester-city.png",
            difficulty: "easy"
        },

        {
            answers: ["liverpool"],
            image: "images/football-clubs/liverpool.png",
            difficulty: "easy"
        },

        {
            answers: ["chelsea"],
            image: "images/football-clubs/chelsea.png",
            difficulty: "easy"
        },

        {
            answers: ["arsenal"],
            image: "images/football-clubs/arsenal.png",
            difficulty: "easy"
        },

        {
            answers: ["tottenham", "tottenham hotspur"],
            image: "images/football-clubs/tottenham.png",
            difficulty: "easy"
        },

        {
            answers: ["bayern munich", "bayern"],
            image: "images/football-clubs/bayern-munich.png",
            difficulty: "easy"
        },

        {
            answers: ["borussia dortmund", "dortmund"],
            image: "images/football-clubs/borussia-dortmund.png",
            difficulty: "easy"
        },

        {
            answers: ["juventus"],
            image: "images/football-clubs/juventus.png",
            difficulty: "easy"
        },

        {
            answers: ["ac milan", "milan"],
            image: "images/football-clubs/ac-milan.png",
            difficulty: "easy"
        },

        {
            answers: ["inter", "inter milan"],
            image: "images/football-clubs/inter.png",
            difficulty: "easy"
        },

        {
            answers: ["napoli"],
            image: "images/football-clubs/napoli.png",
            difficulty: "easy"
        },

        {
            answers: ["paris saint germain", "psg"],
            image: "images/football-clubs/psg.png",
            difficulty: "easy"
        },

        {
            answers: ["atletico madrid"],
            image: "images/football-clubs/atletico-madrid.png",
            difficulty: "easy"
        },

        {
            answers: ["sevilla"],
            image: "images/football-clubs/sevilla.png",
            difficulty: "medium"
        },

        {
            answers: ["valencia"],
            image: "images/football-clubs/valencia.png",
            difficulty: "medium"
        },

        {
            answers: ["ajax"],
            image: "images/football-clubs/ajax.png",
            difficulty: "easy"
        },

        {
            answers: ["psv eindhoven", "psv"],
            image: "images/football-clubs/psv.png",
            difficulty: "medium"
        },

        {
            answers: ["benfica"],
            image: "images/football-clubs/benfica.png",
            difficulty: "easy"
        },

        {
            answers: ["porto"],
            image: "images/football-clubs/porto.png",
            difficulty: "easy"
        },

        {
            answers: ["sporting cp", "sporting lisbon", "sporting"],
            image: "images/football-clubs/sporting.png",
            difficulty: "medium"
        },

        {
            answers: ["galatasaray"],
            image: "images/football-clubs/galatasaray.png",
            difficulty: "medium"
        },

        {
            answers: ["fenerbahce"],
            image: "images/football-clubs/fenerbahce.png",
            difficulty: "medium"
        },

        {
            answers: ["besiktas"],
            image: "images/football-clubs/besiktas.png",
            difficulty: "medium"
        },

        {
            answers: ["olympiacos"],
            image: "images/football-clubs/olympiacos.png",
            difficulty: "easy"
        },

        {
            answers: ["panathinaikos"],
            image: "images/football-clubs/panathinaikos.png",
            difficulty: "easy"
        },

        {
            answers: ["aek athens", "aek"],
            image: "images/football-clubs/aek.png",
            difficulty: "easy"
        },

        {
            answers: ["paok"],
            image: "images/football-clubs/paok.png",
            difficulty: "easy"
        },

        {
            answers: ["celtic"],
            image: "images/football-clubs/celtic.png",
            difficulty: "medium"
        },

        {
            answers: ["rangers"],
            image: "images/football-clubs/rangers.png",
            difficulty: "medium"
        },

        {
            answers: ["anderlecht"],
            image: "images/football-clubs/anderlecht.png",
            difficulty: "hard"
        },

        {
            answers: ["club brugge"],
            image: "images/football-clubs/club-brugge.png",
            difficulty: "medium"
        },

        {
            answers: ["shakhtar donetsk"],
            image: "images/football-clubs/shakhtar-donetsk.png",
            difficulty: "medium"
        },

        {
            answers: ["dinamo zagreb"],
            image: "images/football-clubs/dinamo-zagreb.png",
            difficulty: "medium"
        },

        {
            answers: ["red star belgrade", "crvena zvezda"],
            image: "images/football-clubs/red-star.png",
            difficulty: "medium"
        },

        {
            answers: ["sparta prague"],
            image: "images/football-clubs/sparta-prague.png",
            difficulty: "hard"
        },

        {
            answers: ["slavia prague"],
            image: "images/football-clubs/slavia-prague.png",
            difficulty: "hard"
        },

        {
            answers: ["basel"],
            image: "images/football-clubs/basel.png",
            difficulty: "medium"
        },

        {
            answers: ["villarreal"],
            image: "images/football-clubs/villarreal.png",
            difficulty: "medium"
        },

        {
            answers: ["athletic bilbao"],
            image: "images/football-clubs/athletic-bilbao.png",
            difficulty: "medium"
        },

        {
            answers: ["real sociedad"],
            image: "images/football-clubs/real-sociedad.png",
            difficulty: "medium"
        },

        {
            answers: ["real betis", "betis"],
            image: "images/football-clubs/real-betis.png",
            difficulty: "medium"
        },

        {
            answers: ["lazio"],
            image: "images/football-clubs/lazio.png",
            difficulty: "medium"
        },

        {
            answers: ["roma", "as roma"],
            image: "images/football-clubs/roma.png",
            difficulty: "easy"
        },

        {
            answers: ["atalanta"],
            image: "images/football-clubs/atalanta.png",
            difficulty: "medium"
        },

        {
            answers: ["fiorentina"],
            image: "images/football-clubs/fiorentina.png",
            difficulty: "medium"
        },

        {
            answers: ["bologna"],
            image: "images/football-clubs/bologna.png",
            difficulty: "hard"
        },

        {
            answers: ["monaco"],
            image: "images/football-clubs/monaco.png",
            difficulty: "medium"
        },

        {
            answers: ["marseille"],
            image: "images/football-clubs/marseille.png",
            difficulty: "medium"
        },

        {
            answers: ["lyon"],
            image: "images/football-clubs/lyon.png",
            difficulty: "medium"
        },

        {
            answers: ["lille"],
            image: "images/football-clubs/lille.png",
            difficulty: "medium"
        },

        {
            answers: ["lens"],
            image: "images/football-clubs/lens.png",
            difficulty: "hard"
        },

        {
            answers: ["salzburg", "red bull salzburg"],
            image: "images/football-clubs/salzburg.png",
            difficulty: "medium"
        },

        {
            answers: ["rapid vienna"],
            image: "images/football-clubs/rapid-vienna.png",
            difficulty: "hard"
        },

        {
            answers: ["legia warsaw"],
            image: "images/football-clubs/legia-warsaw.png",
            difficulty: "hard"
        },

        {
            answers: ["copenhagen", "fc copenhagen"],
            image: "images/football-clubs/copenhagen.png",
            difficulty: "medium"
        },

        {
            answers: ["malmo"],
            image: "images/football-clubs/malmo.png",
            difficulty: "hard"
        },

        {
            answers: ["young boys"],
            image: "images/football-clubs/young-boys.png",
            difficulty: "hard"
        },

        {
            answers: ["river plate"],
            image: "images/football-clubs/river-plate.png",
            difficulty: "medium"
        },

        {
            answers: ["boca juniors"],
            image: "images/football-clubs/boca-juniors.png",
            difficulty: "medium"
        },

        {
            answers: ["flamengo"],
            image: "images/football-clubs/flamengo.png",
            difficulty: "medium"
        },

        {
            answers: ["palmeiras"],
            image: "images/football-clubs/palmeiras.png",
            difficulty: "medium"
        },

        {
            answers: ["santos"],
            image: "images/football-clubs/santos.png",
            difficulty: "medium"
        },

        {
            answers: ["corinthians"],
            image: "images/football-clubs/corinthians.png",
            difficulty: "medium"
        },

        {
            answers: ["gremio"],
            image: "images/football-clubs/gremio.png",
            difficulty: "hard"
        },

        {
            answers: ["internacional"],
            image: "images/football-clubs/internacional.png",
            difficulty: "hard"
        },

        {
            answers: ["cruzeiro"],
            image: "images/football-clubs/cruzeiro.png",
            difficulty: "hard"
        },

        {
            answers: ["vasco da gama"],
            image: "images/football-clubs/vasco-da-gama.png",
            difficulty: "hard"
        },

        {
            answers: ["al hilal"],
            image: "images/football-clubs/al-hilal.png",
            difficulty: "medium"
        },

        {
            answers: ["al nassr"],
            image: "images/football-clubs/al-nassr.png",
            difficulty: "easy"
        },

        {
            answers: ["inter miami"],
            image: "images/football-clubs/inter-miami.png",
            difficulty: "easy"
        },

        {
            answers: ["los angeles fc", "lafc"],
            image: "images/football-clubs/lafc.png",
            difficulty: "medium"
        },

        {
            answers: ["galaxy", "la galaxy"],
            image: "images/football-clubs/la-galaxy.png",
            difficulty: "medium"
        },

        {
            answers: ["pachuca"],
            image: "images/football-clubs/pachuca.png",
            difficulty: "hard"
        },

        {
            answers: ["monterrey"],
            image: "images/football-clubs/monterrey.png",
            difficulty: "hard"
        },

        {
            answers: ["tigres uanl", "tigres"],
            image: "images/football-clubs/tigres.png",
            difficulty: "medium"
        },

        {
            answers: ["ajax cape town", "cape town spurs"],
            image: "images/football-clubs/cape-town-spurs.png",
            difficulty: "hard"
        },

        {
            answers: ["kaizer chiefs"],
            image: "images/football-clubs/kaizer-chiefs.png",
            difficulty: "hard"
        }

    ],


"country-flags": [

        {
            answers: ["greece"],
            image: "images/country-flags/greece.png",
            difficulty: "easy"
        },

        {
            answers: ["france"],
            image: "images/country-flags/france.png",
            difficulty: "easy"
        },

        {
            answers: ["germany"],
            image: "images/country-flags/germany.png",
            difficulty: "easy"
        },

        {
            answers: ["italy"],
            image: "images/country-flags/italy.png",
            difficulty: "easy"
        },

        {
            answers: ["spain"],
            image: "images/country-flags/spain.png",
            difficulty: "easy"
        },

        {
            answers: ["portugal"],
            image: "images/country-flags/portugal.png",
            difficulty: "easy"
        },

        {
            answers: ["united kingdom", "uk", "great britain", "britain"],
            image: "images/country-flags/united-kingdom.png",
            difficulty: "easy"
        },

        {
            answers: ["united states", "usa", "united states of america"],
            image: "images/country-flags/united-states.png",
            difficulty: "easy"
        },

        {
            answers: ["canada"],
            image: "images/country-flags/canada.png",
            difficulty: "easy"
        },

        {
            answers: ["mexico"],
            image: "images/country-flags/mexico.png",
            difficulty: "easy"
        },

        {
            answers: ["brazil"],
            image: "images/country-flags/brazil.png",
            difficulty: "easy"
        },

        {
            answers: ["argentina"],
            image: "images/country-flags/argentina.png",
            difficulty: "easy"
        },

        {
            answers: ["netherlands"],
            image: "images/country-flags/netherlands.png",
            difficulty: "easy"
        },

        {
            answers: ["belgium"],
            image: "images/country-flags/belgium.png",
            difficulty: "easy"
        },

        {
            answers: ["switzerland"],
            image: "images/country-flags/switzerland.png",
            difficulty: "easy"
        },

        {
            answers: ["austria"],
            image: "images/country-flags/austria.png",
            difficulty: "easy"
        },

        {
            answers: ["sweden"],
            image: "images/country-flags/sweden.png",
            difficulty: "easy"
        },

        {
            answers: ["norway"],
            image: "images/country-flags/norway.png",
            difficulty: "easy"
        },

        {
            answers: ["finland"],
            image: "images/country-flags/finland.png",
            difficulty: "easy"
        },

        {
            answers: ["denmark"],
            image: "images/country-flags/denmark.png",
            difficulty: "easy"
        },

        {
            answers: ["ireland"],
            image: "images/country-flags/ireland.png",
            difficulty: "easy"
        },

        {
            answers: ["poland"],
            image: "images/country-flags/poland.png",
            difficulty: "easy"
        },

        {
            answers: ["turkey"],
            image: "images/country-flags/turkey.png",
            difficulty: "easy"
        },

        {
            answers: ["japan"],
            image: "images/country-flags/japan.png",
            difficulty: "easy"
        },

        {
            answers: ["china"],
            image: "images/country-flags/china.png",
            difficulty: "easy"
        },

        {
            answers: ["south korea"],
            image: "images/country-flags/south-korea.png",
            difficulty: "easy"
        },

        {
            answers: ["india"],
            image: "images/country-flags/india.png",
            difficulty: "easy"
        },

        {
            answers: ["australia"],
            image: "images/country-flags/australia.png",
            difficulty: "easy"
        },

        {
            answers: ["new zealand"],
            image: "images/country-flags/new-zealand.png",
            difficulty: "medium"
        },

        {
            answers: ["egypt"],
            image: "images/country-flags/egypt.png",
            difficulty: "easy"
        },

        {
            answers: ["south africa"],
            image: "images/country-flags/south-africa.png",
            difficulty: "medium"
        },

        {
            answers: ["morocco"],
            image: "images/country-flags/morocco.png",
            difficulty: "medium"
        },

        {
            answers: ["saudi arabia"],
            image: "images/country-flags/saudi-arabia.png",
            difficulty: "medium"
        },

        {
            answers: ["united arab emirates", "uae"],
            image: "images/country-flags/united-arab-emirates.png",
            difficulty: "medium"
        },

        {
            answers: ["qatar"],
            image: "images/country-flags/qatar.png",
            difficulty: "medium"
        },

        {
            answers: ["croatia"],
            image: "images/country-flags/croatia.png",
            difficulty: "medium"
        },

        {
            answers: ["serbia"],
            image: "images/country-flags/serbia.png",
            difficulty: "medium"
        },

        {
            answers: ["romania"],
            image: "images/country-flags/romania.png",
            difficulty: "medium"
        },

        {
            answers: ["hungary"],
            image: "images/country-flags/hungary.png",
            difficulty: "medium"
        },

        {
            answers: ["czech republic", "czechia"],
            image: "images/country-flags/czech-republic.png",
            difficulty: "medium"
        },

        {
            answers: ["slovakia"],
            image: "images/country-flags/slovakia.png",
            difficulty: "medium"
        },

        {
            answers: ["slovenia"],
            image: "images/country-flags/slovenia.png",
            difficulty: "medium"
        },

        {
            answers: ["ukraine"],
            image: "images/country-flags/ukraine.png",
            difficulty: "medium"
        },

        {
            answers: ["bulgaria"],
            image: "images/country-flags/bulgaria.png",
            difficulty: "medium"
        },

        {
            answers: ["albania"],
            image: "images/country-flags/albania.png",
            difficulty: "medium"
        },

        {
            answers: ["north macedonia"],
            image: "images/country-flags/north-macedonia.png",
            difficulty: "medium"
        },

        {
            answers: ["montenegro"],
            image: "images/country-flags/montenegro.png",
            difficulty: "hard"
        },

        {
            answers: ["bosnia and herzegovina", "bosnia"],
            image: "images/country-flags/bosnia-and-herzegovina.png",
            difficulty: "medium"
        },

        {
            answers: ["iceland"],
            image: "images/country-flags/iceland.png",
            difficulty: "medium"
        },

        {
            answers: ["estonia"],
            image: "images/country-flags/estonia.png",
            difficulty: "medium"
        },

        {
            answers: ["latvia"],
            image: "images/country-flags/latvia.png",
            difficulty: "hard"
        },

        {
            answers: ["lithuania"],
            image: "images/country-flags/lithuania.png",
            difficulty: "medium"
        },

        {
            answers: ["luxembourg"],
            image: "images/country-flags/luxembourg.png",
            difficulty: "hard"
        },

        {
            answers: ["liechtenstein"],
            image: "images/country-flags/liechtenstein.png",
            difficulty: "hard"
        },

        {
            answers: ["monaco"],
            image: "images/country-flags/monaco.png",
            difficulty: "hard"
        },

        {
            answers: ["andorra"],
            image: "images/country-flags/andorra.png",
            difficulty: "hard"
        },

        {
            answers: ["san marino"],
            image: "images/country-flags/san-marino.png",
            difficulty: "hard"
        },

        {
            answers: ["vatican city", "vatican"],
            image: "images/country-flags/vatican-city.png",
            difficulty: "hard"
        },

        {
            answers: ["kazakhstan"],
            image: "images/country-flags/kazakhstan.png",
            difficulty: "medium"
        },

        {
            answers: ["uzbekistan"],
            image: "images/country-flags/uzbekistan.png",
            difficulty: "hard"
        },

        {
            answers: ["pakistan"],
            image: "images/country-flags/pakistan.png",
            difficulty: "medium"
        },

        {
            answers: ["bangladesh"],
            image: "images/country-flags/bangladesh.png",
            difficulty: "medium"
        },

        {
            answers: ["sri lanka"],
            image: "images/country-flags/sri-lanka.png",
            difficulty: "hard"
        },

        {
            answers: ["thailand"],
            image: "images/country-flags/thailand.png",
            difficulty: "medium"
        },

        {
            answers: ["vietnam"],
            image: "images/country-flags/vietnam.png",
            difficulty: "medium"
        },

        {
            answers: ["philippines"],
            image: "images/country-flags/philippines.png",
            difficulty: "hard"
        },

        {
            answers: ["indonesia"],
            image: "images/country-flags/indonesia.png",
            difficulty: "medium"
        },

        {
            answers: ["singapore"],
            image: "images/country-flags/singapore.png",
            difficulty: "medium"
        },

        {
            answers: ["malaysia"],
            image: "images/country-flags/malaysia.png",
            difficulty: "hard"
        },

        {
            answers: ["kenya"],
            image: "images/country-flags/kenya.png",
            difficulty: "hard"
        },

        {
            answers: ["nigeria"],
            image: "images/country-flags/nigeria.png",
            difficulty: "medium"
        },

        {
            answers: ["ghana"],
            image: "images/country-flags/ghana.png",
            difficulty: "hard"
        },

        {
            answers: ["tunisia"],
            image: "images/country-flags/tunisia.png",
            difficulty: "medium"
        },

        {
            answers: ["algeria"],
            image: "images/country-flags/algeria.png",
            difficulty: "medium"
        },

        {
            answers: ["peru"],
            image: "images/country-flags/peru.png",
            difficulty: "medium"
        },

        {
            answers: ["chile"],
            image: "images/country-flags/chile.png",
            difficulty: "medium"
        },

        {
            answers: ["colombia"],
            image: "images/country-flags/colombia.png",
            difficulty: "medium"
        },

        {
            answers: ["uruguay"],
            image: "images/country-flags/uruguay.png",
            difficulty: "medium"
        },

        {
            answers: ["venezuela"],
            image: "images/country-flags/venezuela.png",
            difficulty: "hard"
        },

        {
            answers: ["ecuador"],
            image: "images/country-flags/ecuador.png",
            difficulty: "hard"
        }

    ],

movies: [

        {
            answers: ["avatar"],
            image: "images/movies/avatar.jpg",
            difficulty: "easy"
        },

        {
            answers: ["titanic"],
            image: "images/movies/titanic.jpg",
            difficulty: "easy"
        },

        {
            answers: ["the lion king"],
            image: "images/movies/the-lion-king.jpg",
            difficulty: "easy"
        },

        {
            answers: ["frozen"],
            image: "images/movies/frozen.jpg",
            difficulty: "easy"
        },

        {
            answers: ["toy story"],
            image: "images/movies/toy-story.jpg",
            difficulty: "easy"
        },

        {
            answers: ["cars"],
            image: "images/movies/cars.jpg",
            difficulty: "easy"
        },

        {
            answers: ["finding nemo"],
            image: "images/movies/finding-nemo.jpg",
            difficulty: "easy"
        },

        {
            answers: ["inside out"],
            image: "images/movies/inside-out.jpg",
            difficulty: "easy"
        },

        {
            answers: ["up"],
            image: "images/movies/up.jpg",
            difficulty: "easy"
        },

        {
            answers: ["coco"],
            image: "images/movies/coco.jpg",
            difficulty: "easy"
        },

        {
            answers: ["shrek"],
            image: "images/movies/shrek.jpg",
            difficulty: "easy"
        },

        {
            answers: ["kung fu panda"],
            image: "images/movies/kung-fu-panda.jpg",
            difficulty: "easy"
        },

        {
            answers: ["madagascar"],
            image: "images/movies/madagascar.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ice age"],
            image: "images/movies/ice-age.jpg",
            difficulty: "easy"
        },

        {
            answers: ["despicable me"],
            image: "images/movies/despicable-me.jpg",
            difficulty: "easy"
        },

        {
            answers: ["minions"],
            image: "images/movies/minions.jpg",
            difficulty: "easy"
        },

        {
            answers: ["moana"],
            image: "images/movies/moana.jpg",
            difficulty: "easy"
        },

        {
            answers: ["zootopia"],
            image: "images/movies/zootopia.jpg",
            difficulty: "easy"
        },

        {
            answers: ["encanto"],
            image: "images/movies/encanto.jpg",
            difficulty: "easy"
        },

        {
            answers: ["the incredibles"],
            image: "images/movies/the-incredibles.jpg",
            difficulty: "easy"
        },

        {
            answers: ["harry potter"],
            image: "images/movies/harry-potter.jpg",
            difficulty: "easy"
        },

        {
            answers: ["the lord of the rings"],
            image: "images/movies/the-lord-of-the-rings.jpg",
            difficulty: "easy"
        },

        {
            answers: ["the hobbit"],
            image: "images/movies/the-hobbit.jpg",
            difficulty: "medium"
        },

        {
            answers: ["star wars"],
            image: "images/movies/star-wars.jpg",
            difficulty: "easy"
        },

        {
            answers: ["jurassic park"],
            image: "images/movies/jurassic-park.jpg",
            difficulty: "easy"
        },

        {
            answers: ["jaws"],
            image: "images/movies/jaws.jpg",
            difficulty: "medium"
        },

        {
            answers: ["the dark knight"],
            image: "images/movies/the-dark-knight.jpg",
            difficulty: "easy"
        },

        {
            answers: ["batman begins"],
            image: "images/movies/batman-begins.jpg",
            difficulty: "medium"
        },

        {
            answers: ["joker"],
            image: "images/movies/joker.jpg",
            difficulty: "easy"
        },

        {
            answers: ["avengers endgame", "endgame"],
            image: "images/movies/avengers-endgame.jpg",
            difficulty: "easy"
        },

        {
            answers: ["avengers infinity war", "infinity war"],
            image: "images/movies/avengers-infinity-war.jpg",
            difficulty: "easy"
        },

        {
            answers: ["iron man"],
            image: "images/movies/iron-man.jpg",
            difficulty: "easy"
        },

        {
            answers: ["captain america civil war", "civil war"],
            image: "images/movies/captain-america-civil-war.jpg",
            difficulty: "medium"
        },

        {
            answers: ["doctor strange"],
            image: "images/movies/doctor-strange.jpg",
            difficulty: "medium"
        },

        {
            answers: ["black panther"],
            image: "images/movies/black-panther.jpg",
            difficulty: "medium"
        },

        {
            answers: ["deadpool"],
            image: "images/movies/deadpool.jpg",
            difficulty: "easy"
        },

        {
            answers: ["logan"],
            image: "images/movies/logan.jpg",
            difficulty: "medium"
        },

        {
            answers: ["the matrix"],
            image: "images/movies/the-matrix.jpg",
            difficulty: "medium"
        },

        {
            answers: ["inception"],
            image: "images/movies/inception.jpg",
            difficulty: "medium"
        },

        {
            answers: ["interstellar"],
            image: "images/movies/interstellar.jpg",
            difficulty: "easy"
        },

        {
            answers: ["gladiator"],
            image: "images/movies/gladiator.jpg",
            difficulty: "easy"
        },

        {
            answers: ["braveheart"],
            image: "images/movies/braveheart.jpg",
            difficulty: "medium"
        },

        {
            answers: ["the godfather"],
            image: "images/movies/the-godfather.jpg",
            difficulty: "medium"
        },

        {
            answers: ["scarface"],
            image: "images/movies/scarface.jpg",
            difficulty: "medium"
        },

        {
            answers: ["pulp fiction"],
            image: "images/movies/pulp-fiction.jpg",
            difficulty: "medium"
        },

        {
            answers: ["fight club"],
            image: "images/movies/fight-club.jpg",
            difficulty: "medium"
        },

        {
            answers: ["forrest gump"],
            image: "images/movies/forrest-gump.jpg",
            difficulty: "easy"
        },

        {
            answers: ["the shawshank redemption"],
            image: "images/movies/the-shawshank-redemption.jpg",
            difficulty: "hard"
        },

        {
            answers: ["saving private ryan"],
            image: "images/movies/saving-private-ryan.jpg",
            difficulty: "medium"
        },

        {
            answers: ["the green mile"],
            image: "images/movies/the-green-mile.jpg",
            difficulty: "medium"
        },

        {
            answers: ["cast away"],
            image: "images/movies/cast-away.jpg",
            difficulty: "medium"
        },

        {
            answers: ["top gun"],
            image: "images/movies/top-gun.jpg",
            difficulty: "easy"
        },

        {
            answers: ["top gun maverick"],
            image: "images/movies/top-gun-maverick.jpg",
            difficulty: "easy"
        },

        {
            answers: ["mission impossible"],
            image: "images/movies/mission-impossible.jpg",
            difficulty: "easy"
        },

        {
            answers: ["john wick"],
            image: "images/movies/john-wick.jpg",
            difficulty: "easy"
        },

        {
            answers: ["rocky"],
            image: "images/movies/rocky.jpg",
            difficulty: "easy"
        },

        {
            answers: ["rambo"],
            image: "images/movies/rambo.jpg",
            difficulty: "easy"
        },

        {
            answers: ["terminator 2"],
            image: "images/movies/terminator-2.jpg",
            difficulty: "medium"
        },

        {
            answers: ["alien"],
            image: "images/movies/alien.jpg",
            difficulty: "medium"
        },

        {
            answers: ["predator"],
            image: "images/movies/predator.jpg",
            difficulty: "medium"
        },

        {
            answers: ["the silence of the lambs"],
            image: "images/movies/the-silence-of-the-lambs.jpg",
            difficulty: "hard"
        },

        {
            answers: ["pirates of the caribbean"],
            image: "images/movies/pirates-of-the-caribbean.jpg",
            difficulty: "easy"
        },

        {
            answers: ["transformers"],
            image: "images/movies/transformers.jpg",
            difficulty: "easy"
        },

        {
            answers: ["pacific rim"],
            image: "images/movies/pacific-rim.jpg",
            difficulty: "medium"
        },

        {
            answers: ["godzilla"],
            image: "images/movies/godzilla.jpg",
            difficulty: "easy"
        },

        {
            answers: ["king kong"],
            image: "images/movies/king-kong.jpg",
            difficulty: "easy"
        },

        {
            answers: ["dune"],
            image: "images/movies/dune.jpg",
            difficulty: "easy"
        },

        {
            answers: ["oppenheimer"],
            image: "images/movies/oppenheimer.jpg",
            difficulty: "easy"
        },

        {
            answers: ["barbie"],
            image: "images/movies/barbie.jpg",
            difficulty: "easy"
        },

        {
            answers: ["the batman"],
            image: "images/movies/the-batman.jpg",
            difficulty: "medium"
        },

        {
            answers: ["spider man no way home"],
            image: "images/movies/spider-man-no-way-home.jpg",
            difficulty: "easy"
        },

        {
            answers: ["venom"],
            image: "images/movies/venom.jpg",
            difficulty: "easy"
        },

        {
            answers: ["guardians of the galaxy"],
            image: "images/movies/guardians-of-the-galaxy.jpg",
            difficulty: "medium"
        },

        {
            answers: ["thor ragnarok"],
            image: "images/movies/thor-ragnarok.jpg",
            difficulty: "medium"
        },

        {
            answers: ["black widow"],
            image: "images/movies/black-widow.jpg",
            difficulty: "medium"
        },

        {
            answers: ["captain marvel"],
            image: "images/movies/captain-marvel.jpg",
            difficulty: "medium"
        },

        {
            answers: ["the wolf of wall street"],
            image: "images/movies/the-wolf-of-wall-street.jpg",
            difficulty: "medium"
        },

        {
            answers: ["whiplash"],
            image: "images/movies/whiplash.jpg",
            difficulty: "hard"
        },

        {
            answers: ["la la land"],
            image: "images/movies/la-la-land.jpg",
            difficulty: "medium"
        },

        {
            answers: ["parasite"],
            image: "images/movies/parasite.jpg",
            difficulty: "hard"
        }

    ],


"video-games": [

        {
            answers: ["minecraft"],
            image: "images/video-games/minecraft.jpg",
            difficulty: "easy"
        },

        {
            answers: ["fortnite"],
            image: "images/video-games/fortnite.jpg",
            difficulty: "easy"
        },

        {
            answers: ["roblox"],
            image: "images/video-games/roblox.jpg",
            difficulty: "easy"
        },

        {
            answers: ["grand theft auto v", "gta v", "gta 5"],
            image: "images/video-games/gta-v.jpg",
            difficulty: "easy"
        },

        {
            answers: ["red dead redemption 2", "rdr2"],
            image: "images/video-games/red-dead-redemption-2.jpg",
            difficulty: "easy"
        },

        {
            answers: ["call of duty"],
            image: "images/video-games/call-of-duty.jpg",
            difficulty: "easy"
        },

        {
            answers: ["counter strike 2", "cs2"],
            image: "images/video-games/counter-strike-2.jpg",
            difficulty: "easy"
        },

        {
            answers: ["valorant"],
            image: "images/video-games/valorant.jpg",
            difficulty: "easy"
        },

        {
            answers: ["league of legends", "lol"],
            image: "images/video-games/league-of-legends.jpg",
            difficulty: "easy"
        },

        {
            answers: ["dota 2"],
            image: "images/video-games/dota-2.jpg",
            difficulty: "medium"
        },

        {
            answers: ["rocket league"],
            image: "images/video-games/rocket-league.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ea sports fc 26", "fc 26"],
            image: "images/video-games/fc-26.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ea sports fc 25", "fc 25"],
            image: "images/video-games/fc-25.jpg",
            difficulty: "easy"
        },

        {
            answers: ["efootball"],
            image: "images/video-games/efootball.jpg",
            difficulty: "easy"
        },

        {
            answers: ["among us"],
            image: "images/video-games/among-us.jpg",
            difficulty: "easy"
        },

        {
            answers: ["fall guys"],
            image: "images/video-games/fall-guys.jpg",
            difficulty: "easy"
        },

        {
            answers: ["the sims 4"],
            image: "images/video-games/the-sims-4.jpg",
            difficulty: "easy"
        },

        {
            answers: ["terraria"],
            image: "images/video-games/terraria.jpg",
            difficulty: "medium"
        },

        {
            answers: ["stardew valley"],
            image: "images/video-games/stardew-valley.jpg",
            difficulty: "medium"
        },

        {
            answers: ["subnautica"],
            image: "images/video-games/subnautica.jpg",
            difficulty: "medium"
        },

        {
            answers: ["elden ring"],
            image: "images/video-games/elden-ring.jpg",
            difficulty: "medium"
        },

        {
            answers: ["dark souls"],
            image: "images/video-games/dark-souls.jpg",
            difficulty: "medium"
        },

        {
            answers: ["bloodborne"],
            image: "images/video-games/bloodborne.jpg",
            difficulty: "medium"
        },

        {
            answers: ["sekiro"],
            image: "images/video-games/sekiro.jpg",
            difficulty: "hard"
        },

        {
            answers: ["cyberpunk 2077"],
            image: "images/video-games/cyberpunk-2077.jpg",
            difficulty: "medium"
        },

        {
            answers: ["the witcher 3"],
            image: "images/video-games/the-witcher-3.jpg",
            difficulty: "medium"
        },

        {
            answers: ["skyrim"],
            image: "images/video-games/skyrim.jpg",
            difficulty: "medium"
        },

        {
            answers: ["fallout 4"],
            image: "images/video-games/fallout-4.jpg",
            difficulty: "medium"
        },

        {
            answers: ["halo"],
            image: "images/video-games/halo.jpg",
            difficulty: "medium"
        },

        {
            answers: ["doom eternal"],
            image: "images/video-games/doom-eternal.jpg",
            difficulty: "medium"
        },

        {
            answers: ["resident evil 4"],
            image: "images/video-games/resident-evil-4.jpg",
            difficulty: "medium"
        },

        {
            answers: ["silent hill 2"],
            image: "images/video-games/silent-hill-2.jpg",
            difficulty: "hard"
        },

        {
            answers: ["god of war"],
            image: "images/video-games/god-of-war.jpg",
            difficulty: "easy"
        },

        {
            answers: ["horizon forbidden west"],
            image: "images/video-games/horizon-forbidden-west.jpg",
            difficulty: "medium"
        },

        {
            answers: ["ghost of tsushima"],
            image: "images/video-games/ghost-of-tsushima.jpg",
            difficulty: "medium"
        },

        {
            answers: ["death stranding"],
            image: "images/video-games/death-stranding.jpg",
            difficulty: "hard"
        },

        {
            answers: ["assassin's creed", "assassins creed"],
            image: "images/video-games/assassins-creed.jpg",
            difficulty: "easy"
        },

        {
            answers: ["far cry 3"],
            image: "images/video-games/far-cry-3.jpg",
            difficulty: "medium"
        },

        {
            answers: ["watch dogs"],
            image: "images/video-games/watch-dogs.jpg",
            difficulty: "medium"
        },

        {
            answers: ["need for speed"],
            image: "images/video-games/need-for-speed.jpg",
            difficulty: "easy"
        },

        {
            answers: ["gran turismo 7"],
            image: "images/video-games/gran-turismo-7.jpg",
            difficulty: "easy"
        },

        {
            answers: ["forza horizon 5"],
            image: "images/video-games/forza-horizon-5.jpg",
            difficulty: "easy"
        },

        {
            answers: ["mario kart 8 deluxe"],
            image: "images/video-games/mario-kart-8-deluxe.jpg",
            difficulty: "easy"
        },

        {
            answers: ["super mario odyssey"],
            image: "images/video-games/super-mario-odyssey.jpg",
            difficulty: "easy"
        },

        {
            answers: ["super mario bros wonder"],
            image: "images/video-games/super-mario-bros-wonder.jpg",
            difficulty: "easy"
        },

        {
            answers: ["the legend of zelda breath of the wild", "breath of the wild"],
            image: "images/video-games/breath-of-the-wild.jpg",
            difficulty: "easy"
        },

        {
            answers: ["the legend of zelda tears of the kingdom", "tears of the kingdom"],
            image: "images/video-games/tears-of-the-kingdom.jpg",
            difficulty: "medium"
        },

        {
            answers: ["animal crossing new horizons"],
            image: "images/video-games/animal-crossing-new-horizons.jpg",
            difficulty: "easy"
        },

        {
            answers: ["pokemon scarlet"],
            image: "images/video-games/pokemon-scarlet.jpg",
            difficulty: "medium"
        },

        {
            answers: ["pokemon violet"],
            image: "images/video-games/pokemon-violet.jpg",
            difficulty: "medium"
        },

        {
            answers: ["pokemon legends arceus"],
            image: "images/video-games/pokemon-legends-arceus.jpg",
            difficulty: "medium"
        },

        {
            answers: ["palworld"],
            image: "images/video-games/palworld.jpg",
            difficulty: "medium"
        },

        {
            answers: ["black myth wukong"],
            image: "images/video-games/black-myth-wukong.jpg",
            difficulty: "medium"
        },

        {
            answers: ["clair obscur expedition 33"],
            image: "images/video-games/clair-obscur-expedition-33.jpg",
            difficulty: "hard"
        },

        {
            answers: ["split fiction"],
            image: "images/video-games/split-fiction.jpg",
            difficulty: "hard"
        },

        {
            answers: ["it takes two"],
            image: "images/video-games/it-takes-two.jpg",
            difficulty: "medium"
        },

        {
            answers: ["a way out"],
            image: "images/video-games/a-way-out.jpg",
            difficulty: "medium"
        },

        {
            answers: ["hades"],
            image: "images/video-games/hades.jpg",
            difficulty: "medium"
        },

        {
            answers: ["hollow knight"],
            image: "images/video-games/hollow-knight.jpg",
            difficulty: "medium"
        },

        {
            answers: ["cuphead"],
            image: "images/video-games/cuphead.jpg",
            difficulty: "medium"
        },

        {
            answers: ["celeste"],
            image: "images/video-games/celeste.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ori and the blind forest"],
            image: "images/video-games/ori-and-the-blind-forest.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ori and the will of the wisps"],
            image: "images/video-games/ori-and-the-will-of-the-wisps.jpg",
            difficulty: "hard"
        },

        {
            answers: ["portal 2"],
            image: "images/video-games/portal-2.jpg",
            difficulty: "medium"
        },

        {
            answers: ["half life 2"],
            image: "images/video-games/half-life-2.jpg",
            difficulty: "hard"
        },

        {
            answers: ["left 4 dead 2"],
            image: "images/video-games/left-4-dead-2.jpg",
            difficulty: "hard"
        },

        {
            answers: ["team fortress 2"],
            image: "images/video-games/team-fortress-2.jpg",
            difficulty: "hard"
        },

        {
            answers: ["world of warcraft"],
            image: "images/video-games/world-of-warcraft.jpg",
            difficulty: "medium"
        },

        {
            answers: ["diablo iv", "diablo 4"],
            image: "images/video-games/diablo-4.jpg",
            difficulty: "medium"
        },

        {
            answers: ["starcraft ii", "starcraft 2"],
            image: "images/video-games/starcraft-2.jpg",
            difficulty: "hard"
        },

        {
            answers: ["age of empires ii", "age of empires 2"],
            image: "images/video-games/age-of-empires-2.jpg",
            difficulty: "medium"
        },

        {
            answers: ["civilization vi", "civilization 6"],
            image: "images/video-games/civilization-6.jpg",
            difficulty: "hard"
        },

        {
            answers: ["rainbow six siege"],
            image: "images/video-games/rainbow-six-siege.jpg",
            difficulty: "medium"
        },

        {
            answers: ["escape from tarkov"],
            image: "images/video-games/escape-from-tarkov.jpg",
            difficulty: "hard"
        },

        {
            answers: ["war thunder"],
            image: "images/video-games/war-thunder.jpg",
            difficulty: "medium"
        },

        {
            answers: ["world of tanks"],
            image: "images/video-games/world-of-tanks.jpg",
            difficulty: "medium"
        },

        {
            answers: ["brawl stars"],
            image: "images/video-games/brawl-stars.jpg",
            difficulty: "easy"
        },

        {
            answers: ["clash royale"],
            image: "images/video-games/clash-royale.jpg",
            difficulty: "easy"
        },

        {
            answers: ["clash of clans"],
            image: "images/video-games/clash-of-clans.jpg",
            difficulty: "easy"
        },

        {
            answers: ["hay day"],
            image: "images/video-games/hay-day.jpg",
            difficulty: "easy"
        }

    ],

"basketball-players": [

         {
            answers: ["michael jordan", "jordan"],
            image: "images/basketball-players/michael-jordan.jpg",
            difficulty: "easy"
        },

        {
            answers: ["lebron james", "james"],
            image: "images/basketball-players/lebron-james.jpg",
            difficulty: "easy"
        },

        {
            answers: ["kobe bryant", "bryant"],
            image: "images/basketball-players/kobe-bryant.jpg",
            difficulty: "easy"
        },

        {
            answers: ["stephen curry", "curry"],
            image: "images/basketball-players/stephen-curry.jpg",
            difficulty: "easy"
        },

        {
            answers: ["kevin durant", "durant"],
            image: "images/basketball-players/kevin-durant.jpg",
            difficulty: "easy"
        },

        {
            answers: ["giannis antetokounmpo", "antetokounmpo"],
            image: "images/basketball-players/giannis-antetokounmpo.jpg",
            difficulty: "easy"
        },

        {
            answers: ["nikola jokic", "jokic"],
            image: "images/basketball-players/nikola-jokic.jpg",
            difficulty: "easy"
        },

        {
            answers: ["luka doncic", "doncic"],
            image: "images/basketball-players/luka-doncic.jpg",
            difficulty: "easy"
        },

        {
            answers: ["shaquille o neal", "oneal"],
            image: "images/basketball-players/shaquille-oneal.jpg",
            difficulty: "easy"
        },

        {
            answers: ["tim duncan", "duncan"],
            image: "images/basketball-players/tim-duncan.jpg",
            difficulty: "easy"
        },

        {
            answers: ["magic johnson", "johnson"],
            image: "images/basketball-players/magic-johnson.jpg",
            difficulty: "easy"
        },

        {
            answers: ["larry bird", "bird"],
            image: "images/basketball-players/larry-bird.jpg",
            difficulty: "easy"
        },

        {
            answers: ["kareem abdul jabbar", "abdul jabbar"],
            image: "images/basketball-players/kareem-abdul-jabbar.jpg",
            difficulty: "easy"
        },

        {
            answers: ["bill russell", "russell"],
            image: "images/basketball-players/bill-russell.jpg",
            difficulty: "medium"
        },

        {
            answers: ["wilt chamberlain", "chamberlain"],
            image: "images/basketball-players/wilt-chamberlain.jpg",
            difficulty: "medium"
        },

        {
            answers: ["hakeem olajuwon", "olajuwon"],
            image: "images/basketball-players/hakeem-olajuwon.jpg",
            difficulty: "medium"
        },

        {
            answers: ["dirk nowitzki", "nowitzki"],
            image: "images/basketball-players/dirk-nowitzki.jpg",
            difficulty: "easy"
        },

        {
            answers: ["allen iverson", "iverson"],
            image: "images/basketball-players/allen-iverson.jpg",
            difficulty: "easy"
        },

        {
            answers: ["charles barkley", "barkley"],
            image: "images/basketball-players/charles-barkley.jpg",
            difficulty: "medium"
        },

        {
            answers: ["dwyane wade", "wade"],
            image: "images/basketball-players/dwyane-wade.jpg",
            difficulty: "easy"
        },

        {
            answers: ["kawhi leonard", "leonard"],
            image: "images/basketball-players/kawhi-leonard.jpg",
            difficulty: "easy"
        },

        {
            answers: ["james harden", "harden"],
            image: "images/basketball-players/james-harden.jpg",
            difficulty: "easy"
        },

        {
            answers: ["kyrie irving", "irving"],
            image: "images/basketball-players/kyrie-irving.jpg",
            difficulty: "easy"
        },

        {
            answers: ["damian lillard", "lillard"],
            image: "images/basketball-players/damian-lillard.jpg",
            difficulty: "easy"
        },

        {
            answers: ["anthony davis", "davis"],
            image: "images/basketball-players/anthony-davis.jpg",
            difficulty: "easy"
        },

        {
            answers: ["joel embiid", "embiid"],
            image: "images/basketball-players/joel-embiid.jpg",
            difficulty: "easy"
        },

        {
            answers: ["victor wembanyama", "wembanyama"],
            image: "images/basketball-players/victor-wembanyama.jpg",
            difficulty: "easy"
        },

        {
            answers: ["shai gilgeous alexander", "gilgeous alexander"],
            image: "images/basketball-players/shai-gilgeous-alexander.jpg",
            difficulty: "easy"
        },

        {
            answers: ["jayson tatum", "tatum"],
            image: "images/basketball-players/jayson-tatum.jpg",
            difficulty: "easy"
        },

        {
            answers: ["jaylen brown", "brown"],
            image: "images/basketball-players/jaylen-brown.jpg",
            difficulty: "medium"
        },

        {
            answers: ["anthony edwards", "edwards"],
            image: "images/basketball-players/anthony-edwards.jpg",
            difficulty: "easy"
        },

        {
            answers: ["ja morant", "morant"],
            image: "images/basketball-players/ja-morant.jpg",
            difficulty: "easy"
        },

        {
            answers: ["devin booker", "booker"],
            image: "images/basketball-players/devin-booker.jpg",
            difficulty: "easy"
        },

        {
            answers: ["jimmy butler", "butler"],
            image: "images/basketball-players/jimmy-butler.jpg",
            difficulty: "easy"
        },

        {
            answers: ["trae young", "young"],
            image: "images/basketball-players/trae-young.jpg",
            difficulty: "easy"
        },

        {
            answers: ["tyrese haliburton", "haliburton"],
            image: "images/basketball-players/tyrese-haliburton.jpg",
            difficulty: "medium"
        },

        {
            answers: ["zion williamson", "williamson"],
            image: "images/basketball-players/zion-williamson.jpg",
            difficulty: "easy"
        },

        {
            answers: ["klay thompson", "thompson"],
            image: "images/basketball-players/klay-thompson.jpg",
            difficulty: "easy"
        },

        {
            answers: ["draymond green", "green"],
            image: "images/basketball-players/draymond-green.jpg",
            difficulty: "medium"
        },

        {
            answers: ["russell westbrook", "westbrook"],
            image: "images/basketball-players/russell-westbrook.jpg",
            difficulty: "easy"
        },

        {
            answers: ["paul george", "george"],
            image: "images/basketball-players/paul-george.jpg",
            difficulty: "medium"
        },

        {
            answers: ["demar derozan", "derozan"],
            image: "images/basketball-players/demar-derozan.jpg",
            difficulty: "medium"
        },

        {
            answers: ["domantas sabonis", "sabonis"],
            image: "images/basketball-players/domantas-sabonis.jpg",
            difficulty: "medium"
        },

        {
            answers: ["deaaron fox", "fox"],
            image: "images/basketball-players/deaaron-fox.jpg",
            difficulty: "medium"
        },

        {
            answers: ["lamelo ball", "ball"],
            image: "images/basketball-players/lamelo-ball.jpg",
            difficulty: "medium"
        },

        {
            answers: ["paolo banchero", "banchero"],
            image: "images/basketball-players/paolo-banchero.jpg",
            difficulty: "medium"
        },

        {
            answers: ["chet holmgren", "holmgren"],
            image: "images/basketball-players/chet-holmgren.jpg",
            difficulty: "medium"
        },

        {
            answers: ["bradley beal", "beal"],
            image: "images/basketball-players/bradley-beal.jpg",
            difficulty: "medium"
        },

        {
            answers: ["carmelo anthony", "anthony"],
            image: "images/basketball-players/carmelo-anthony.jpg",
            difficulty: "easy"
        },

        {
            answers: ["yao ming", "ming"],
            image: "images/basketball-players/yao-ming.jpg",
            difficulty: "medium"
        },

        {
            answers: ["pau gasol", "gasol"],
            image: "images/basketball-players/pau-gasol.jpg",
            difficulty: "medium"
        },

        {
            answers: ["tony parker", "parker"],
            image: "images/basketball-players/tony-parker.jpg",
            difficulty: "medium"
        },

        {
            answers: ["manu ginobili", "ginobili"],
            image: "images/basketball-players/manu-ginobili.jpg",
            difficulty: "medium"
        },

        {
            answers: ["scottie pippen", "pippen"],
            image: "images/basketball-players/scottie-pippen.jpg",
            difficulty: "medium"
        },

        {
            answers: ["nikos galis", "galis"],
            image: "images/basketball-players/nikos-galis.jpg",
            difficulty: "medium"
        },

        {
            answers: ["vassilis spanoulis", "spanoulis"],
            image: "images/basketball-players/vassilis-spanoulis.jpg",
            difficulty: "medium"
        },

        {
            answers: ["giorgos printezis", "printezis"],
            image: "images/basketball-players/giorgos-printezis.jpg",
            difficulty: "medium"
        },

        {
            answers: ["sasha vezenkov", "vezenkov"],
            image: "images/basketball-players/sasha-vezenkov.jpg",
            difficulty: "easy"
        },

        {
            answers: ["nikola milutinov", "milutinov"],
            image: "images/basketball-players/nikola-milutinov.jpg",
            difficulty: "medium"
        },

        {
            answers: ["thomas walkup", "walkup"],
            image: "images/basketball-players/thomas-walkup.jpg",
            difficulty: "medium"
        },

        {
            answers: ["alec peters", "peters"],
            image: "images/basketball-players/alec-peters.jpg",
            difficulty: "hard"
        },

        {
            answers: ["shaquielle mckissic", "mckissic"],
            image: "images/basketball-players/shaquielle-mckissic.jpg",
            difficulty: "hard"
        },

        {
            answers: ["kendrick nunn", "nunn"],
            image: "images/basketball-players/kendrick-nunn.jpg",
            difficulty: "easy"
        },

        {
            answers: ["kostas sloukas", "sloukas"],
            image: "images/basketball-players/kostas-sloukas.jpg",
            difficulty: "medium"
        },

        {
            answers: ["mathias lessort", "lessort"],
            image: "images/basketball-players/mathias-lessort.jpg",
            difficulty: "medium"
        },

        {
            answers: ["dimitris diamantidis", "diamantidis"],
            image: "images/basketball-players/dimitris-diamantidis.jpg",
            difficulty: "medium"
        },

        {
            answers: ["mike james", "james"],
            image: "images/basketball-players/mike-james.jpg",
            difficulty: "medium"
        },

        {
            answers: ["walter tavares", "tavares"],
            image: "images/basketball-players/walter-tavares.jpg",
            difficulty: "medium"
        },

        {
            answers: ["facundo campazzo", "campazzo"],
            image: "images/basketball-players/facundo-campazzo.jpg",
            difficulty: "medium"
        },

        {
            answers: ["sergio llull", "llull"],
            image: "images/basketball-players/sergio-llull.jpg",
            difficulty: "medium"
        },

        {
            answers: ["shane larkin", "larkin"],
            image: "images/basketball-players/shane-larkin.jpg",
            difficulty: "medium"
        },

        {
            answers: ["nikola mirotic", "mirotic"],
            image: "images/basketball-players/nikola-mirotic.jpg",
            difficulty: "medium"
        },

        {
            answers: ["milos teodosic", "teodosic"],
            image: "images/basketball-players/milos-teodosic.jpg",
            difficulty: "medium"
        },

        {
            answers: ["sarunas jasikevicius", "jasikevicius"],
            image: "images/basketball-players/sarunas-jasikevicius.jpg",
            difficulty: "hard"
        },

        {
            answers: ["anthony parker", "parker"],
            image: "images/basketball-players/anthony-parker.jpg",
            difficulty: "hard"
        },

        {
            answers: ["oscar schmidt", "schmidt"],
            image: "images/basketball-players/oscar-schmidt.jpg",
            difficulty: "hard"
        },

        {
            answers: ["ramunas siskauskas", "siskauskas"],
            image: "images/basketball-players/ramunas-siskauskas.jpg",
            difficulty: "hard"
        },

        {
            answers: ["dejan bodiroga", "bodiroga"],
            image: "images/basketball-players/dejan-bodiroga.jpg",
            difficulty: "hard"
        },

        {
            answers: ["nando de colo", "de colo"],
            image: "images/basketball-players/nando-de-colo.jpg",
            difficulty: "hard"
        },

        {
            answers: ["jrue holiday", "holiday"],
            image: "images/basketball-players/jrue-holiday.jpg",
            difficulty: "medium"
        }

    ],

    brands: [

    {
        answers: ["nike"],
        image: "images/brands/nike.jpg",
        difficulty: "easy"
    },

    {
        answers: ["adidas"],
        image: "images/brands/adidas.jpg",
        difficulty: "easy"
    },

    {
        answers: ["puma"],
        image: "images/brands/puma.jpg",
        difficulty: "easy"
    },

    {
        answers: ["under armour"],
        image: "images/brands/under-armour.jpg",
        difficulty: "medium"
    },

    {
        answers: ["new balance"],
        image: "images/brands/new-balance.jpg",
        difficulty: "easy"
    },

    {
        answers: ["asics"],
        image: "images/brands/asics.jpg",
        difficulty: "medium"
    },

    {
        answers: ["anta"],
        image: "images/brands/anta.jpg",
        difficulty: "medium"
    },

    {
        answers: ["reebok"],
        image: "images/brands/reebok.jpg",
        difficulty: "medium"
    },

    {
        answers: ["converse"],
        image: "images/brands/converse.jpg",
        difficulty: "easy"
    },

    {
        answers: ["vans"],
        image: "images/brands/vans.jpg",
        difficulty: "easy"
    },

    {
        answers: ["gucci"],
        image: "images/brands/gucci.jpg",
        difficulty: "easy"
    },

    {
        answers: ["louis vuitton"],
        image: "images/brands/louis-vuitton.jpg",
        difficulty: "easy"
    },

    {
        answers: ["prada"],
        image: "images/brands/prada.jpg",
        difficulty: "easy"
    },

    {
        answers: ["chanel"],
        image: "images/brands/chanel.jpg",
        difficulty: "easy"
    },

    {
        answers: ["dior"],
        image: "images/brands/dior.jpg",
        difficulty: "easy"
    },

    {
        answers: ["versace"],
        image: "images/brands/versace.jpg",
        difficulty: "easy"
    },

    {
        answers: ["armani"],
        image: "images/brands/armani.jpg",
        difficulty: "medium"
    },

    {
        answers: ["dolce and gabbana"],
        image: "images/brands/dolce-gabbana.jpg",
        difficulty: "medium"
    },

    {
        answers: ["balenciaga"],
        image: "images/brands/balenciaga.jpg",
        difficulty: "easy"
    },

    {
        answers: ["burberry"],
        image: "images/brands/burberry.jpg",
        difficulty: "medium"
    },

    {
        answers: ["fendi"],
        image: "images/brands/fendi.jpg",
        difficulty: "medium"
    },

    {
        answers: ["givenchy"],
        image: "images/brands/givenchy.jpg",
        difficulty: "medium"
    },

    {
        answers: ["valentino"],
        image: "images/brands/valentino.jpg",
        difficulty: "medium"
    },

    {
        answers: ["moncler"],
        image: "images/brands/moncler.jpg",
        difficulty: "medium"
    },

    {
        answers: ["stone island"],
        image: "images/brands/stone-island.jpg",
        difficulty: "medium"
    },

    {
        answers: ["off white"],
        image: "images/brands/off-white.jpg",
        difficulty: "medium"
    },

    {
        answers: ["supreme"],
        image: "images/brands/supreme.jpg",
        difficulty: "easy"
    },

    {
        answers: ["palm angels"],
        image: "images/brands/palm-angels.jpg",
        difficulty: "medium"
    },

    {
        answers: ["fear of god"],
        image: "images/brands/fear-of-god.jpg",
        difficulty: "hard"
    },

    {
        answers: ["tommy hilfiger"],
        image: "images/brands/tommy-hilfiger.jpg",
        difficulty: "easy"
    },

    {
        answers: ["calvin klein"],
        image: "images/brands/calvin-klein.jpg",
        difficulty: "easy"
    },

    {
        answers: ["hugo boss"],
        image: "images/brands/hugo-boss.jpg",
        difficulty: "easy"
    },

    {
        answers: ["ralph lauren"],
        image: "images/brands/ralph-lauren.jpg",
        difficulty: "easy"
    },

    {
        answers: ["lacoste"],
        image: "images/brands/lacoste.jpg",
        difficulty: "easy"
    },

    {
        answers: ["cartier"],
        image: "images/brands/cartier.jpg",
        difficulty: "medium"
    },

    {
        answers: ["rolex"],
        image: "images/brands/rolex.jpg",
        difficulty: "easy"
    },

    {
        answers: ["tiffany and co"],
        image: "images/brands/tiffany-co.jpg",
        difficulty: "medium"
    },

    {
        answers: ["hermes"],
        image: "images/brands/hermes.jpg",
        difficulty: "medium"
    },

    {
        answers: ["omega"],
        image: "images/brands/omega.jpg",
        difficulty: "medium"
    },

    {
        answers: ["tag heuer"],
        image: "images/brands/tag-heuer.jpg",
        difficulty: "medium"
    },

    {
        answers: ["apple"],
        image: "images/brands/apple.jpg",
        difficulty: "easy"
    },

    {
        answers: ["samsung"],
        image: "images/brands/samsung.jpg",
        difficulty: "easy"
    },

    {
        answers: ["google"],
        image: "images/brands/google.jpg",
        difficulty: "easy"
    },

    {
        answers: ["microsoft"],
        image: "images/brands/microsoft.jpg",
        difficulty: "easy"
    },

    {
        answers: ["intel"],
        image: "images/brands/intel.jpg",
        difficulty: "easy"
    },

    {
        answers: ["amd"],
        image: "images/brands/amd.jpg",
        difficulty: "medium"
    },

    {
        answers: ["nvidia"],
        image: "images/brands/nvidia.jpg",
        difficulty: "easy"
    },

    {
        answers: ["huawei"],
        image: "images/brands/huawei.jpg",
        difficulty: "medium"
    },

    {
        answers: ["xiaomi"],
        image: "images/brands/xiaomi.jpg",
        difficulty: "easy"
    },

    {
        answers: ["oneplus"],
        image: "images/brands/oneplus.jpg",
        difficulty: "medium"
    },

    {
        answers: ["playstation"],
        image: "images/brands/playstation.jpg",
        difficulty: "easy"
    },

    {
        answers: ["xbox"],
        image: "images/brands/xbox.jpg",
        difficulty: "easy"
    },

    {
        answers: ["nintendo"],
        image: "images/brands/nintendo.jpg",
        difficulty: "easy"
    },

    {
        answers: ["steam"],
        image: "images/brands/steam.jpg",
        difficulty: "easy"
    },

    {
        answers: ["epic games"],
        image: "images/brands/epic-games.jpg",
        difficulty: "medium"
    },

    {
        answers: ["rockstar games"],
        image: "images/brands/rockstar-games.jpg",
        difficulty: "easy"
    },

    {
        answers: ["ubisoft"],
        image: "images/brands/ubisoft.jpg",
        difficulty: "medium"
    },

    {
        answers: ["ea sports"],
        image: "images/brands/ea-sports.jpg",
        difficulty: "easy"
    },

    {
        answers: ["riot games"],
        image: "images/brands/riot-games.jpg",
        difficulty: "medium"
    },

    {
        answers: ["blizzard"],
        image: "images/brands/blizzard.jpg",
        difficulty: "medium"
    },

    {
        answers: ["netflix"],
        image: "images/brands/netflix.jpg",
        difficulty: "easy"
    },

    {
        answers: ["disney plus"],
        image: "images/brands/disney-plus.jpg",
        difficulty: "easy"
    },

    {
        answers: ["youtube"],
        image: "images/brands/youtube.jpg",
        difficulty: "easy"
    },

    {
        answers: ["instagram"],
        image: "images/brands/instagram.jpg",
        difficulty: "easy"
    },

    {
        answers: ["tiktok"],
        image: "images/brands/tiktok.jpg",
        difficulty: "easy"
    },

    {
        answers: ["spotify"],
        image: "images/brands/spotify.jpg",
        difficulty: "easy"
    },

    {
        answers: ["twitch"],
        image: "images/brands/twitch.jpg",
        difficulty: "medium"
    },

    {
        answers: ["amazon"],
        image: "images/brands/amazon.jpg",
        difficulty: "easy"
    },

    {
        answers: ["ikea"],
        image: "images/brands/ikea.jpg",
        difficulty: "easy"
    },

    {
        answers: ["lego"],
        image: "images/brands/lego.jpg",
        difficulty: "easy"
    },

    {
        answers: ["coca cola"],
        image: "images/brands/coca-cola.jpg",
        difficulty: "easy"
    },

    {
        answers: ["pepsi"],
        image: "images/brands/pepsi.jpg",
        difficulty: "easy"
    },

    {
        answers: ["red bull"],
        image: "images/brands/red-bull.jpg",
        difficulty: "easy"
    },

    {
        answers: ["monster energy"],
        image: "images/brands/monster-energy.jpg",
        difficulty: "medium"
    },

    {
        answers: ["starbucks"],
        image: "images/brands/starbucks.jpg",
        difficulty: "easy"
    },

    {
        answers: ["mcdonalds"],
        image: "images/brands/mcdonalds.jpg",
        difficulty: "easy"
    },

    {
        answers: ["kfc"],
        image: "images/brands/kfc.jpg",
        difficulty: "easy"
    },

    {
        answers: ["burger king"],
        image: "images/brands/burger-king.jpg",
        difficulty: "easy"
    },

    {
        answers: ["dominos"],
        image: "images/brands/dominos.jpg",
        difficulty: "easy"
    },

    {
        answers: ["pizza hut"],
        image: "images/brands/pizza-hut.jpg",
        difficulty: "easy"
    }

],

"famous-people": [

    {
        answers: ["albert einstein", "einstein"],
        image: "images/famous-people/albert-einstein.jpg",
        difficulty: "easy"
    },

    {
        answers: ["isaac newton", "newton"],
        image: "images/famous-people/isaac-newton.jpg",
        difficulty: "easy"
    },

    {
        answers: ["nikola tesla", "tesla"],
        image: "images/famous-people/nikola-tesla.jpg",
        difficulty: "easy"
    },

    {
        answers: ["stephen hawking", "hawking"],
        image: "images/famous-people/stephen-hawking.jpg",
        difficulty: "easy"
    },

    {
        answers: ["marie curie", "curie"],
        image: "images/famous-people/marie-curie.jpg",
        difficulty: "medium"
    },

    {
        answers: ["galileo galilei", "galilei"],
        image: "images/famous-people/galileo-galilei.jpg",
        difficulty: "medium"
    },

    {
        answers: ["charles darwin", "darwin"],
        image: "images/famous-people/charles-darwin.jpg",
        difficulty: "medium"
    },

    {
        answers: ["leonardo da vinci", "da vinci"],
        image: "images/famous-people/leonardo-da-vinci.jpg",
        difficulty: "easy"
    },

    {
        answers: ["elon musk", "musk"],
        image: "images/famous-people/elon-musk.jpg",
        difficulty: "easy"
    },

    {
        answers: ["bill gates", "gates"],
        image: "images/famous-people/bill-gates.jpg",
        difficulty: "easy"
    },

    {
        answers: ["steve jobs", "jobs"],
        image: "images/famous-people/steve-jobs.jpg",
        difficulty: "easy"
    },

    {
        answers: ["mark zuckerberg", "zuckerberg"],
        image: "images/famous-people/mark-zuckerberg.jpg",
        difficulty: "easy"
    },

    {
        answers: ["jeff bezos", "bezos"],
        image: "images/famous-people/jeff-bezos.jpg",
        difficulty: "easy"
    },

    {
        answers: ["warren buffett", "buffett"],
        image: "images/famous-people/warren-buffett.jpg",
        difficulty: "medium"
    },

    {
        answers: ["tim cook", "cook"],
        image: "images/famous-people/tim-cook.jpg",
        difficulty: "medium"
    },

    {
        answers: ["barack obama", "obama"],
        image: "images/famous-people/barack-obama.jpg",
        difficulty: "easy"
    },

    {
        answers: ["donald trump", "trump"],
        image: "images/famous-people/donald-trump.jpg",
        difficulty: "easy"
    },

    {
        answers: ["joe biden", "biden"],
        image: "images/famous-people/joe-biden.jpg",
        difficulty: "easy"
    },

    {
        answers: ["vladimir putin", "putin"],
        image: "images/famous-people/vladimir-putin.jpg",
        difficulty: "easy"
    },

    {
        answers: ["volodymyr zelenskyy", "zelenskyy"],
        image: "images/famous-people/volodymyr-zelenskyy.jpg",
        difficulty: "medium"
    },

    {
        answers: ["queen elizabeth ii", "queen elizabeth", "elizabeth"],
        image: "images/famous-people/queen-elizabeth-ii.jpg",
        difficulty: "easy"
    },

    {
        answers: ["king charles iii", "king charles", "charles"],
        image: "images/famous-people/king-charles-iii.jpg",
        difficulty: "medium"
    },

    {
        answers: ["mahatma gandhi", "gandhi"],
        image: "images/famous-people/mahatma-gandhi.jpg",
        difficulty: "medium"
    },

    {
        answers: ["nelson mandela", "mandela"],
        image: "images/famous-people/nelson-mandela.jpg",
        difficulty: "medium"
    },

    {
        answers: ["martin luther king jr", "martin luther king"],
        image: "images/famous-people/martin-luther-king-jr.jpg",
        difficulty: "medium"
    },

    {
        answers: ["napoleon bonaparte", "napoleon"],
        image: "images/famous-people/napoleon-bonaparte.jpg",
        difficulty: "medium"
    },

    {
        answers: ["julius caesar", "caesar"],
        image: "images/famous-people/julius-caesar.jpg",
        difficulty: "medium"
    },

    {
        answers: ["cleopatra"],
        image: "images/famous-people/cleopatra.jpg",
        difficulty: "medium"
    },

    {
        answers: ["christopher columbus", "columbus"],
        image: "images/famous-people/christopher-columbus.jpg",
        difficulty: "medium"
    },

    {
        answers: ["neil armstrong", "armstrong"],
        image: "images/famous-people/neil-armstrong.jpg",
        difficulty: "easy"
    },

    {
        answers: ["buzz aldrin", "aldrin"],
        image: "images/famous-people/buzz-aldrin.jpg",
        difficulty: "medium"
    },

    {
        answers: ["yuri gagarin", "gagarin"],
        image: "images/famous-people/yuri-gagarin.jpg",
        difficulty: "medium"
    },

    {
        answers: ["mrbeast"],
        image: "images/famous-people/mrbeast.jpg",
        difficulty: "easy"
    },

    {
        answers: ["ksi"],
        image: "images/famous-people/ksi.jpg",
        difficulty: "medium"
    },

    {
        answers: ["logan paul", "paul"],
        image: "images/famous-people/logan-paul.jpg",
        difficulty: "medium"
    },

    {
        answers: ["jake paul", "paul"],
        image: "images/famous-people/jake-paul.jpg",
        difficulty: "medium"
    },

    {
        answers: ["khaby lame", "lame"],
        image: "images/famous-people/khaby-lame.jpg",
        difficulty: "easy"
    },

    {
        answers: ["kim kardashian", "kardashian"],
        image: "images/famous-people/kim-kardashian.jpg",
        difficulty: "medium"
    },

    {
        answers: ["princess diana", "diana"],
        image: "images/famous-people/princess-diana.jpg",
        difficulty: "easy"
    },

    {
        answers: ["mother teresa", "teresa"],
        image: "images/famous-people/mother-teresa.jpg",
        difficulty: "medium"
    },

    {
        answers: ["sigmund freud", "freud"],
        image: "images/famous-people/sigmund-freud.jpg",
        difficulty: "medium"
    },

    {
        answers: ["pablo picasso", "picasso"],
        image: "images/famous-people/pablo-picasso.jpg",
        difficulty: "medium"
    },

    {
        answers: ["vincent van gogh", "van gogh"],
        image: "images/famous-people/vincent-van-gogh.jpg",
        difficulty: "medium"
    },

    {
        answers: ["wolfgang amadeus mozart", "mozart"],
        image: "images/famous-people/wolfgang-amadeus-mozart.jpg",
        difficulty: "medium"
    },

    {
        answers: ["ludwig van beethoven", "beethoven"],
        image: "images/famous-people/ludwig-van-beethoven.jpg",
        difficulty: "medium"
    },

    {
        answers: ["william shakespeare", "shakespeare"],
        image: "images/famous-people/william-shakespeare.jpg",
        difficulty: "medium"
    },

    {
        answers: ["johannes gutenberg", "gutenberg"],
        image: "images/famous-people/johannes-gutenberg.jpg",
        difficulty: "hard"
    },

    {
        answers: ["ferdinand magellan", "magellan"],
        image: "images/famous-people/ferdinand-magellan.jpg",
        difficulty: "hard"
    },

    {
        answers: ["marco polo", "polo"],
        image: "images/famous-people/marco-polo.jpg",
        difficulty: "medium"
    },

    {
        answers: ["thomas edison", "edison"],
        image: "images/famous-people/thomas-edison.jpg",
        difficulty: "easy"
    },

    {
        answers: ["alexander graham bell", "bell"],
        image: "images/famous-people/alexander-graham-bell.jpg",
        difficulty: "medium"
    },

    {
        answers: ["wright brothers", "wright"],
        image: "images/famous-people/wright-brothers.jpg",
        difficulty: "hard"
    },

    {
        answers: ["alan turing", "turing"],
        image: "images/famous-people/alan-turing.jpg",
        difficulty: "medium"
    },

    {
        answers: ["edward snowden", "snowden"],
        image: "images/famous-people/edward-snowden.jpg",
        difficulty: "hard"
    },

    {
        answers: ["julian assange", "assange"],
        image: "images/famous-people/julian-assange.jpg",
        difficulty: "hard"
    },

    {
        answers: ["george washington", "washington"],
        image: "images/famous-people/george-washington.jpg",
        difficulty: "medium"
    },

    {
        answers: ["abraham lincoln", "lincoln"],
        image: "images/famous-people/abraham-lincoln.jpg",
        difficulty: "medium"
    },

    {
        answers: ["john f kennedy", "kennedy"],
        image: "images/famous-people/john-f-kennedy.jpg",
        difficulty: "medium"
    },

    {
        answers: ["winston churchill", "churchill"],
        image: "images/famous-people/winston-churchill.jpg",
        difficulty: "medium"
    },

    {
        answers: ["margaret thatcher", "thatcher"],
        image: "images/famous-people/margaret-thatcher.jpg",
        difficulty: "medium"
    },

    {
        answers: ["pope francis", "francis"],
        image: "images/famous-people/pope-francis.jpg",
        difficulty: "easy"
    },

    {
        answers: ["dalai lama", "lama"],
        image: "images/famous-people/dalai-lama.jpg",
        difficulty: "medium"
    },

    {
        answers: ["osama bin laden", "bin laden"],
        image: "images/famous-people/osama-bin-laden.jpg",
        difficulty: "hard"
    },

    {
        answers: ["che guevara", "guevara"],
        image: "images/famous-people/che-guevara.jpg",
        difficulty: "medium"
    },

    {
        answers: ["julian alvarez", "alvarez"],
        image: "images/famous-people/julian-alvarez.jpg",
        difficulty: "hard"
    },

    {
        answers: ["andrew tate", "tate"],
        image: "images/famous-people/andrew-tate.jpg",
        difficulty: "easy"
    },

    {
        answers: ["greta thunberg", "thunberg"],
        image: "images/famous-people/greta-thunberg.jpg",
        difficulty: "medium"
    },

    {
        answers: ["markiplier"],
        image: "images/famous-people/markiplier.jpg",
        difficulty: "medium"
    },

    {
        answers: ["pewdiepie"],
        image: "images/famous-people/pewdiepie.jpg",
        difficulty: "easy"
    },

    {
        answers: ["ishowspeed", "speed"],
        image: "images/famous-people/ishowspeed.jpg",
        difficulty: "easy"
    },

    {
        answers: ["kai cenat", "cenat"],
        image: "images/famous-people/kai-cenat.jpg",
        difficulty: "easy"
    },

    {
        answers: ["xqc"],
        image: "images/famous-people/xqc.jpg",
        difficulty: "hard"
    },

    {
        answers: ["charli damelio", "damelio"],
        image: "images/famous-people/charli-damelio.jpg",
        difficulty: "medium"
    },

    {
        answers: ["addison rae", "rae"],
        image: "images/famous-people/addison-rae.jpg",
        difficulty: "medium"
    },

    {
        answers: ["george soros", "soros"],
        image: "images/famous-people/george-soros.jpg",
        difficulty: "hard"
    },

    {
        answers: ["bernard arnault", "arnault"],
        image: "images/famous-people/bernard-arnault.jpg",
        difficulty: "hard"
    },

    {
        answers: ["sam altman", "altman"],
        image: "images/famous-people/sam-altman.jpg",
        difficulty: "medium"
    },

    {
        answers: ["jensen huang", "huang"],
        image: "images/famous-people/jensen-huang.jpg",
        difficulty: "medium"
    },

    {
        answers: ["larry page", "page"],
        image: "images/famous-people/larry-page.jpg",
        difficulty: "hard"
    },

    {
        answers: ["sergey brin", "brin"],
        image: "images/famous-people/sergey-brin.jpg",
        difficulty: "hard"
    }

],


singers: [

    {
        answers: ["michael jackson", "jackson"],
        image: "images/singers/michael-jackson.jpg",
        difficulty: "easy"
    },

    {
        answers: ["elvis presley", "presley"],
        image: "images/singers/elvis-presley.jpg",
        difficulty: "easy"
    },

    {
        answers: ["freddie mercury", "mercury"],
        image: "images/singers/freddie-mercury.jpg",
        difficulty: "easy"
    },

    {
        answers: ["madonna"],
        image: "images/singers/madonna.jpg",
        difficulty: "easy"
    },

    {
        answers: ["adele"],
        image: "images/singers/adele.jpg",
        difficulty: "easy"
    },

    {
        answers: ["taylor swift", "swift"],
        image: "images/singers/taylor-swift.jpg",
        difficulty: "easy"
    },

    {
        answers: ["ed sheeran", "sheeran"],
        image: "images/singers/ed-sheeran.jpg",
        difficulty: "easy"
    },

    {
        answers: ["justin bieber", "bieber"],
        image: "images/singers/justin-bieber.jpg",
        difficulty: "easy"
    },

    {
        answers: ["bruno mars", "mars"],
        image: "images/singers/bruno-mars.jpg",
        difficulty: "easy"
    },

    {
        answers: ["the weeknd", "weeknd"],
        image: "images/singers/the-weeknd.jpg",
        difficulty: "easy"
    },

    {
        answers: ["billie eilish", "eilish"],
        image: "images/singers/billie-eilish.jpg",
        difficulty: "easy"
    },

    {
        answers: ["dua lipa", "lipa"],
        image: "images/singers/dua-lipa.jpg",
        difficulty: "easy"
    },

    {
        answers: ["ariana grande", "grande"],
        image: "images/singers/ariana-grande.jpg",
        difficulty: "easy"
    },

    {
        answers: ["selena gomez", "gomez"],
        image: "images/singers/selena-gomez.jpg",
        difficulty: "easy"
    },

    {
        answers: ["rihanna"],
        image: "images/singers/rihanna.jpg",
        difficulty: "easy"
    },

    {
        answers: ["beyonce"],
        image: "images/singers/beyonce.jpg",
        difficulty: "easy"
    },

    {
        answers: ["lady gaga", "gaga"],
        image: "images/singers/lady-gaga.jpg",
        difficulty: "easy"
    },

    {
        answers: ["katy perry", "perry"],
        image: "images/singers/katy-perry.jpg",
        difficulty: "easy"
    },

    {
        answers: ["shakira"],
        image: "images/singers/shakira.jpg",
        difficulty: "easy"
    },

    {
        answers: ["sia"],
        image: "images/singers/sia.jpg",
        difficulty: "medium"
    },

    {
        answers: ["post malone", "malone"],
        image: "images/singers/post-malone.jpg",
        difficulty: "easy"
    },

    {
        answers: ["drake"],
        image: "images/singers/drake.jpg",
        difficulty: "easy"
    },

    {
        answers: ["eminem"],
        image: "images/singers/eminem.jpg",
        difficulty: "easy"
    },

    {
        answers: ["kendrick lamar", "lamar"],
        image: "images/singers/kendrick-lamar.jpg",
        difficulty: "medium"
    },

    {
        answers: ["travis scott", "scott"],
        image: "images/singers/travis-scott.jpg",
        difficulty: "easy"
    },

    {
        answers: ["21 savage", "savage"],
        image: "images/singers/21-savage.jpg",
        difficulty: "medium"
    },

    {
        answers: ["future"],
        image: "images/singers/future.jpg",
        difficulty: "medium"
    },

    {
        answers: ["lil wayne", "wayne"],
        image: "images/singers/lil-wayne.jpg",
        difficulty: "medium"
    },

    {
        answers: ["snoop dogg", "snoop"],
        image: "images/singers/snoop-dogg.jpg",
        difficulty: "easy"
    },

    {
        answers: ["ice cube"],
        image: "images/singers/ice-cube.jpg",
        difficulty: "medium"
    },

    {
        answers: ["50 cent"],
        image: "images/singers/50-cent.jpg",
        difficulty: "medium"
    },

    {
        answers: ["usher"],
        image: "images/singers/usher.jpg",
        difficulty: "medium"
    },

    {
        answers: ["chris brown", "brown"],
        image: "images/singers/chris-brown.jpg",
        difficulty: "easy"
    },

    {
        answers: ["pitbull"],
        image: "images/singers/pitbull.jpg",
        difficulty: "easy"
    },

    {
        answers: ["jennifer lopez", "lopez"],
        image: "images/singers/jennifer-lopez.jpg",
        difficulty: "easy"
    },

    {
        answers: ["enrique iglesias", "iglesias"],
        image: "images/singers/enrique-iglesias.jpg",
        difficulty: "medium"
    },

    {
        answers: ["charlie puth", "puth"],
        image: "images/singers/charlie-puth.jpg",
        difficulty: "medium"
    },

    {
        answers: ["shawn mendes", "mendes"],
        image: "images/singers/shawn-mendes.jpg",
        difficulty: "medium"
    },

    {
        answers: ["olivia rodrigo", "rodrigo"],
        image: "images/singers/olivia-rodrigo.jpg",
        difficulty: "easy"
    },

    {
        answers: ["sabrina carpenter", "carpenter"],
        image: "images/singers/sabrina-carpenter.jpg",
        difficulty: "easy"
    },

    {
        answers: ["harry styles", "styles"],
        image: "images/singers/harry-styles.jpg",
        difficulty: "easy"
    },

    {
        answers: ["zayn malik", "zayn", "malik"],
        image: "images/singers/zayn-malik.jpg",
        difficulty: "medium"
    },

    {
        answers: ["louis tomlinson", "tomlinson"],
        image: "images/singers/louis-tomlinson.jpg",
        difficulty: "medium"
    },

    {
        answers: ["niall horan", "horan"],
        image: "images/singers/niall-horan.jpg",
        difficulty: "medium"
    },

    {
        answers: ["liam payne", "payne"],
        image: "images/singers/liam-payne.jpg",
        difficulty: "medium"
    },

    {
        answers: ["camila cabello", "cabello"],
        image: "images/singers/camila-cabello.jpg",
        difficulty: "easy"
    },

    {
        answers: ["halsey"],
        image: "images/singers/halsey.jpg",
        difficulty: "medium"
    },

    {
        answers: ["doja cat"],
        image: "images/singers/doja-cat.jpg",
        difficulty: "easy"
    },

    {
        answers: ["lizzo"],
        image: "images/singers/lizzo.jpg",
        difficulty: "medium"
    },

    {
        answers: ["ava max"],
        image: "images/singers/ava-max.jpg",
        difficulty: "easy"
    },

    {
        answers: ["bebe rexha", "rexha"],
        image: "images/singers/bebe-rexha.jpg",
        difficulty: "medium"
    },

    {
        answers: ["alan walker", "walker"],
        image: "images/singers/alan-walker.jpg",
        difficulty: "medium"
    },

    {
        answers: ["avicii"],
        image: "images/singers/avicii.jpg",
        difficulty: "easy"
    },

    {
        answers: ["david guetta", "guetta"],
        image: "images/singers/david-guetta.jpg",
        difficulty: "easy"
    },

    {
        answers: ["martin garrix", "garrix"],
        image: "images/singers/martin-garrix.jpg",
        difficulty: "easy"
    },

    {
        answers: ["marshmello"],
        image: "images/singers/marshmello.jpg",
        difficulty: "easy"
    },

    {
        answers: ["calvin harris", "harris"],
        image: "images/singers/calvin-harris.jpg",
        difficulty: "medium"
    },

    {
        answers: ["steve aoki", "aoki"],
        image: "images/singers/steve-aoki.jpg",
        difficulty: "medium"
    },

    {
        answers: ["dj snake", "snake"],
        image: "images/singers/dj-snake.jpg",
        difficulty: "medium"
    },

    {
        answers: ["tiesto"],
        image: "images/singers/tiesto.jpg",
        difficulty: "medium"
    },

    {
        answers: ["bono"],
        image: "images/singers/bono.jpg",
        difficulty: "hard"
    },

    {
        answers: ["sting"],
        image: "images/singers/sting.jpg",
        difficulty: "hard"
    },

    {
        answers: ["phil collins", "collins"],
        image: "images/singers/phil-collins.jpg",
        difficulty: "medium"
    },

    {
        answers: ["george michael", "michael"],
        image: "images/singers/george-michael.jpg",
        difficulty: "medium"
    },

    {
        answers: ["whitney houston", "houston"],
        image: "images/singers/whitney-houston.jpg",
        difficulty: "medium"
    },

    {
        answers: ["celine dion", "dion"],
        image: "images/singers/celine-dion.jpg",
        difficulty: "medium"
    },

    {
        answers: ["mariah carey", "carey"],
        image: "images/singers/mariah-carey.jpg",
        difficulty: "medium"
    },

    {
        answers: ["janet jackson", "janet", "jackson"],
        image: "images/singers/janet-jackson.jpg",
        difficulty: "medium"
    },

    {
        answers: ["prince"],
        image: "images/singers/prince.jpg",
        difficulty: "medium"
    },

    {
        answers: ["bob marley", "marley"],
        image: "images/singers/bob-marley.jpg",
        difficulty: "easy"
    },

    {
        answers: ["john lennon", "lennon"],
        image: "images/singers/john-lennon.jpg",
        difficulty: "medium"
    },

    {
        answers: ["paul mccartney", "mccartney"],
        image: "images/singers/paul-mccartney.jpg",
        difficulty: "medium"
    },

    {
        answers: ["ozzy osbourne", "osbourne"],
        image: "images/singers/ozzy-osbourne.jpg",
        difficulty: "medium"
    },

    {
        answers: ["kurt cobain", "cobain"],
        image: "images/singers/kurt-cobain.jpg",
        difficulty: "medium"
    },

    {
        answers: ["chester bennington", "bennington"],
        image: "images/singers/chester-bennington.jpg",
        difficulty: "medium"
    },

    {
        answers: ["amy winehouse", "winehouse"],
        image: "images/singers/amy-winehouse.jpg",
        difficulty: "medium"
    },

    {
        answers: ["tina turner", "turner"],
        image: "images/singers/tina-turner.jpg",
        difficulty: "medium"
    },

    {
        answers: ["rod stewart", "stewart"],
        image: "images/singers/rod-stewart.jpg",
        difficulty: "hard"
    },

    {
        answers: ["seal"],
        image: "images/singers/seal.jpg",
        difficulty: "hard"
    },

    {
        answers: ["lenny kravitz", "kravitz"],
        image: "images/singers/lenny-kravitz.jpg",
        difficulty: "medium"
    }

],

"star-wars": [

{
    answers: ["luke skywalker", "skywalker"],
    image: "images/star-wars/luke-skywalker.jpg",
    difficulty: "easy"
},

{
    answers: ["darth vader", "vader"],
    image: "images/star-wars/darth-vader.jpg",
    difficulty: "easy"
},

{
    answers: ["anakin skywalker", "anakin"],
    image: "images/star-wars/anakin-skywalker.jpg",
    difficulty: "easy"
},

{
    answers: ["obi wan kenobi", "obi wan", "kenobi"],
    image: "images/star-wars/obi-wan-kenobi.jpg",
    difficulty: "easy"
},

{
    answers: ["yoda"],
    image: "images/star-wars/yoda.jpg",
    difficulty: "easy"
},

{
    answers: ["emperor palpatine", "palpatine", "darth sidious", "sidious"],
    image: "images/star-wars/palpatine.jpg",
    difficulty: "easy"
},

{
    answers: ["han solo", "solo"],
    image: "images/star-wars/han-solo.jpg",
    difficulty: "easy"
},

{
    answers: ["princess leia", "leia organa", "leia"],
    image: "images/star-wars/princess-leia.jpg",
    difficulty: "easy"
},

{
    answers: ["chewbacca", "chewie"],
    image: "images/star-wars/chewbacca.jpg",
    difficulty: "easy"
},

{
    answers: ["r2 d2", "r2d2"],
    image: "images/star-wars/r2-d2.jpg",
    difficulty: "easy"
},

{
    answers: ["c3po", "c 3po", "c3 po"],
    image: "images/star-wars/c3po.jpg",
    difficulty: "easy"
},

{
    answers: ["bb8", "bb 8"],
    image: "images/star-wars/bb8.jpg",
    difficulty: "easy"
},

{
    answers: ["rey"],
    image: "images/star-wars/rey.jpg",
    difficulty: "easy"
},

{
    answers: ["kylo ren", "kylo"],
    image: "images/star-wars/kylo-ren.jpg",
    difficulty: "easy"
},

{
    answers: ["darth maul", "maul"],
    image: "images/star-wars/darth-maul.jpg",
    difficulty: "easy"
},

{
    answers: ["general grievous", "grievous"],
    image: "images/star-wars/general-grievous.jpg",
    difficulty: "easy"
},

{
    answers: ["boba fett", "boba"],
    image: "images/star-wars/boba-fett.jpg",
    difficulty: "easy"
},

{
    answers: ["the mandalorian", "din djarin", "mandalorian"],
    image: "images/star-wars/the-mandalorian.jpg",
    difficulty: "easy"
},

{
    answers: ["grogu", "baby yoda"],
    image: "images/star-wars/grogu.jpg",
    difficulty: "easy"
},

{
    answers: ["jabba the hutt", "jabba"],
    image: "images/star-wars/jabba-the-hutt.jpg",
    difficulty: "easy"
},

{
    answers: ["stormtrooper"],
    image: "images/star-wars/stormtrooper.jpg",
    difficulty: "easy"
},

{
    answers: ["finn"],
    image: "images/star-wars/finn.jpg",
    difficulty: "medium"
},

{
    answers: ["poe dameron", "poe"],
    image: "images/star-wars/poe-dameron.jpg",
    difficulty: "medium"
},

{
    answers: ["padme amidala", "padme"],
    image: "images/star-wars/padme-amidala.jpg",
    difficulty: "medium"
},

{
    answers: ["mace windu", "windu"],
    image: "images/star-wars/mace-windu.jpg",
    difficulty: "medium"
},

{
    answers: ["qui gon jinn", "qui gon"],
    image: "images/star-wars/qui-gon-jinn.jpg",
    difficulty: "medium"
},

{
    answers: ["count dooku", "dooku"],
    image: "images/star-wars/count-dooku.jpg",
    difficulty: "medium"
},

{
    answers: ["jango fett", "jango"],
    image: "images/star-wars/jango-fett.jpg",
    difficulty: "medium"
},

{
    answers: ["ahsoka tano", "ahsoka"],
    image: "images/star-wars/ahsoka-tano.jpg",
    difficulty: "medium"
},

{
    answers: ["captain rex", "rex"],
    image: "images/star-wars/captain-rex.jpg",
    difficulty: "medium"
},

{
    answers: ["commander cody", "cody"],
    image: "images/star-wars/commander-cody.jpg",
    difficulty: "medium"
},

{
    answers: ["clone trooper"],
    image: "images/star-wars/clone-trooper.jpg",
    difficulty: "medium"
},

{
    answers: ["royal guard", "imperial royal guard"],
    image: "images/star-wars/royal-guard.jpg",
    difficulty: "medium"
},

{
    answers: ["lando calrissian", "lando"],
    image: "images/star-wars/lando-calrissian.jpg",
    difficulty: "medium"
},

{
    answers: ["admiral ackbar", "ackbar"],
    image: "images/star-wars/admiral-ackbar.jpg",
    difficulty: "medium"
},

{
    answers: ["moff gideon", "gideon"],
    image: "images/star-wars/moff-gideon.jpg",
    difficulty: "medium"
},

{
    answers: ["wicket"],
    image: "images/star-wars/wicket.jpg",
    difficulty: "medium"
},

{
    answers: ["cad bane"],
    image: "images/star-wars/cad-bane.jpg",
    difficulty: "hard"
},

{
    answers: ["asajj ventress", "ventress"],
    image: "images/star-wars/asajj-ventress.jpg",
    difficulty: "hard"
},

{
    answers: ["grand admiral thrawn", "thrawn"],
    image: "images/star-wars/grand-admiral-thrawn.jpg",
    difficulty: "hard"
},

{
    answers: ["ezra bridger", "ezra"],
    image: "images/star-wars/ezra-bridger.jpg",
    difficulty: "hard"
},

{
    answers: ["sabine wren", "sabine"],
    image: "images/star-wars/sabine-wren.jpg",
    difficulty: "hard"
},

{
    answers: ["kanan jarrus", "kanan"],
    image: "images/star-wars/kanan-jarrus.jpg",
    difficulty: "hard"
},

{
    answers: ["hera syndulla", "hera"],
    image: "images/star-wars/hera-syndulla.jpg",
    difficulty: "hard"
},

{
    answers: ["chopper", "c1 10p", "c1-10p"],
    image: "images/star-wars/chopper.jpg",
    difficulty: "hard"
},

{
    answers: ["director krennic", "krennic"],
    image: "images/star-wars/director-krennic.jpg",
    difficulty: "hard"
},

{
    answers: ["ig11", "ig-11"],
    image: "images/star-wars/ig-11.jpg",
    difficulty: "hard"
},

{
    answers: ["captain phasma", "phasma"],
    image: "images/star-wars/captain-phasma.jpg",
    difficulty: "hard"
},

{
    answers: ["nute gunray", "gunray"],
    image: "images/star-wars/nute-gunray.jpg",
    difficulty: "hard"
},

{
    answers: ["wat tambor", "tambor"],
    image: "images/star-wars/wat-tambor.jpg",
    difficulty: "hard"
},

{
    answers: ["plo koon"],
    image: "images/star-wars/plo-koon.jpg",
    difficulty: "hard"
},

{
    answers: ["kit fisto"],
    image: "images/star-wars/kit-fisto.jpg",
    difficulty: "hard"
},

{
    answers: ["ki adi mundi", "ki-adi-mundi"],
    image: "images/star-wars/ki-adi-mundi.jpg",
    difficulty: "hard"
},

{
    answers: ["shaak ti"],
    image: "images/star-wars/shaak-ti.jpg",
    difficulty: "hard"
},

{
    answers: ["savage opress", "opress"],
    image: "images/star-wars/savage-opress.jpg",
    difficulty: "hard"
},

{
    answers: ["mother talzin", "talzin"],
    image: "images/star-wars/mother-talzin.jpg",
    difficulty: "hard"
},

{
    answers: ["bail organa", "bail"],
    image: "images/star-wars/bail-organa.jpg",
    difficulty: "hard"
},

{
    answers: ["millennium falcon"],
    image: "images/star-wars/millennium-falcon.jpg",
    difficulty: "easy"
},

{
    answers: ["x wing", "x-wing"],
    image: "images/star-wars/x-wing.jpg",
    difficulty: "easy"
},

{
    answers: ["tie fighter", "tie"],
    image: "images/star-wars/tie-fighter.jpg",
    difficulty: "easy"
},

{
    answers: ["death star"],
    image: "images/star-wars/death-star.jpg",
    difficulty: "easy"
},

{
    answers: ["at at", "at-at"],
    image: "images/star-wars/at-at.jpg",
    difficulty: "medium"
},

{
    answers: ["at st", "at-st"],
    image: "images/star-wars/at-st.jpg",
    difficulty: "medium"
},

{
    answers: ["star destroyer", "imperial star destroyer"],
    image: "images/star-wars/star-destroyer.jpg",
    difficulty: "medium"
},

{
    answers: ["slave i", "slave 1"],
    image: "images/star-wars/slave-1.jpg",
    difficulty: "medium"
},

{
    answers: ["executor"],
    image: "images/star-wars/executor.jpg",
    difficulty: "hard"
},

{
    answers: ["lambda shuttle", "imperial shuttle"],
    image: "images/star-wars/lambda-shuttle.jpg",
    difficulty: "hard"
},

{
    answers: ["landspeeder"],
    image: "images/star-wars/landspeeder.jpg",
    difficulty: "medium"
},

{
    answers: ["podracer"],
    image: "images/star-wars/podracer.jpg",
    difficulty: "medium"
},

{
    answers: ["death watch"],
    image: "images/star-wars/death-watch.jpg",
    difficulty: "hard"
},

{
    answers: ["darksaber"],
    image: "images/star-wars/darksaber.jpg",
    difficulty: "medium"
},

{
    answers: ["grogu pod", "grogu pram", "pram"],
    image: "images/star-wars/grogu-pram.jpg",
    difficulty: "hard"
},

{
    answers: ["mustafar"],
    image: "images/star-wars/mustafar.jpg",
    difficulty: "hard"
},

{
    answers: ["endor"],
    image: "images/star-wars/endor.jpg",
    difficulty: "medium"
},

{
    answers: ["tattooine", "tatooine"],
    image: "images/star-wars/tatooine.jpg",
    difficulty: "easy"
},

{
    answers: ["coruscant"],
    image: "images/star-wars/coruscant.jpg",
    difficulty: "medium"
},

{
    answers: ["kamino"],
    image: "images/star-wars/kamino.jpg",
    difficulty: "medium"
},

{
    answers: ["mustafar duel"],
    image: "images/star-wars/mustafar-duel.jpg",
    difficulty: "hard"
},

{
    answers: ["jedi temple"],
    image: "images/star-wars/jedi-temple.jpg",
    difficulty: "hard"
},

{
    answers: ["order 66"],
    image: "images/star-wars/order-66.jpg",
    difficulty: "hard"
}
],

horror: [

{
    answers: ["michael myers", "michael"],
    image: "images/horror/michael-myers.jpg",
    difficulty: "easy"
},

{
    answers: ["jason voorhees", "jason"],
    image: "images/horror/jason-voorhees.jpg",
    difficulty: "easy"
},

{
    answers: ["freddy krueger", "freddy"],
    image: "images/horror/freddy-krueger.jpg",
    difficulty: "easy"
},

{
    answers: ["ghostface", "ghost face"],
    image: "images/horror/ghostface.jpg",
    difficulty: "easy"
},

{
    answers: ["chucky", "charles lee ray"],
    image: "images/horror/chucky.jpg",
    difficulty: "easy"
},

{
    answers: ["pennywise"],
    image: "images/horror/pennywise.jpg",
    difficulty: "easy"
},

{
    answers: ["leatherface", "leather face"],
    image: "images/horror/leatherface.jpg",
    difficulty: "easy"
},

{
    answers: ["jigsaw", "john kramer"],
    image: "images/horror/jigsaw.jpg",
    difficulty: "easy"
},

{
    answers: ["billy", "billy the puppet"],
    image: "images/horror/billy-the-puppet.jpg",
    difficulty: "easy"
},

{
    answers: ["pinhead", "pin head", "hellraiser"],
    image: "images/horror/pinhead.jpg",
    difficulty: "easy"
},

{
    answers: ["annabelle", "annabelle doll"],
    image: "images/horror/annabelle.jpg",
    difficulty: "easy"
},

{
    answers: ["valak", "the nun", "nun"],
    image: "images/horror/valak.jpg",
    difficulty: "easy"
},

{
    answers: ["samara", "samara morgan"],
    image: "images/horror/samara-morgan.jpg",
    difficulty: "easy"
},

{
    answers: ["candyman", "candy man"],
    image: "images/horror/candyman.jpg",
    difficulty: "easy"
},

{
    answers: ["hannibal lecter", "hannibal"],
    image: "images/horror/hannibal-lecter.jpg",
    difficulty: "easy"
},

{
    answers: ["art the clown", "art"],
    image: "images/horror/art-the-clown.jpg",
    difficulty: "easy"
},

{
    answers: ["freddy fazbear", "freddy"],
    image: "images/horror/freddy-fazbear.jpg",
    difficulty: "easy"
},

{
    answers: ["bonnie"],
    image: "images/horror/bonnie.jpg",
    difficulty: "easy"
},

{
    answers: ["chica"],
    image: "images/horror/chica.jpg",
    difficulty: "easy"
},

{
    answers: ["foxy"],
    image: "images/horror/foxy.jpg",
    difficulty: "easy"
},

{
    answers: ["huggy wuggy", "huggy"],
    image: "images/horror/huggy-wuggy.jpg",
    difficulty: "easy"
},

{
    answers: ["slender man", "slenderman"],
    image: "images/horror/slender-man.jpg",
    difficulty: "easy"
},

{
    answers: ["siren head"],
    image: "images/horror/siren-head.jpg",
    difficulty: "easy"
},

{
    answers: ["pyramid head"],
    image: "images/horror/pyramid-head.jpg",
    difficulty: "easy"
},

{
    answers: ["nemesis"],
    image: "images/horror/nemesis.jpg",
    difficulty: "easy"
},

{
    answers: ["mr x", "mr. x", "tyrant"],
    image: "images/horror/mr-x.jpg",
    difficulty: "easy"
},

{
    answers: ["lady dimitrescu", "dimitrescu"],
    image: "images/horror/lady-dimitrescu.jpg",
    difficulty: "easy"
},

{
    answers: ["alien", "xenomorph"],
    image: "images/horror/xenomorph.jpg",
    difficulty: "easy"
},

{
    answers: ["predator"],
    image: "images/horror/predator.jpg",
    difficulty: "easy"
},

{
    answers: ["demogorgon"],
    image: "images/horror/demogorgon.jpg",
    difficulty: "easy"
},

{
    answers: ["vecna"],
    image: "images/horror/vecna.jpg",
    difficulty: "easy"
},

{
    answers: ["m3gan", "megan"],
    image: "images/horror/m3gan.jpg",
    difficulty: "easy"
},

{
    answers: ["jeff the killer", "jeff"],
    image: "images/horror/jeff-the-killer.jpg",
    difficulty: "easy"
},

{
    answers: ["norman bates", "norman"],
    image: "images/horror/norman-bates.jpg",
    difficulty: "medium"
},

{
    answers: ["esther"],
    image: "images/horror/esther.jpg",
    difficulty: "medium"
},

{
    answers: ["springtrap"],
    image: "images/horror/springtrap.jpg",
    difficulty: "medium"
},

{
    answers: ["golden freddy"],
    image: "images/horror/golden-freddy.jpg",
    difficulty: "medium"
},

{
    answers: ["puppet", "marionette", "the puppet"],
    image: "images/horror/puppet.jpg",
    difficulty: "medium"
},

{
    answers: ["circus baby", "baby"],
    image: "images/horror/circus-baby.jpg",
    difficulty: "medium"
},

{
    answers: ["glamrock freddy"],
    image: "images/horror/glamrock-freddy.jpg",
    difficulty: "medium"
},

{
    answers: ["mommy long legs", "mommy"],
    image: "images/horror/mommy-long-legs.jpg",
    difficulty: "medium"
},

{
    answers: ["catnap", "cat nap"],
    image: "images/horror/catnap.jpg",
    difficulty: "medium"
},

{
    answers: ["bendy"],
    image: "images/horror/bendy.jpg",
    difficulty: "medium"
},

{
    answers: ["ink demon", "the ink demon"],
    image: "images/horror/ink-demon.jpg",
    difficulty: "hard"
},

{
    answers: ["jack baker"],
    image: "images/horror/jack-baker.jpg",
    difficulty: "medium"
},

{
    answers: ["the creeper", "creeper", "jeepers creepers"],
    image: "images/horror/the-creeper.jpg",
    difficulty: "medium"
},

{
    answers: ["the babadook", "babadook"],
    image: "images/horror/the-babadook.jpg",
    difficulty: "medium"
},

{
    answers: ["longlegs"],
    image: "images/horror/longlegs.jpg",
    difficulty: "medium"
},

{
    answers: ["tiffany", "tiffany valentine"],
    image: "images/horror/tiffany-valentine.jpg",
    difficulty: "medium"
},

{
    answers: ["ryuk"],
    image: "images/horror/ryuk.jpg",
    difficulty: "hard"
},

{
    answers: ["robert the doll", "robert"],
    image: "images/horror/robert-the-doll.jpg",
    difficulty: "hard"
},

{
    answers: ["kayako", "kayako saeki"],
    image: "images/horror/kayako-saeki.jpg",
    difficulty: "hard"
},

{
    answers: ["roxanne wolf", "roxanne", "roxy"],
    image: "images/horror/roxanne-wolf.jpg",
    difficulty: "hard"
},

{
    answers: ["montgomery gator", "monty"],
    image: "images/horror/montgomery-gator.jpg",
    difficulty: "hard"
},

{
    answers: ["pumpkinhead"],
    image: "images/horror/pumpkinhead.jpg",
    difficulty: "hard"
},

{
    answers: ["the thing"],
    image: "images/horror/the-thing.jpg",
    difficulty: "hard"
},

{
    answers: ["the bride", "bride"],
    image: "images/horror/the-bride.jpg",
    difficulty: "hard"
},

{
    answers: ["the ferryman", "ferryman"],
    image: "images/horror/the-ferryman.jpg",
    difficulty: "hard"
}
 
 ],

disney: [

{
    answers: ["mickey mouse", "mickey"],
    image: "images/disney/mickey-mouse.jpg",
    difficulty: "easy"
},

{
    answers: ["minnie mouse", "minnie"],
    image: "images/disney/minnie-mouse.jpg",
    difficulty: "easy"
},

{
    answers: ["donald duck", "donald"],
    image: "images/disney/donald-duck.jpg",
    difficulty: "easy"
},

{
    answers: ["daisy duck", "daisy"],
    image: "images/disney/daisy-duck.jpg",
    difficulty: "easy"
},

{
    answers: ["goofy"],
    image: "images/disney/goofy.jpg",
    difficulty: "easy"
},

{
    answers: ["pluto"],
    image: "images/disney/pluto.jpg",
    difficulty: "easy"
},

{
    answers: ["stitch"],
    image: "images/disney/stitch.jpg",
    difficulty: "easy"
},

{
    answers: ["lilo"],
    image: "images/disney/lilo.jpg",
    difficulty: "easy"
},

{
    answers: ["simba"],
    image: "images/disney/simba.jpg",
    difficulty: "easy"
},

{
    answers: ["mufasa"],
    image: "images/disney/mufasa.jpg",
    difficulty: "easy"
},

{
    answers: ["scar"],
    image: "images/disney/scar.jpg",
    difficulty: "easy"
},

{
    answers: ["timon"],
    image: "images/disney/timon.jpg",
    difficulty: "easy"
},

{
    answers: ["pumbaa", "pumba"],
    image: "images/disney/pumbaa.jpg",
    difficulty: "easy"
},

{
    answers: ["elsa"],
    image: "images/disney/elsa.jpg",
    difficulty: "easy"
},

{
    answers: ["anna"],
    image: "images/disney/anna.jpg",
    difficulty: "easy"
},

{
    answers: ["olaf"],
    image: "images/disney/olaf.jpg",
    difficulty: "easy"
},

{
    answers: ["kristoff"],
    image: "images/disney/kristoff.jpg",
    difficulty: "easy"
},

{
    answers: ["sven"],
    image: "images/disney/sven.jpg",
    difficulty: "easy"
},

{
    answers: ["moana"],
    image: "images/disney/moana.jpg",
    difficulty: "easy"
},

{
    answers: ["maui"],
    image: "images/disney/maui.jpg",
    difficulty: "easy"
},

{
    answers: ["ariel"],
    image: "images/disney/ariel.jpg",
    difficulty: "easy"
},

{
    answers: ["flounder"],
    image: "images/disney/flounder.jpg",
    difficulty: "easy"
},

{
    answers: ["sebastian"],
    image: "images/disney/sebastian.jpg",
    difficulty: "easy"
},

{
    answers: ["ursula"],
    image: "images/disney/ursula.jpg",
    difficulty: "easy"
},

{
    answers: ["aladdin"],
    image: "images/disney/aladdin.jpg",
    difficulty: "easy"
},

{
    answers: ["genie"],
    image: "images/disney/genie.jpg",
    difficulty: "easy"
},

{
    answers: ["jasmine"],
    image: "images/disney/jasmine.jpg",
    difficulty: "easy"
},

{
    answers: ["jafar"],
    image: "images/disney/jafar.jpg",
    difficulty: "easy"
},

{
    answers: ["hercules"],
    image: "images/disney/hercules.jpg",
    difficulty: "easy"
},

{
    answers: ["hades"],
    image: "images/disney/hades.jpg",
    difficulty: "easy"
},

{
    answers: ["peter pan"],
    image: "images/disney/peter-pan.jpg",
    difficulty: "easy"
},

{
    answers: ["captain hook", "hook"],
    image: "images/disney/captain-hook.jpg",
    difficulty: "easy"
},

{
    answers: ["tinker bell", "tinkerbell"],
    image: "images/disney/tinker-bell.jpg",
    difficulty: "easy"
},

{
    answers: ["winnie the pooh", "pooh", "winnie"],
    image: "images/disney/winnie-the-pooh.jpg",
    difficulty: "easy"
},

{
    answers: ["tigger"],
    image: "images/disney/tigger.jpg",
    difficulty: "easy"
},

{
    answers: ["piglet"],
    image: "images/disney/piglet.jpg",
    difficulty: "easy"
},

{
    answers: ["eeyore"],
    image: "images/disney/eeyore.jpg",
    difficulty: "easy"
},

{
    answers: ["buzz lightyear", "buzz"],
    image: "images/disney/buzz-lightyear.jpg",
    difficulty: "easy"
},

{
    answers: ["woody"],
    image: "images/disney/woody.jpg",
    difficulty: "easy"
},

{
    answers: ["jessie"],
    image: "images/disney/jessie.jpg",
    difficulty: "easy"
},

{
    answers: ["lightning mcqueen", "mcqueen"],
    image: "images/disney/lightning-mcqueen.jpg",
    difficulty: "easy"
},

{
    answers: ["mater", "tow mater"],
    image: "images/disney/mater.jpg",
    difficulty: "easy"
},

{
    answers: ["sulley", "james p. sullivan"],
    image: "images/disney/sulley.jpg",
    difficulty: "easy"
},

{
    answers: ["mike wazowski", "mike"],
    image: "images/disney/mike-wazowski.jpg",
    difficulty: "easy"
},

{
    answers: ["nemo"],
    image: "images/disney/nemo.jpg",
    difficulty: "easy"
},

{
    answers: ["dory"],
    image: "images/disney/dory.jpg",
    difficulty: "easy"
},

{
    answers: ["wall-e", "walle"],
    image: "images/disney/wall-e.jpg",
    difficulty: "easy"
},

{
    answers: ["eve", "eva"],
    image: "images/disney/eve.jpg",
    difficulty: "easy"
},

{
    answers: ["belle"],
    image: "images/disney/belle.jpg",
    difficulty: "easy"
},

{
    answers: ["beast", "the beast"],
    image: "images/disney/beast.jpg",
    difficulty: "easy"
},

{
    answers: ["snow white"],
    image: "images/disney/snow-white.jpg",
    difficulty: "easy"
},

{
    answers: ["cinderella"],
    image: "images/disney/cinderella.jpg",
    difficulty: "easy"
},

{
    answers: ["pinocchio"],
    image: "images/disney/pinocchio.jpg",
    difficulty: "easy"
},

{
    answers: ["jiminy cricket", "jiminy"],
    image: "images/disney/jiminy-cricket.jpg",
    difficulty: "easy"
},

{
    answers: ["alice"],
    image: "images/disney/alice.jpg",
    difficulty: "easy"
},

{
    answers: ["cheshire cat", "cheshire"],
    image: "images/disney/cheshire-cat.jpg",
    difficulty: "easy"
},

{
    answers: ["baymax"],
    image: "images/disney/baymax.jpg",
    difficulty: "easy"
},

{
    answers: ["mirabel"],
    image: "images/disney/mirabel.jpg",
    difficulty: "easy"
},

{
    answers: ["tiana"],
    image: "images/disney/tiana.jpg",
    difficulty: "easy"
},

{
    answers: ["ralph", "wreck-it ralph"],
    image: "images/disney/ralph.jpg",
    difficulty: "easy"
},

{
    answers: ["gaston"],
    image: "images/disney/gaston.jpg",
    difficulty: "medium"
},

{
    answers: ["lumiere"],
    image: "images/disney/lumiere.jpg",
    difficulty: "medium"
},

{
    answers: ["cogsworth"],
    image: "images/disney/cogsworth.jpg",
    difficulty: "medium"
},

{
    answers: ["rapunzel"],
    image: "images/disney/rapunzel.jpg",
    difficulty: "medium"
},

{
    answers: ["flynn rider", "flynn", "eugene fitzherbert", "eugene"],
    image: "images/disney/flynn-rider.jpg",
    difficulty: "medium"
},

{
    answers: ["pascal"],
    image: "images/disney/pascal.jpg",
    difficulty: "medium"
},

{
    answers: ["maximus"],
    image: "images/disney/maximus.jpg",
    difficulty: "medium"
},

{
    answers: ["merida"],
    image: "images/disney/merida.jpg",
    difficulty: "medium"
},

{
    answers: ["pocahontas"],
    image: "images/disney/pocahontas.jpg",
    difficulty: "medium"
},

{
    answers: ["mulan"],
    image: "images/disney/mulan.jpg",
    difficulty: "medium"
},

{
    answers: ["mushu"],
    image: "images/disney/mushu.jpg",
    difficulty: "medium"
},

{
    answers: ["tarzan"],
    image: "images/disney/tarzan.jpg",
    difficulty: "medium"
},

{
    answers: ["jane", "jane porter"],
    image: "images/disney/jane.jpg",
    difficulty: "medium"
},

{
    answers: ["baloo"],
    image: "images/disney/baloo.jpg",
    difficulty: "medium"
},

{
    answers: ["bagheera"],
    image: "images/disney/bagheera.jpg",
    difficulty: "medium"
},

{
    answers: ["king louie"],
    image: "images/disney/king-louie.jpg",
    difficulty: "medium"
},

{
    answers: ["dumbo"],
    image: "images/disney/dumbo.jpg",
    difficulty: "medium"
},

{
    answers: ["bambi"],
    image: "images/disney/bambi.jpg",
    difficulty: "medium"
},

{
    answers: ["thumper"],
    image: "images/disney/thumper.jpg",
    difficulty: "medium"
},

{
    answers: ["lady"],
    image: "images/disney/lady.jpg",
    difficulty: "medium"
},

{
    answers: ["sally carrera", "sally"],
    image: "images/disney/sally-carrera.jpg",
    difficulty: "medium"
},

{
    answers: ["doc hudson", "doc"],
    image: "images/disney/doc-hudson.jpg",
    difficulty: "medium"
},

{
    answers: ["chick hicks", "chick"],
    image: "images/disney/chick-hicks.jpg",
    difficulty: "medium"
},

{
    answers: ["cruella de vil", "cruella"],
    image: "images/disney/cruella-de-vil.jpg",
    difficulty: "medium"
},

{
    answers: ["fairy godmother", "godmother"],
    image: "images/disney/fairy-godmother.jpg",
    difficulty: "medium"
},

{
    answers: ["the evil queen", "evil queen", "queen grimhilde", "grimhilde"],
    image: "images/disney/evil-queen.jpg",
    difficulty: "medium"
},

{
    answers: ["maleficent"],
    image: "images/disney/maleficent.jpg",
    difficulty: "medium"
},

{
    answers: ["aurora", "sleeping beauty"],
    image: "images/disney/aurora.jpg",
    difficulty: "medium"
},

{
    answers: ["prince phillip", "phillip"],
    image: "images/disney/prince-phillip.jpg",
    difficulty: "medium"
},

{
    answers: ["vanellope", "vanellope von schweetz"],
    image: "images/disney/vanellope.jpg",
    difficulty: "medium"
},

{
    answers: ["dr facilier", "facilier", "doctor facilier"],
    image: "images/disney/dr-facilier.jpg",
    difficulty: "hard"
},

{
    answers: ["kuzco"],
    image: "images/disney/kuzco.jpg",
    difficulty: "hard"
},

{
    answers: ["yzma"],
    image: "images/disney/yzma.jpg",
    difficulty: "hard"
},

{
    answers: ["kronk"],
    image: "images/disney/kronk.jpg",
    difficulty: "hard"
},

{
    answers: ["carl fredricksen", "carl"],
    image: "images/disney/carl-fredricksen.jpg",
    difficulty: "hard"
},

{
    answers: ["russell"],
    image: "images/disney/russell.jpg",
    difficulty: "hard"
},

{
    answers: ["dug"],
    image: "images/disney/dug.jpg",
    difficulty: "hard"
},

{
    answers: ["kevin"],
    image: "images/disney/kevin.jpg",
    difficulty: "hard"
},

{
    answers: ["remy"],
    image: "images/disney/remy.jpg",
    difficulty: "hard"
}, 

{
    answers: ["mother gothel", "gothel"],
    image: "images/disney/mother-gothel.jpg",
    difficulty: "hard"
},

{
    answers: ["rex"],
    image: "images/disney/rex.jpg",
    difficulty: "easy"
},

{
    answers: ["forky"],
    image: "images/disney/forky.jpg",
    difficulty: "medium"
},

{
    answers: ["lotso"],
    image: "images/disney/lotso.jpg",
    difficulty: "medium"
},

{
    answers: ["marlin"],
    image: "images/disney/marlin.jpg",
    difficulty: "easy"
},

{
    answers: ["crush"],
    image: "images/disney/crush.jpg",
    difficulty: "medium"
},

{
    answers: ["boo"],
    image: "images/disney/boo.jpg",
    difficulty: "easy"
},

{
    answers: ["randall", "randall boggs"],
    image: "images/disney/randall.jpg",
    difficulty: "medium"
},

{
    answers: ["mr incredible"],
    image: "images/disney/mr-incredible.jpg",
    difficulty: "easy"
},

{
    answers: ["elastigirl", "helen parr"],
    image: "images/disney/elastigirl.jpg",
    difficulty: "easy"
},

{
    answers: ["jack-jack", "jack jack"],
    image: "images/disney/jack-jack.jpg",
    difficulty: "medium"
},

{
    answers: ["joy"],
    image: "images/disney/joy.jpg",
    difficulty: "easy"
},

{
    answers: ["sadness"],
    image: "images/disney/sadness.jpg",
    difficulty: "easy"
},

{
    answers: ["anxiety"],
    image: "images/disney/anxiety.jpg",
    difficulty: "medium"
},

{
    answers: ["miguel", "miguel rivera"],
    image: "images/disney/miguel.jpg",
    difficulty: "easy"
},

{
    answers: ["hector", "hector rivera"],
    image: "images/disney/hector.jpg",
    difficulty: "medium"
},

{
    answers: ["ember", "ember lumen"],
    image: "images/disney/ember.jpg",
    difficulty: "easy"
},

{
    answers: ["wade", "wade ripple"],
    image: "images/disney/wade.jpg",
    difficulty: "easy"
},

{
    answers: ["luca", "luca paguro"],
    image: "images/disney/luca.jpg",
    difficulty: "easy"
},

{
    answers: ["alberto", "alberto scorfano"],
    image: "images/disney/alberto.jpg",
    difficulty: "medium"
},

{
    answers: ["luigi"],
    image: "images/disney/luigi.jpg",
    difficulty: "medium"
},

{
    answers: ["guido"],
    image: "images/disney/guido.jpg",
    difficulty: "medium"
},

{
    answers: ["ramone"],
    image: "images/disney/ramone.jpg",
    difficulty: "medium"
},

{
    answers: ["sheriff"],
    image: "images/disney/sheriff.jpg",
    difficulty: "medium"
},

{
    answers: ["flo"],
    image: "images/disney/flo.jpg",
    difficulty: "medium"
},

{
    answers: ["the king", "king"],
    image: "images/disney/the-king.jpg",
    difficulty: "hard"
}
],

monuments: [

{
    answers: ["πύργος του άιφελ", "άιφελ", "eiffel tower", "eiffel"],
    image: "images/monuments/eiffel-tower.jpg",
    difficulty: "easy"
},

{
    answers: ["άγαλμα της ελευθερίας", "statue of liberty"],
    image: "images/monuments/statue-of-liberty.jpg",
    difficulty: "easy"
},

{
    answers: ["μπιγκ μπεν", "big ben"],
    image: "images/monuments/big-ben.jpg",
    difficulty: "easy"
},

{
    answers: ["κολοσσαίο", "colosseum"],
    image: "images/monuments/colosseum.jpg",
    difficulty: "easy"
},

{
    answers: ["κεκλιμένος πύργος της πίζας", "πύργος της πίζας", "leaning tower of pisa", "tower of pisa"],
    image: "images/monuments/leaning-tower-of-pisa.jpg",
    difficulty: "easy"
},

{
    answers: ["μεγάλη πυραμίδα της γκίζας", "πυραμίδα της γκίζας", "great pyramid of giza", "pyramid of giza"],
    image: "images/monuments/great-pyramid-of-giza.jpg",
    difficulty: "easy"
},

{
    answers: ["σφίγγα", "μεγάλη σφίγγα", "great sphinx", "sphinx"],
    image: "images/monuments/sphinx.jpg",
    difficulty: "easy"
},

{
    answers: ["ταζ μαχάλ", "taj mahal"],
    image: "images/monuments/taj-mahal.jpg",
    difficulty: "easy"
},

{
    answers: ["όπερα του σίδνεϊ", "sydney opera house"],
    image: "images/monuments/sydney-opera-house.jpg",
    difficulty: "easy"
},

{
    answers: ["χριστός λυτρωτής", "christ the redeemer"],
    image: "images/monuments/christ-the-redeemer.jpg",
    difficulty: "easy"
},

{
    answers: ["αψίδα του θριάμβου", "arc de triomphe"],
    image: "images/monuments/arc-de-triomphe.jpg",
    difficulty: "easy"
},

{
    answers: ["όρος ράσμορ", "mount rushmore"],
    image: "images/monuments/mount-rushmore.jpg",
    difficulty: "easy"
},

{
    answers: ["πύλη του βρανδεμβούργου", "brandenburg gate"],
    image: "images/monuments/brandenburg-gate.jpg",
    difficulty: "easy"
},

{
    answers: ["στόουνχεντζ", "stonehenge"],
    image: "images/monuments/stonehenge.jpg",
    difficulty: "easy"
},

{
    answers: ["μάτσου πίτσου", "machu picchu"],
    image: "images/monuments/machu-picchu.jpg",
    difficulty: "easy"
},

{
    answers: ["παρθενώνας", "parthenon"],
    image: "images/monuments/parthenon.jpg",
    difficulty: "easy"
},

{
    answers: ["γέφυρα γκόλντεν γκέιτ", "golden gate bridge"],
    image: "images/monuments/golden-gate-bridge.jpg",
    difficulty: "easy"
},

{
    answers: ["μπουρτζ χαλίφα", "burj khalifa"],
    image: "images/monuments/burj-khalifa.jpg",
    difficulty: "easy"
},

{
    answers: ["τάουερ μπριτζ", "tower bridge"],
    image: "images/monuments/tower-bridge.jpg",
    difficulty: "easy"
},

{
    answers: ["πυραμίδα του λούβρου", "louvre pyramid"],
    image: "images/monuments/louvre-pyramid.jpg",
    difficulty: "easy"
},


{
    answers: ["κάστρο νοϊσβανστάιν", "neuschwanstein castle"],
    image: "images/monuments/neuschwanstein-castle.jpg",
    difficulty: "medium"
},

{
    answers: ["ανγκόρ βατ", "angkor wat"],
    image: "images/monuments/angkor-wat.jpg",
    difficulty: "medium"
},

{
    answers: ["πέτρα", "petra"],
    image: "images/monuments/petra.jpg",
    difficulty: "medium"
},

{
    answers: ["τσιτσέν ιτζά", "chichen itza"],
    image: "images/monuments/chichen-itza.jpg",
    difficulty: "medium"
},

{
    answers: ["μοάι", "αγάλματα μοάι", "moai", "easter island statues"],
    image: "images/monuments/moai.jpg",
    difficulty: "medium"
},

{
    answers: ["κρήνη ντι τρέβι", "fontana di trevi", "trevi fountain"],
    image: "images/monuments/trevi-fountain.jpg",
    difficulty: "medium"
},

{
    answers: ["πάνθεον", "pantheon"],
    image: "images/monuments/pantheon.jpg",
    difficulty: "medium"
},

{
    answers: ["σακρέ κερ", "sacre coeur", "sacré-cœur"],
    image: "images/monuments/sacre-coeur.jpg",
    difficulty: "medium"
},

{
    answers: ["σαγραδα φαμίλια", "sagrada familia"],
    image: "images/monuments/sagrada-familia.jpg",
    difficulty: "medium"
},

{
    answers: ["απαγορευμένη πόλη", "forbidden city"],
    image: "images/monuments/forbidden-city.jpg",
    difficulty: "medium"
},

{
    answers: ["κρεμλίνο", "kremlin"],
    image: "images/monuments/kremlin.jpg",
    difficulty: "medium"
},

{
    answers: ["μπλε τζαμί", "blue mosque"],
    image: "images/monuments/blue-mosque.jpg",
    difficulty: "medium"
},

{
    answers: ["αγία σοφία", "hagia sophia"],
    image: "images/monuments/hagia-sophia.jpg",
    difficulty: "medium"
},

{
    answers: ["μνημείο λίνκολν", "lincoln memorial"],
    image: "images/monuments/lincoln-memorial.jpg",
    difficulty: "medium"
},

{
    answers: ["cn tower", "πύργος cn"],
    image: "images/monuments/cn-tower.jpg",
    difficulty: "medium"
},

{
    answers: ["πύργος του τόκιο", "tokyo tower"],
    image: "images/monuments/tokyo-tower.jpg",
    difficulty: "medium"
},

{
    answers: ["marina bay sands"],
    image: "images/monuments/marina-bay-sands.jpg",
    difficulty: "medium"
},

{
    answers: ["ατόμιουμ", "atomium"],
    image: "images/monuments/atomium.jpg",
    difficulty: "medium"
},

{
    answers: ["αλάμπρα", "alhambra"],
    image: "images/monuments/alhambra.jpg",
    difficulty: "medium"
},

{
    answers: ["ακρόπολη", "ακρόπολη αθηνών", "acropolis", "acropolis of athens"],
    image: "images/monuments/acropolis.jpg",
    difficulty: "medium"
},

{
    answers: ["βορομπουντούρ", "borobudur"],
    image: "images/monuments/borobudur.jpg",
    difficulty: "hard"
},

{
    answers: ["ναοί του μπαγκάν", "bagan temples", "bagan"],
    image: "images/monuments/bagan.jpg",
    difficulty: "hard"
},

{
    answers: ["ναός του ουρανού", "temple of heaven"],
    image: "images/monuments/temple-of-heaven.jpg",
    difficulty: "hard"
},

{
    answers: ["μον σεν μισέλ", "mont saint-michel"],
    image: "images/monuments/mont-saint-michel.jpg",
    difficulty: "hard"
},

{
    answers: ["κάστρο χιμέτζι", "himeji castle"],
    image: "images/monuments/himeji-castle.jpg",
    difficulty: "hard"
},

{
    answers: ["μεγάλο τζαμί σεΐχη ζαγιέντ", "sheikh zayed grand mosque"],
    image: "images/monuments/sheikh-zayed-grand-mosque.jpg",
    difficulty: "hard"
},

{
    answers: ["παλάτι του γουέστμινστερ", "palace of westminster"],
    image: "images/monuments/palace-of-westminster.jpg",
    difficulty: "hard"
},

{
    answers: ["πραμπανάν", "prambanan"],
    image: "images/monuments/prambanan.jpg",
    difficulty: "hard"
},

{
    answers: ["μοναστήρι της ρίλα", "rila monastery"],
    image: "images/monuments/rila-monastery.jpg",
    difficulty: "hard"
},

{
    answers: ["κάστρο σαντ' άντζελο", "castel sant'angelo"],
    image: "images/monuments/castel-santangelo.jpg",
    difficulty: "hard"
},

{
    answers: ["καθεδρικός της νοτρ νταμ", "notre dame", "notre-dame cathedral"],
    image: "images/monuments/notre-dame.jpg",
    difficulty: "medium"
},

{
    answers: ["empire state building", "εμπάιαρ στέιτ"],
    image: "images/monuments/empire-state-building.jpg",
    difficulty: "medium"
},

{
    answers: ["one world trade center", "world trade center"],
    image: "images/monuments/one-world-trade-center.jpg",
    difficulty: "medium"
},

{
    answers: ["γέφυρα του μπρούκλιν", "brooklyn bridge"],
    image: "images/monuments/brooklyn-bridge.jpg",
    difficulty: "medium"
},

{
    answers: ["gateway arch"],
    image: "images/monuments/gateway-arch.jpg",
    difficulty: "medium"
},

{
    answers: ["space needle"],
    image: "images/monuments/space-needle.jpg",
    difficulty: "medium"
},

{
    answers: ["καθεδρικός του αγίου βασιλείου", "st basil's cathedral"],
    image: "images/monuments/st-basils-cathedral.jpg",
    difficulty: "medium"
},

{
    answers: ["μπουρτζ αλ αράμπ", "burj al arab"],
    image: "images/monuments/burj-al-arab.jpg",
    difficulty: "medium"
},

{
    answers: ["ναός του λωτού", "lotus temple"],
    image: "images/monuments/lotus-temple.jpg",
    difficulty: "medium"
},

{
    answers: ["δίδυμοι πυργοι", "petronas twin towers", "petronas towers"],
    image: "images/monuments/petronas-towers.jpg",
    difficulty: "medium"
},

{
    answers: ["πύργος της κουάλα λουμπούρ", "kl tower", "kuala lumpur tower"],
    image: "images/monuments/kl-tower.jpg",
    difficulty: "medium"
},

{
    answers: ["παλάτι γκιονγκμποκ", "gyeongbokgung palace"],
    image: "images/monuments/gyeongbokgung-palace.jpg",
    difficulty: "medium"
},

{
    answers: ["πύργος της σεούλ", "n seoul tower"],
    image: "images/monuments/n-seoul-tower.jpg",
    difficulty: "medium"
},

{
    answers: ["φουσίμι ινάρι", "fushimi inari"],
    image: "images/monuments/fushimi-inari.jpg",
    difficulty: "medium"
},

{
    answers: ["κινκακού τζι", "golden pavilion", "kinkaku-ji"],
    image: "images/monuments/kinkaku-ji.jpg",
    difficulty: "hard"
},

{
    answers: ["ιερό ιτσουκουσίμα", "itsukushima shrine"],
    image: "images/monuments/itsukushima-shrine.jpg",
    difficulty: "hard"
},

{
    answers: ["καθεδρικός του μιλάνου", "duomo di milano", "milan cathedral"],
    image: "images/monuments/milan-cathedral.jpg",
    difficulty: "medium"
},

{
    answers: ["καθεδρικός της φλωρεντίας", "florence cathedral"],
    image: "images/monuments/florence-cathedral.jpg",
    difficulty: "medium"
},

{
    answers: ["βασιλική του αγίου πέτρου", "st peter's basilica"],
    image: "images/monuments/st-peters-basilica.jpg",
    difficulty: "medium"
},

{
    answers: ["καπέλα σιστίνα", "sistine chapel"],
    image: "images/monuments/sistine-chapel.jpg",
    difficulty: "medium"
},

{
    answers: ["γέφυρα του καρόλου", "charles bridge"],
    image: "images/monuments/charles-bridge.jpg",
    difficulty: "medium"
},

{
    answers: ["κάστρο της πράγας", "prague castle"],
    image: "images/monuments/prague-castle.jpg",
    difficulty: "medium"
},

{
    answers: ["πύργος του μπελέμ", "belem tower"],
    image: "images/monuments/belem-tower.jpg",
    difficulty: "medium"
},

{
    answers: ["μοναστήρι των ιερονυμιτών", "jeronimos monastery"],
    image: "images/monuments/jeronimos-monastery.jpg",
    difficulty: "hard"
},

{
    answers: ["κάστρο του εδιμβούργου", "edinburgh castle"],
    image: "images/monuments/edinburgh-castle.jpg",
    difficulty: "medium"
},

{
    answers: ["γίγαντας του δρόμου", "giant's causeway"],
    image: "images/monuments/giants-causeway.jpg",
    difficulty: "hard"
},

{
    answers: ["παλάτι πένα", "pena palace"],
    image: "images/monuments/pena-palace.jpg",
    difficulty: "medium"
}

],


countries: [

{
    answers: ["ελλάδα", "greece"],
    image: "images/countries/greece.png",
    difficulty: "easy"
},

{
    answers: ["γαλλία", "france"],
    image: "images/countries/france.png",
    difficulty: "easy"
},

{
    answers: ["γερμανία", "germany"],
    image: "images/countries/germany.png",
    difficulty: "easy"
},

{
    answers: ["ιταλία", "italy"],
    image: "images/countries/italy.png",
    difficulty: "easy"
},

{
    answers: ["ισπανία", "spain"],
    image: "images/countries/spain.png",
    difficulty: "easy"
},

{
    answers: ["πορτογαλία", "portugal"],
    image: "images/countries/portugal.png",
    difficulty: "easy"
},

{
    answers: ["ηνωμένο βασίλειο", "αγγλία", "united kingdom", "uk", "england"],
    image: "images/countries/united-kingdom.png",
    difficulty: "easy"
},

{
    answers: ["ιρλανδία", "ireland"],
    image: "images/countries/ireland.png",
    difficulty: "easy"
},

{
    answers: ["βελγιο", "βέλγιο", "belgium"],
    image: "images/countries/belgium.png",
    difficulty: "easy"
},

{
    answers: ["ολλανδία", "κάτω χώρες", "netherlands"],
    image: "images/countries/netherlands.png",
    difficulty: "easy"
},

{
    answers: ["ελβετία", "switzerland"],
    image: "images/countries/switzerland.png",
    difficulty: "easy"
},

{
    answers: ["αυστρία", "austria"],
    image: "images/countries/austria.png",
    difficulty: "easy"
},

{
    answers: ["σουηδία", "sweden"],
    image: "images/countries/sweden.png",
    difficulty: "easy"
},

{
    answers: ["νορβηγία", "norway"],
    image: "images/countries/norway.png",
    difficulty: "easy"
},

{
    answers: ["δανία", "denmark"],
    image: "images/countries/denmark.png",
    difficulty: "easy"
},

{
    answers: ["φινλανδία", "finland"],
    image: "images/countries/finland.png",
    difficulty: "easy"
},

{
    answers: ["πολωνία", "poland"],
    image: "images/countries/poland.png",
    difficulty: "easy"
},

{
    answers: ["τσεχία", "czech republic", "czechia"],
    image: "images/countries/czechia.png",
    difficulty: "easy"
},

{
    answers: ["ουγγαρία", "hungary"],
    image: "images/countries/hungary.png",
    difficulty: "easy"
},

{
    answers: ["ρουμανία", "romania"],
    image: "images/countries/romania.png",
    difficulty: "easy"
},

{
    answers: ["βουλγαρία", "bulgaria"],
    image: "images/countries/bulgaria.png",
    difficulty: "easy"
},

{
    answers: ["τουρκία", "turkey", "türkiye"],
    image: "images/countries/turkiye.png",
    difficulty: "easy"
},

{
    answers: ["ρωσία", "russia"],
    image: "images/countries/russia.png",
    difficulty: "easy"
},

{
    answers: ["ουκρανία", "ukraine"],
    image: "images/countries/ukraine.png",
    difficulty: "easy"
},

{
    answers: ["καναδάς", "canada"],
    image: "images/countries/canada.png",
    difficulty: "easy"
},

{
    answers: ["ηνωμένες πολιτείες", "ηπα", "αμερική", "united states", "usa", "us", "america"],
    image: "images/countries/united-states.png",
    difficulty: "easy"
},

{
    answers: ["μεξικό", "mexico"],
    image: "images/countries/mexico.png",
    difficulty: "easy"
},

{
    answers: ["βραζιλία", "brazil"],
    image: "images/countries/brazil.png",
    difficulty: "easy"
},

{
    answers: ["αργεντινή", "argentina"],
    image: "images/countries/argentina.png",
    difficulty: "easy"
},

{
    answers: ["χιλή", "chile"],
    image: "images/countries/chile.png",
    difficulty: "easy"
},

{
    answers: ["περού", "peru"],
    image: "images/countries/peru.png",
    difficulty: "easy"
},

{
    answers: ["κολομβία", "colombia"],
    image: "images/countries/colombia.png",
    difficulty: "easy"
},

{
    answers: ["ιαπωνία", "japan"],
    image: "images/countries/japan.png",
    difficulty: "easy"
},

{
    answers: ["κίνα", "china"],
    image: "images/countries/china.png",
    difficulty: "easy"
},

{
    answers: ["νότια κορέα", "κορέα", "south korea"],
    image: "images/countries/south-korea.png",
    difficulty: "easy"
},

{
    answers: ["βόρεια κορέα", "north korea"],
    image: "images/countries/north-korea.png",
    difficulty: "medium"
},

{
    answers: ["ινδία", "india"],
    image: "images/countries/india.png",
    difficulty: "easy"
},

{
    answers: ["πακιστάν", "pakistan"],
    image: "images/countries/pakistan.png",
    difficulty: "medium"
},

{
    answers: ["ταϊλάνδη", "thailand"],
    image: "images/countries/thailand.png",
    difficulty: "medium"
},

{
    answers: ["βιετνάμ", "vietnam"],
    image: "images/countries/vietnam.png",
    difficulty: "medium"
},

{
    answers: ["ινδονησία", "indonesia"],
    image: "images/countries/indonesia.png",
    difficulty: "medium"
},

{
    answers: ["φιλιππίνες", "philippines"],
    image: "images/countries/philippines.png",
    difficulty: "medium"
},

{
    answers: ["αυστραλία", "australia"],
    image: "images/countries/australia.png",
    difficulty: "easy"
},

{
    answers: ["νέα ζηλανδία", "new zealand"],
    image: "images/countries/new-zealand.png",
    difficulty: "medium"
},

{
    answers: ["αιγυπτος", "αιγυπτος", "egypt"],
    image: "images/countries/egypt.png",
    difficulty: "easy"
},

{
    answers: ["μαρόκο", "morocco"],
    image: "images/countries/morocco.png",
    difficulty: "medium"
},

{
    answers: ["νότια αφρική", "south africa"],
    image: "images/countries/south-africa.png",
    difficulty: "medium"
},

{
    answers: ["σαουδική αραβία", "saudi arabia"],
    image: "images/countries/saudi-arabia.png",
    difficulty: "medium"
},

{
    answers: ["ηνωμένα αραβικά εμιράτα", "uae", "united arab emirates"],
    image: "images/countries/united-arab-emirates.png",
    difficulty: "medium"
},

{
    answers: ["ισραήλ", "israel"],
    image: "images/countries/israel.png",
    difficulty: "medium"
},

{
    answers: ["σιγκαπούρη", "singapore"],
    image: "images/countries/singapore.png",
    difficulty: "medium"
},

{
    answers: ["μαλαισία", "malaysia"],
    image: "images/countries/malaysia.png",
    difficulty: "medium"
},

{
    answers: ["κατάρ", "qatar"],
    image: "images/countries/qatar.png",
    difficulty: "medium"
},

{
    answers: ["κροατία", "croatia"],
    image: "images/countries/croatia.png",
    difficulty: "easy"
},

{
    answers: ["σερβία", "serbia"],
    image: "images/countries/serbia.png",
    difficulty: "medium"
},


{
    answers: ["σλοβενία", "slovenia"],
    image: "images/countries/slovenia.png",
    difficulty: "medium"
},

{
    answers: ["σλοβακία", "slovakia"],
    image: "images/countries/slovakia.png",
    difficulty: "medium"
},

{
    answers: ["βοσνία και ερζεγοβίνη", "bosnia and herzegovina", "bosnia"],
    image: "images/countries/bosnia-and-herzegovina.png",
    difficulty: "medium"
},

{
    answers: ["μαυροβούνιο", "montenegro"],
    image: "images/countries/montenegro.png",
    difficulty: "medium"
},

{
    answers: ["βόρεια μακεδονία", "north macedonia"],
    image: "images/countries/north-macedonia.png",
    difficulty: "medium"
},

{
    answers: ["αλβανία", "albania"],
    image: "images/countries/albania.png",
    difficulty: "easy"
},

{
    answers: ["κύπρος", "cyprus"],
    image: "images/countries/cyprus.png",
    difficulty: "easy"
},

{
    answers: ["λιθουανία", "lithuania"],
    image: "images/countries/lithuania.png",
    difficulty: "medium"
},

{
    answers: ["λετονία", "latvia"],
    image: "images/countries/latvia.png",
    difficulty: "medium"
},

{
    answers: ["εσθονία", "estonia"],
    image: "images/countries/estonia.png",
    difficulty: "medium"
},

{
    answers: ["λευκορωσία", "belarus"],
    image: "images/countries/belarus.png",
    difficulty: "medium"
},

{
    answers: ["μολδαβία", "moldova"],
    image: "images/countries/moldova.png",
    difficulty: "medium"
},

{
    answers: ["γεωργία", "georgia"],
    image: "images/countries/georgia.png",
    difficulty: "medium"
},

{
    answers: ["αρμενία", "armenia"],
    image: "images/countries/armenia.png",
    difficulty: "medium"
},

{
    answers: ["αζερμπαϊτζάν", "azerbaijan"],
    image: "images/countries/azerbaijan.png",
    difficulty: "medium"
},

{
    answers: ["καζακστάν", "kazakhstan"],
    image: "images/countries/kazakhstan.png",
    difficulty: "medium"
},

{
    answers: ["ουζμπεκιστάν", "uzbekistan"],
    image: "images/countries/uzbekistan.png",
    difficulty: "hard"
},

{
    answers: ["τουρκμενιστάν", "turkmenistan"],
    image: "images/countries/turkmenistan.png",
    difficulty: "hard"
},

{
    answers: ["κιργιζιστάν", "kyrgyzstan"],
    image: "images/countries/kyrgyzstan.png",
    difficulty: "hard"
},

{
    answers: ["τατζικιστάν", "tajikistan"],
    image: "images/countries/tajikistan.png",
    difficulty: "hard"
},

{
    answers: ["νεπάλ", "nepal"],
    image: "images/countries/nepal.png",
    difficulty: "medium"
},

{
    answers: ["μπανγκλαντές", "bangladesh"],
    image: "images/countries/bangladesh.png",
    difficulty: "medium"
},

{
    answers: ["σρι λάνκα", "sri lanka"],
    image: "images/countries/sri-lanka.png",
    difficulty: "medium"
},

{
    answers: ["μογγολία", "mongolia"],
    image: "images/countries/mongolia.png",
    difficulty: "medium"
},

{
    answers: ["ταϊβάν", "taiwan"],
    image: "images/countries/taiwan.png",
    difficulty: "medium"
},

{
    answers: ["καμπότζη", "cambodia"],
    image: "images/countries/cambodia.png",
    difficulty: "medium"
},

{
    answers: ["λάος", "laos"],
    image: "images/countries/laos.png",
    difficulty: "hard"
},

{
    answers: ["μιανμάρ", "myanmar", "burma"],
    image: "images/countries/myanmar.png",
    difficulty: "hard"
},

{
    answers: ["μπρουνέι", "brunei"],
    image: "images/countries/brunei.png",
    difficulty: "hard"
},

{
    answers: ["παπούα νέα γουινέα", "papua new guinea"],
    image: "images/countries/papua-new-guinea.png",
    difficulty: "hard"
},


{
    answers: ["ισλανδία", "iceland"],
    image: "images/countries/iceland.png",
    difficulty: "medium"
},

{
    answers: ["λουξεμβούργο", "luxembourg"],
    image: "images/countries/luxembourg.png",
    difficulty: "medium"
},

{
    answers: ["λιχτενστάιν", "liechtenstein"],
    image: "images/countries/liechtenstein.png",
    difficulty: "hard"
},

{
    answers: ["ανδόρα", "andorra"],
    image: "images/countries/andorra.png",
    difficulty: "hard"
},

{
    answers: ["μονακό", "monaco"],
    image: "images/countries/monaco.png",
    difficulty: "medium"
},

{
    answers: ["άγιος μαρίνος", "san marino"],
    image: "images/countries/san-marino.png",
    difficulty: "hard"
},

{
    answers: ["βατικανό", "vatican", "vatican city"],
    image: "images/countries/vatican-city.png",
    difficulty: "medium"
},

{
    answers: ["μαλτα", "μάλτα", "malta"],
    image: "images/countries/malta.png",
    difficulty: "medium"
},

{
    answers: ["τυνησία", "tunisia"],
    image: "images/countries/tunisia.png",
    difficulty: "medium"
},

{
    answers: ["αλγερία", "algeria"],
    image: "images/countries/algeria.png",
    difficulty: "medium"
},

{
    answers: ["λιβύη", "libya"],
    image: "images/countries/libya.png",
    difficulty: "medium"
},

{
    answers: ["νιγηρία", "nigeria"],
    image: "images/countries/nigeria.png",
    difficulty: "medium"
},

{
    answers: ["κένυα", "kenya"],
    image: "images/countries/kenya.png",
    difficulty: "medium"
},

{
    answers: ["αιθιοπία", "ethiopia"],
    image: "images/countries/ethiopia.png",
    difficulty: "medium"
},

{
    answers: ["τανζανία", "tanzania"],
    image: "images/countries/tanzania.png",
    difficulty: "hard"
},

{
    answers: ["ζιμπάμπουε", "zimbabwe"],
    image: "images/countries/zimbabwe.png",
    difficulty: "hard"
},

{
    answers: ["ζάμπια", "zambia"],
    image: "images/countries/zambia.png",
    difficulty: "hard"
},

{
    answers: ["ναμίμπια", "namibia"],
    image: "images/countries/namibia.png",
    difficulty: "hard"
},

{
    answers: ["μποτσουάνα", "botswana"],
    image: "images/countries/botswana.png",
    difficulty: "hard"
},

{
    answers: ["βενεζουέλα", "venezuela"],
    image: "images/countries/venezuela.png",
    difficulty: "medium"
},

{
    answers: ["εκουαδόρ", "ecuador"],
    image: "images/countries/ecuador.png",
    difficulty: "medium"
},

{
    answers: ["ουρουγουάη", "uruguay"],
    image: "images/countries/uruguay.png",
    difficulty: "medium"
},

{
    answers: ["παραγουάη", "paraguay"],
    image: "images/countries/paraguay.png",
    difficulty: "medium"
},

{
    answers: ["βολιβία", "bolivia"],
    image: "images/countries/bolivia.png",
    difficulty: "medium"
},

{
    answers: ["κούβα", "cuba"],
    image: "images/countries/cuba.png",
    difficulty: "medium"
},

{
    answers: ["τζαμάικα", "jamaica"],
    image: "images/countries/jamaica.png",
    difficulty: "medium"
},

{
    answers: ["δομινικανή δημοκρατία", "dominican republic"],
    image: "images/countries/dominican-republic.png",
    difficulty: "medium"
},

{
    answers: ["κοστα ρίκα", "costa rica"],
    image: "images/countries/costa-rica.png",
    difficulty: "medium"
},

{
    answers: ["παναμάς", "panama"],
    image: "images/countries/panama.png",
    difficulty: "medium"
},


{
    answers: ["κομόρες", "comoros"],
    image: "images/countries/comoros.png",
    difficulty: "hard"
},

{
    answers: ["τζιμπουτί", "djibouti"],
    image: "images/countries/djibouti.png",
    difficulty: "hard"
},

{
    answers: ["ερυθραία", "eritrea"],
    image: "images/countries/eritrea.png",
    difficulty: "hard"
},

{
    answers: ["εσουατίνι", "eswatini", "swaziland"],
    image: "images/countries/eswatini.png",
    difficulty: "hard"
},

{
    answers: ["λεσότο", "lesotho"],
    image: "images/countries/lesotho.png",
    difficulty: "hard"
},

{
    answers: ["γκάμπια", "gambia"],
    image: "images/countries/gambia.png",
    difficulty: "hard"
},

{
    answers: ["γκουινέα", "guinea"],
    image: "images/countries/guinea.png",
    difficulty: "hard"
},

{
    answers: ["γκουινέα μπισάου", "guinea-bissau", "guinea bissau"],
    image: "images/countries/guinea-bissau.png",
    difficulty: "hard"
},

{
    answers: ["ισημερινή γουινέα", "equatorial guinea"],
    image: "images/countries/equatorial-guinea.png",
    difficulty: "hard"
},

{
    answers: ["σάο τομέ και πρίνσιπε", "sao tome and principe"],
    image: "images/countries/sao-tome-and-principe.png",
    difficulty: "hard"
},

{
    answers: ["κεντροαφρικανική δημοκρατία", "central african republic"],
    image: "images/countries/central-african-republic.png",
    difficulty: "hard"
},

{
    answers: ["μπουρκίνα φάσο", "burkina faso"],
    image: "images/countries/burkina-faso.png",
    difficulty: "hard"
},

{
    answers: ["μπουρούντι", "burundi"],
    image: "images/countries/burundi.png",
    difficulty: "hard"
},

{
    answers: ["ρουάντα", "rwanda"],
    image: "images/countries/rwanda.png",
    difficulty: "hard"
},

{
    answers: ["σομαλία", "somalia"],
    image: "images/countries/somalia.png",
    difficulty: "hard"
},

{
    answers: ["σουδάν", "sudan"],
    image: "images/countries/sudan.png",
    difficulty: "hard"
},

{
    answers: ["νότιο σουδάν", "south sudan"],
    image: "images/countries/south-sudan.png",
    difficulty: "hard"
},

{
    answers: ["μαυριτανία", "mauritania"],
    image: "images/countries/mauritania.png",
    difficulty: "hard"
},

{
    answers: ["νίγηρας", "niger"],
    image: "images/countries/niger.png",
    difficulty: "hard"
},

{
    answers: ["τσαντ", "chad"],
    image: "images/countries/chad.png",
    difficulty: "hard"
},

{
    answers: ["μπουτάν", "bhutan"],
    image: "images/countries/bhutan.png",
    difficulty: "hard"
},

{
    answers: ["μαδαγασκάρη", "madagascar"],
    image: "images/countries/madagascar.png",
    difficulty: "hard"
},

{
    answers: ["μαυρίκιος", "mauritius"],
    image: "images/countries/mauritius.png",
    difficulty: "hard"
},

{
    answers: ["σεϋχέλλες", "seychelles"],
    image: "images/countries/seychelles.png",
    difficulty: "hard"
},

{
    answers: ["μαλδίβες", "maldives"],
    image: "images/countries/maldives.png",
    difficulty: "hard"
},

{
    answers: ["ανατολικό τιμόρ", "timor-leste", "east timor"],
    image: "images/countries/timor-leste.png",
    difficulty: "hard"
},

{
    answers: ["παλάου", "palau"],
    image: "images/countries/palau.png",
    difficulty: "hard"
},

{
    answers: ["φιτζι", "φίτζι", "fiji"],
    image: "images/countries/fiji.png",
    difficulty: "hard"
},

{
    answers: ["τόνγκα", "tonga"],
    image: "images/countries/tonga.png",
    difficulty: "hard"
},

{
    answers: ["σαμόα", "samoa"],
    image: "images/countries/samoa.png",
    difficulty: "hard"
},

{
    answers: ["βανουάτου", "vanuatu"],
    image: "images/countries/vanuatu.png",
    difficulty: "hard"
},

{
    answers: ["ναουρού", "nauru"],
    image: "images/countries/nauru.png",
    difficulty: "hard"
},

{
    answers: ["τουβαλού", "tuvalu"],
    image: "images/countries/tuvalu.png",
    difficulty: "hard"
},

{
    answers: ["κιριμπάτι", "kiribati"],
    image: "images/countries/kiribati.png",
    difficulty: "hard"
},

{
    answers: ["νησιά μάρσαλ", "marshall islands"],
    image: "images/countries/marshall-islands.png",
    difficulty: "hard"
}


]

};